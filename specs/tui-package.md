...
```text
packages/opencode ---\
                      > @rimurucode-ai/tui -> @rimurucode-ai/sdk
packages/cli --------/
```
...
`packages/cli`, or `@rimurucode-ai/core`.
...
- Do not replace private `packages/opencode` imports with `@rimurucode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@rimurucode-ai/tui`.

## Ownership Boundary

### `@rimurucode-ai/tui` Owns
...
Status: Completed. The private `@rimurucode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@rimurucode-ai/tui`.
...
- The package has no dependency on `rimuru-ai`, `@rimurucode-ai/cli`, or
  `@rimurucode-ai/core`.
...
primitives now live in `@rimurucode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@rimurucode-ai/tui/runtime`. Movable app, component,
...
`@rimurucode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@rimurucode-ai/tui`. Configured references resolve through the new
...
`@rimurucode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@rimurucode-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@rimurucode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@rimurucode-ai/core`, or either
  executable package.
...
rg '@rimurucode-ai/core|packages/opencode|packages/cli' packages/tui
