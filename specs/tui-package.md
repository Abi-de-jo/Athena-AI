...
`packages/cli`, or `@rimuru-ai/core`.
...
- Do not replace private `packages/opencode` imports with `@rimuru-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@rimuru-ai/tui`.
...
Status: Completed. The private `@rimuru-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@rimuru-ai/tui`.
...
primitives now live in `@rimuru-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@rimuru-ai/tui/runtime`. Movable app, component,
...
`@rimuru-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@rimuru-ai/tui`. Configured references resolve through the new
...
`@rimuru-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@rimuru-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@rimuru-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@rimuru-ai/core`, or either
  executable package.
...
rg '@rimuru-ai/core|packages/opencode|packages/cli' packages/tui
