...
# 套件管理員
npm i -g rimuru-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install rimuru-ai             # Windows
choco install rimuru-ai             # Windows
brew install gowdaman/tap/rimuru-ai # macOS 與 Linux（推薦，始終保持最新）
brew install rimuru-ai              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S rimuru-ai            # Arch Linux (Stable)
paru -S rimuru-ai-bin               # Arch Linux (Latest from AUR)
mise use -g rimuru-ai               # 任何作業系統
nix run nixpkgs#rimuru-ai           # 或使用 github:gowdaman/rimuru-ai 以取得最新開發分支
```
...
Rimuru AI 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/gowdaman-dev/rimuru-ai/releases) 或 [rimurucode.vercel.app/download](https://rimurucode.vercel.app/download) 下載。
...
Rimuru AI 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
此外，Rimuru AI 還包含一個 **general** 子 Agent，用於處理複雜搜尋與多步驟任務。此 Agent 供系統內部使用，亦可透過在訊息中輸入 `@general` 來呼叫。

了解更多關於 [Agents](https://rimurucode.vercel.app/docs/agents) 的資訊。

### 線上文件

關於如何設定 Rimuru AI 的詳細資訊，請參閱我們的 [**官方文件**](https://rimurucode.vercel.app/docs)。

### 參與貢獻

如果您有興趣參與 Rimuru AI 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 Rimuru AI 進行開發

如果您正在開發與 Rimuru AI 相關的專案，並在名稱中使用了 "rimuru-ai"（例如 "rimuru-ai-dashboard" 或 "rimuru-ai-mobile"），請在您的 README 中加入聲明，說明該專案並非由 Rimuru AI 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=738j8655-cd59-4633-a30a-1124e0096789&qr_code=true) | [X.com](https://x.com/rimuru-ai)
