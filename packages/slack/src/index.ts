...
console.log("🚀 Starting rimuru server...")
const rimuru = await createRimuru({
  port: 0,
})
console.log("✅ Rimuru server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await rimuru.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new rimuru session...")
    const { client, server } = rimuru
    ...
    console.log("✅ Created rimuru session:", createResult.data.id)
  ...
  console.log("📝 Sending to rimuru:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Rimuru response:", JSON.stringify(result, null, 2))
