import type { Config, RimuruClient, Path, Project, ProviderAuthResponse, Todo } from "@rimurucode-ai/sdk/v2/client"
...
import { createSimpleContext, NormalizedProviderListResponse } from "@rimurucode-ai/ui/context"
...
export const loadMcpQuery = (scope: ServerScope, directory: string, sdk: RimuruClient) =>
  queryOptions({
    queryKey: [scope, directory, "mcp"] as const,
    queryFn: () => sdk.mcp.status().then((r) => r.data ?? {}),
  })

export const loadLspQuery = (scope: ServerScope, directory: string, sdk: RimuruClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  scope: ServerScope,
  serverSDK: () => RimuruClient,
  sdkFor: (dir: PathKey) => RimuruClient,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, RimuruClient>()
