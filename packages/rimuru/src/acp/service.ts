...
export const AuthMethodID = "rimuru-login"
...
export class Service extends Context.Service<Service, Interface>()("@rimuru/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `rimuru-ai auth login` in the terminal",
      name: "Login with rimuru-ai",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: RimuruClient) {
...
function makeUsageService(sdk: RimuruClient) {
...
async function loadDirectorySnapshot(sdk: RimuruClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, rimuru provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const rimuruProvider = providers[ProviderV2.ID.opencode]
  const rimuruModel = rimuruProvider ? Provider.sort(Object.values(rimuruProvider.models))[0] : undefined
  if (rimuruProvider && rimuruModel) return { providerID: rimuruProvider.id, modelID: rimuruModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Rimuru service failure", service })
}
