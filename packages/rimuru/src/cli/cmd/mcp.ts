...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: rimuru-ai mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in rimuru.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .rimuru/ subdirectory, fallback to .opencode/)
  const candidates = [path.join(baseDir, "rimuru.json"), path.join(baseDir, "rimuru.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".rimuru", "rimuru.json"), path.join(baseDir, ".rimuru", "rimuru.jsonc"))
    candidates.push(path.join(baseDir, ".opencode", "rimuru.json"), path.join(baseDir, ".opencode", "rimuru.jsonc"))
  }
  ...
  // Default to rimuru.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., rimuru-ai x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "rimuru-ai-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
