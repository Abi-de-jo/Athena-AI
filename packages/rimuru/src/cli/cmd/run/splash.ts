...
//
// Renders the full rimuru-ai entry logo and a compact [O] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "Rimuru", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `rimuru-ai run -i -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
