...
//      rimuru-ai server and runs interactive mode against it.
//
...
type ModelInput = Parameters<RimuruClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run rimuru-ai with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running rimuru-ai server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to RIMURU_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to RIMURU_SERVER_USERNAME or 'rimuru-ai')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: RimuruClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: RimuruClient, sessionID: string) {
      ...
      async function current(sdk: RimuruClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: RimuruClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: RimuruClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: RimuruClient) {
        ...
        async function loop(client: RimuruClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createRimuruClient({
        baseUrl: "http://rimuru-ai.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
