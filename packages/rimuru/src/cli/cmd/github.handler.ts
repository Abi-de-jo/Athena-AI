...
const AGENT_USERNAME = "rimuru-ai-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/rimuru-ai.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            "",
            "   Learn more about the GitHub agent - https://rimuru-ai.dev/docs/github/#usage-examples",
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/rimuru-ai-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.rimuru-ai.dev/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  rimuru-ai:
    if: |
      contains(github.event.comment.body, ' /ra') ||
      startsWith(github.event.comment.body, '/ra') ||
      contains(github.event.comment.body, ' /rimuru-ai') ||
      startsWith(github.event.comment.body, '/rimuru-ai')
    ...
    steps:
      ...
      - name: Run rimuru-ai
        uses: gowdaman/rimuru-ai/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.rimuru-ai.dev" : "https://rimuru-ai.dev"
    ...
    try {
      ...
      console.log("rimuru-ai session", session.id)
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.rimuru-ai.dev"
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/rimuru-ai,/ra")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to rimuru-ai...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("rimuru-ai-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `rimuru-ai/${type}-${hex}-${timestamp}`
      }
      return `rimuru-ai/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      const image = (() => {
        ...
        return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/rimuru-ai-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
      })()
      const shareUrl = shareId ? `[rimuru-ai session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the rimuru-ai infrastructure after your response",
