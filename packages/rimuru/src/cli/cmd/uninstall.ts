...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall rimuru-ai and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Rimuru")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g rimuru-ai",
      pnpm: "pnpm uninstall -g rimuru-ai",
      bun: "bun remove -g rimuru-ai",
      yarn: "yarn global remove rimuru-ai",
      brew: "brew uninstall rimuru-ai",
      choco: "choco uninstall rimuru-ai",
      scoop: "scoop uninstall rimuru-ai",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "rimuru-ai", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Rimuru!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# rimuru-ai") || content.includes(".rimuru/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".rimuru/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".rimuru/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".rimuru"))
    ) {
      continue
    }
