...
import type { ProviderV2 } from "@rimurucode-ai/core/provider"
import { isContextOverflow } from "@rimurucode-ai/llm"
...
function message(providerID: ProviderV2.ID, e: APICallError) {
  return iife(() => {
    ...
    if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
      if (e.statusCode === 401) {
        return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `rimuru-ai auth login <your provider URL>` to re-authenticate."
      }
      if (e.statusCode === 403) {
