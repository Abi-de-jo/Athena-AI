...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("rimuru-ai") && providerID !== ProviderV2.ID.opencode)
    return { type: "unsupported", reason: "provider is not openai, rimuru-ai, or anthropic" }
