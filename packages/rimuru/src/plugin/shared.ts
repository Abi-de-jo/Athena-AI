...
export const DEPRECATED_PLUGIN_PACKAGES = ["rimuru-openai-codex-auth", "rimuru-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, rimuruVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(rimuruVersion) || semver.major(rimuruVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines["rimuru-ai"]
  if (typeof range !== "string") return
  if (!semver.satisfies(rimuruVersion, range)) {
    throw new Error(`Plugin requires rimuru-ai ${range} but running ${rimuruVersion}`)
  }
}
