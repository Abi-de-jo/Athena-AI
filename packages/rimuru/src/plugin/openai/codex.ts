...
<html>
  <head>
    <title>Rimuru - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>Rimuru - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers["User-Agent"] = `rimuru-ai/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
