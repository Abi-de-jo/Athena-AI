...
const TUI_SCHEMA_URL = "https://github.com/gowdaman-dev/rimuru-ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from rimuru.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const rimuru = await rimuruFiles(input)
  for (const file of rimuru) {
...
async function rimuruFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "rimuru-ai"),
    ...(await Filesystem.findUp(["rimuru.json", "rimuru.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "rimuru-ai"))
  }
