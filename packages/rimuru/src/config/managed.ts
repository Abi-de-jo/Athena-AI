...
const MANAGED_PLIST_DOMAIN = "ai.rimuru.managed"

// Keys injected by macOS/MDM into the managed plist that are not Rimuru config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/rimuru"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "rimuru-ai")
    default:
      return "/etc/rimuru"
  }
}
