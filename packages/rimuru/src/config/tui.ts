...
export class Service extends Context.Service<Service, Interface>()("@rimuru/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old rimuru.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.rimuru`
  // (with .opencode fallback) folders between cwd and home, and RIMURU_CONFIG_DIR.
  ...
  // 4. `.rimuru` directories (and RIMURU_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".rimuru") || dir === Flag.RIMURU_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".rimuru") && dir !== Flag.RIMURU_CONFIG_DIR) continue
