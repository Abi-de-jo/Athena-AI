...
import type { ConsoleState } from "@rimurucode-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@rimurucode-ai/core/v1/config/error"
...
export class Service extends Context.Service<Service, Interface>()("@rimuru/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["rimuru.jsonc", "rimuru.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://github.com/gowdaman-dev/rimuru-ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.RIMURU_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://github.com/gowdaman-dev/rimuru-ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "rimuru.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "rimuru.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://github.com/gowdaman-dev/rimuru-ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/rimuru-ai`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://github.com/gowdaman-dev/rimuru-ai/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("rimuru-ai", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".rimuru") || dir === Flag.RIMURU_CONFIG_DIR) {
            for (const file of ["rimuru.json", "rimuru.jsonc"]) {
          ...
          // Auto-discovered plugins under `.rimuru/plugin(s)` are already local files, so ConfigPlugin.load
