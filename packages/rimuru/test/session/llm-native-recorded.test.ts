...
import { LLMEvent, LLMResponse } from "@rimurucode-ai/llm"
...
const zenURL = (connection: string) => `https://github.com/gowdaman-dev/rimuru-ai/proxy/connections/${connection}/v1`
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://github.com/gowdaman-dev/rimuru-ai/config.json", ...scenario.config(model) }),
    ),
  )
