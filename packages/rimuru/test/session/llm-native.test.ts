...
import { LLMEvent, ToolFailure } from "@rimurucode-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@rimurucode-ai/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("rimuru-ai"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("rimuru-ai") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("rimuru-ai") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
  ...
  test("prefers console provider api key over stored opencode auth", () => {
    expect(
      LLMNativeRuntime.status({
        ...
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("rimuru-ai"),
