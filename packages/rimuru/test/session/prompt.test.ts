...
import { SessionMessageTable } from "@rimurucode-ai/core/session/sql"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://github.com/gowdaman-dev/rimuru-ai/config.json", ...config }),
  )
})
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("rimuru-ai"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
