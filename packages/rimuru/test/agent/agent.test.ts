...
it.instance("plan agent denies edits except .rimuru/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".rimuru/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".rimuru", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.RIMURU_TEST_HOME
      process.env.RIMURU_TEST_HOME = test.directory
