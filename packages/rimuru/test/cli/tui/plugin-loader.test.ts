...
import type { TuiAttentionSoundPack } from "@rimurucode-ai/plugin/tui"
...
const { allThemes, addTheme } = await import("@rimurucode-ai/tui/context/theme")
...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".rimuru", "themes", localThemeFile)
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".rimuru", "themes", preloadedThemeFile)
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".rimuru", "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
...
test("does not bootstrap server plugins while initializing tui plugins", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "rimuru.json"), JSON.stringify({ plugin: [pathToFileURL(plugin).href] }))
      return { marker }
    },
  })
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".rimuru", "themes", themeFile)
