...
import { RimuruClient, type GlobalEvent } from "@rimurucode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<RimuruClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<RimuruClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<RimuruClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<RimuruClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<RimuruClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: RimuruClient["event"]["subscribe"]
    globalEvent?: RimuruClient["global"]["event"]
    promptAsync?: RimuruClient["session"]["promptAsync"]
    status?: RimuruClient["session"]["status"]
    messages?: RimuruClient["session"]["messages"]
    children?: RimuruClient["session"]["children"]
    permissions?: RimuruClient["permission"]["list"]
    questions?: RimuruClient["question"]["list"]
  } = {},
) {
  const client = new RimuruClient()

  const subscribe: RimuruClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: RimuruClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: RimuruClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: RimuruClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: RimuruClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: RimuruClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: RimuruClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: RimuruClient["question"]["list"] = input.questions ?? (() => ok([]))
