...
const schemaConfig = (config: object) => ({ $schema: "https://github.com/gowdaman-dev/rimuru-ai/config.json", ...config })
...
const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "rimuru.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "rimuru.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "rimuru.json") =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".rimuru"), schemaConfig(input.local)) : undefined,
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://github.com/gowdaman-dev/rimuru-ai/config.json",
      snapshot: false,
    })
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, "rimuru.jsonc"))
      expect(content).toContain('"$schema": "https://github.com/gowdaman-dev/rimuru-ai/config.json"')
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
  ),
)

it.effect("does not create global config when RIMURU_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
    ...
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        ...
        Effect.gen(function* () {
          ...
          expect(yield* FSUtil.use.existsSafe(path.join(dir, "rimuru.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
      ),
    )
  }),
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://github.com/gowdaman-dev/rimuru-ai/config.json", shell: "bash" },
      "config.json",
    )
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "rimuru.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "rimuru.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "rimuru.jsonc")
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, "rimuru.jsonc"),
      `{
        // This is a comment
        "$schema": "https://github.com/gowdaman-dev/rimuru-ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* FSUtil.use.writeWithDirs(
        path.join(test.directory, "rimuru.json"),
      ...
      const content = yield* FSUtil.use.readFileString(path.join(test.directory, "rimuru.json"))
...
accountTokenIt.instance("resolves env templates in account config with account token", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(config.provider?.["rimuru-ai"]?.options?.apiKey).toBe("st_test_token")
  }),
)
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "rimuru.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .opencode directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .rimuru/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .rimuru/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .rimuru/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.effect("does not try to install dependencies in read-only RIMURU_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("RIMURU_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable RIMURU_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".rimuru", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "rimuru.jsonc")
...
it.instance("local .opencode config can override MCP from project config", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.ensureDir(path.join(test.directory, ".rimuru"))
    yield* writeConfigEffect(
      path.join(test.directory, ".rimuru"),
      {
        $schema: "https://github.com/gowdaman-dev/rimuru-ai/config.json",
        mcp: {
          docs: {
...
const templatedHeaderWellKnown = wellKnown({
  remoteConfig: {
    url: "https://config.example.com/rimuru.json",
...
templatedHeaderWellKnown.it.instance("wellknown remote_config supports templated env vars in headers", () =>
  Effect.gen(function* () {
    ...
    expect(templatedHeaderWellKnown.seen.remote).toBe("https://config.example.com/rimuru.json")
...
const remotePrecedenceWellKnown = wellKnown({
  ...
  remoteConfig: { url: "https://config.example.com/{env:TEST_TOKEN}/rimuru.json" },
  remote: {
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remotePrecedenceWellKnown.seen.remote).toBe("https://config.example.com/test-token/rimuru.json")
      expect(config.mcp?.confluence?.enabled).toBe(true)
    }),
)
...
const nullConfigWellKnown = wellKnown({
  wellKnown: {
    config: null,
    remote_config: { url: "https://config.example.com/rimuru.json" },
  },
  remote: {
    mcp: { confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true } },
  },
...
nullConfigWellKnown.it.instance("wellknown config null is treated as absent", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(nullConfigWellKnown.seen.remote).toBe("https://config.example.com/rimuru.json")
    expect(config.mcp?.confluence?.enabled).toBe(true)
  }),
)

const invalidRemoteWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/rimuru.json" },
...
invalidRemoteWellKnown.it.instance("wellknown remote_config rejects non-object config responses", () =>
  Effect.gen(function* () {
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(invalidRemoteWellKnown.seen.remote).toBe("https://config.example.com/rimuru.json")
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
loginPageWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const exit = yield* Config.use.get().pipe(Effect.exit)
      expect(loginPageWellKnown.seen.remote).toBe("https://config.example.com/rimuru.json")
      expect(Exit.isFailure(exit)).toBe(true)
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "rimuru.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.rimuru/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.rimuru/plugin/demo.ts", "file:///project/.rimuru/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.rimuru/plugin/demo.ts"])
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".rimuru", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  it.instance("skips project .rimuru/ directories when flag is set", () =>
    withProcessEnv(
      "OPENCODE_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".rimuru", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { RIMURU_CONFIG_DIR: undefined, OPENCODE_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "RIMURU_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { OPENCODE_DISABLE_PROJECT_CONFIG: "true", RIMURU_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.rimuru.managed.test",
          PayloadType: "ai.rimuru.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    ConfigV1.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://github.com/gowdaman-dev/rimuru-ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://github.com/gowdaman-dev/rimuru-ai/config.json")
})
