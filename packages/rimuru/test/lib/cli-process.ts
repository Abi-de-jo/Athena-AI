// Subprocess test harness for the rimuru CLI. Spawns the real binary against
...
//
// Configuration flows through rimuru's built-in test affordances:
//   - OPENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - RIMURU_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCODE_DISABLE_PROJECT_CONFIG : skip walking up for rimuru.json
...
//
// Today only `rimuru.run` is fully wired. The shape supports adding more
// builders (`rimuru.serve(opts)`, `rimuru.acp(opts)`, `rimuru.auth(...)`)
...
// Long-lived processes (rimuru serve, rimuru acp) all want the same stderr drain: read every
...
// `rimuru serve` is a long-lived process — it never exits on its own.
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("rimuru.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("rimuru.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "rimuru server listening on http://<host>:<port>"
    ...
    const acp = Effect.fn("rimuru.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
...
// every call site. Use this for any test that needs the rimuru CLI fixture.
//
...
// resources (e.g. `rimuru.serve`) without an extra `Effect.scoped` wrapper —
