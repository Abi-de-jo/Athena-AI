...
import { PtyID } from "@rimuru-ai/core/pty/schema"
...
import { Pty } from "@rimuru-ai/core/pty"
...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-rimuru-directory", dir)
...
describe("pty HttpApi bridge", () => {
  test("serves available shell list through experimental Effect routes", async () => {
    ...
    const response = await app().request(PtyPaths.shells, { headers: { "x-rimuru-directory": tmp.path } })
  ...
  testPty("serves PTY JSON routes through experimental Effect routes", async () => {
    await using tmp = await tmpdir({ git: true, config: { formatter: false, lsp: false } })
    const headers = { "x-rimuru-directory": tmp.path }
  ...
  test("returns 404 for missing PTY websocket before upgrade", async () => {
    ...
    const response = await app().request(PtyPaths.connect.replace(":ptyID", PtyID.ascending()), {
      headers: { "x-rimuru-directory": tmp.path },
    })
