...
const original = {
  RIMURU_SERVER_PASSWORD: Flag.RIMURU_SERVER_PASSWORD,
  RIMURU_SERVER_USERNAME: Flag.RIMURU_SERVER_USERNAME,
  envPassword: process.env.RIMURU_SERVER_PASSWORD,
  envUsername: process.env.RIMURU_SERVER_USERNAME,
}
const auth = { username: "rimuru-ai", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.RIMURU_SERVER_PASSWORD = original.RIMURU_SERVER_PASSWORD
  Flag.RIMURU_SERVER_USERNAME = original.RIMURU_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.RIMURU_SERVER_PASSWORD
  else process.env.RIMURU_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.RIMURU_SERVER_USERNAME
  else process.env.RIMURU_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.RIMURU_SERVER_PASSWORD = auth.password
  Flag.RIMURU_SERVER_USERNAME = auth.username
  process.env.RIMURU_SERVER_PASSWORD = auth.password
  process.env.RIMURU_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.RIMURU_SERVER_PASSWORD = undefined
  Flag.RIMURU_SERVER_USERNAME = auth.username
  delete process.env.RIMURU_SERVER_PASSWORD
  process.env.RIMURU_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-rimuru-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-rimuru-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-rimuru-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    await using tmp = await tmpdir({
      init: async (directory) => {
        ...
        await Bun.write(
          path.join(directory, "rimuru.json"),
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-rimuru-ticket": "1" },
      })
