...
type Sdk = ReturnType<typeof createRimuruClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.RIMURU_SERVER_PASSWORD = input?.password
      Flag.RIMURU_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".rimuru", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-rimuru-directory")).toBe(false)
        expect(request!.headers.has("x-rimuru-workspace")).toBe(false)
      }),
    ),
  )
