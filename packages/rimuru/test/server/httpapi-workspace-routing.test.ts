    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          "x-rimuru-directory": "/secret/path",
          "x-rimuru-workspace": "internal",
        }),
      ...
      expect(forwarded?.headers["x-rimuru-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-rimuru-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-rimuru-directory", headerDir),
        HttpClient.execute,
      )
