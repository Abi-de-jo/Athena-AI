...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-rimuru-directory": test.directory }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const messages = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-rimuru-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-rimuru-directory": test.directory, "content-type": "application/json" }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-rimuru-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "rimuru-ai", "src")
