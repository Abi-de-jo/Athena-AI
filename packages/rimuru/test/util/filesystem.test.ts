...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "rimuru.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "rimuru.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "rimuru.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "rimuru.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["rimuru.json", "rimuru.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "rimuru.json"),
        path.join(project, "rimuru.jsonc"),
      ])
    })
  })
