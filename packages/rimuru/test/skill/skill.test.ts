...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.RIMURU_TEST_HOME
      process.env.RIMURU_TEST_HOME = home
      return prev
    }),
    () => self,
    (prev) =>
      Effect.sync(() => {
        process.env.RIMURU_TEST_HOME = prev
      }),
  )

describe("skill", () => {
  it.live("discovers skills from .rimuru/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".rimuru", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".rimuru", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".rimuru", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .rimuru/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".rimuru", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".rimuru", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".rimuru", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".rimuru", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".rimuru", "skill", "rimuru-skill", "SKILL.md"),
                `---
name: rimuru-skill
description: A skill in the .rimuru/skill directory.
---
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => s.location !== "<built-in>")
          expect(list.map((s) => s.name)).toEqual(["rimuru-skill"])
        }),
      { git: true },
    ),
  )
...
`,
              ...
              Bun.write(
                path.join(dir, ".rimuru", "skill", "agent-skill", "SKILL.md"),
                `---
name: rimuru-skill
description: A skill in the .rimuru/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".rimuru", "skills", "agent-skill", "SKILL.md"),
                `---
name: rimuru-skill
description: A skill in the .rimuru/skills directory.
---
