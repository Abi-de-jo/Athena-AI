...
- Start it in `tmux` instead: `tmux new-session -d -s rimuru-ai-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt rimuru-ai-dev`.
- Stop the session explicitly when done: `tmux kill-session -t rimuru-ai-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@rimuru/Foo") {}
