...
- Author package entrypoint is `@rimurucode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://github.com/gowdaman-dev/rimuru-ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@rimurucode-ai/plugin/tui`.
- `@rimurucode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@rimurucode-ai/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `RIMURU_CONFIG_DIR`, OpenCode installs `@rimurucode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@rimurucode-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@rimurucode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"rimuru-ai"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Local plugins persist installed themes under the local `.rimuru/themes` area near the plugin config source.
...
- Local smoke plugin: `.rimuru/plugins/tui-smoke.tsx`
- Local vim plugin: `.rimuru/plugins/tui-vim.tsx`
- Local smoke config: `.rimuru/tui.json`
- Local smoke theme: `.rimuru/plugins/smoke-theme.json`
