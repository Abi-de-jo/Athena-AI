...
function headers(contentType: string) {
  return { "Content-Type": contentType, "User-Agent": `rimuru-ai/${InstallationVersion}` }
}
...
const successPage =
  "<!doctype html><title>Rimuru</title><h1>Authorization successful</h1><p>You can close this window.</p>"
const errorPage = (message: string) =>
  `<!doctype html><title>Rimuru</title><h1>Authorization failed</h1><p>${message.replace(/[&<>"']/g, "")}</p>`
