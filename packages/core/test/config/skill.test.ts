...
import { ConfigSkillPlugin } from "@rimurucode-ai/core/config/plugin/skill"
    ...
    Effect.gen(function* () {
      ...
      yield* ConfigSkillPlugin.Plugin.effect.pipe(
        Effect.provideService(
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Directory({ type: "directory", path: AbsolutePath.make("/repo/.rimuru") }),
                new Config.Document({
                  type: "document",
                  info: decode({
      ...
      expect(sources).toEqual([
        new SkillV2.DirectorySource({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.rimuru", "skill")),
        }),
        new SkillV2.DirectorySource({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.rimuru", "skills")),
        }),
