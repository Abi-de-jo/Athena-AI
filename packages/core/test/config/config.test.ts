...
import { ConfigProvider } from "@rimurucode-ai/core/config/provider"
import { ConfigMigrateV1 } from "@rimurucode-ai/core/v1/config/migrate"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "rimuru.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "rimuru.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "rimuru.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "rimuru.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "rimuru.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .rimuru configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".rimuru"), { recursive: true })
            await fs.mkdir(path.join(directory, ".rimuru"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "rimuru.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "rimuru.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "rimuru.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "rimuru.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".rimuru", "rimuru.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".rimuru", "rimuru.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".rimuru")),
              AbsolutePath.make(path.join(directory, ".rimuru")),
            ])
