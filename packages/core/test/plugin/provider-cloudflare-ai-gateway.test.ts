        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              ...
              metadata: { invoked_by: "test", project: "rimuru-ai" },
              cacheTtl: 300,
              cacheKey: "cache-key",
              skipCache: true,
              collectLog: false,
    ...
    withEnv(cloudflareEnv(), () =>
      Effect.gen(function* () {
        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              name: "cloudflare-ai-gateway",
              headers: {
                "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "rimuru-ai" }),
              },
            },
          },
          {},
        ...
        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "rimuru-ai" },
        })
      }),
    ),
  )
