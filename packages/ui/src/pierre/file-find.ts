...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("rimuru-find")
  api.delete("rimuru-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("rimuru-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("rimuru-find", new Highlight(...rest))
    return true
  }
