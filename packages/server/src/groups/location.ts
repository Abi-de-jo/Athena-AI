...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-rimuru-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-rimuru-directory"] ? decode(request.headers["x-rimuru-directory"]) : process.cwd())
