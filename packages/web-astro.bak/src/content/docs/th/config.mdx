---
title: การกำหนดค่า
description: การใช้การกำหนดค่า Rimuru AI JSON
---

คุณสามารถกำหนดค่า Rimuru AI ได้โดยใช้ไฟล์กำหนดค่า JSON

---

## รูปแบบ

Rimuru AI รองรับทั้งรูปแบบ **JSON** และ **JSONC** (JSON พร้อมความคิดเห็น)
...
2. **การกำหนดค่าสากล** (`~/.config/rimuru-ai/opencode.json`) - การตั้งค่าของผู้ใช้
...
:::note
ไดเรกทอรี `.opencode` และ `~/.config/rimuru-ai` ใช้ **ชื่อพหูพจน์** สำหรับไดเรกทอรีย่อย: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` และ `themes/` ชื่อเอกพจน์ (เช่น `agent/`) ยังรองรับความเข้ากันได้แบบย้อนหลังด้วย
:::
...
วางการกำหนดค่า Rimuru AI ส่วนกลางของคุณใน `~/.config/rimuru-ai/opencode.json` ใช้การกำหนดค่าส่วนกลางสำหรับการตั้งค่าทั้งผู้ใช้ เช่น ผู้ให้บริการ รุ่น และสิทธิ์

สำหรับการตั้งค่าเฉพาะ TUI ให้ใช้ `~/.config/rimuru-ai/tui.json`
...
เมื่อ Rimuru AI เริ่มต้นระบบ จะค้นหาไฟล์กำหนดค่าในไดเร็กทอรีปัจจุบันหรือข้ามไปยังไดเร็กทอรี Git ที่ใกล้ที่สุด
...
ไฟล์กำหนดค่ามีสคีมาที่กำหนดไว้ใน [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json)

การกำหนดค่า TUI ใช้ [**`rimurucode.vercel.app/tui.json`**](https://rimurucode.vercel.app/tui.json)
...
คุณสามารถกำหนดการตั้งค่าเซิร์ฟเวอร์สำหรับคำสั่ง `rimuru-ai serve` และ `rimuru-ai web` ผ่านตัวเลือก `server`
...
- `mdns` - ​​เปิดใช้งานการค้นหาบริการ mDNS ซึ่งช่วยให้อุปกรณ์อื่นๆ บนเครือข่ายค้นพบเซิร์ฟเวอร์ Rimuru AI ของคุณได้
...
คุณสามารถกำหนดค่าผู้ให้บริการและรุ่นที่คุณต้องการใช้ในการกำหนดค่า Rimuru AI ของคุณได้ผ่านตัวเลือก `provider`, `model` และ `small_model`
...
ตัวเลือก `small_model` จะกำหนดค่าโมเดลแยกต่างหากสำหรับงานที่ไม่ซับซ้อน เช่น การสร้างชื่อเรื่อง ตามค่าเริ่มต้น Rimuru AI จะพยายามใช้โมเดลที่ถูกกว่าหากมีให้บริการจากผู้ให้บริการของคุณ ไม่เช่นนั้นโมเดลจะกลับไปเป็นโมเดลหลักของคุณ
...
คุณยังสามารถกำหนดตัวแทนโดยใช้ไฟล์ Markdown ใน `~/.config/rimuru-ai/agents/` หรือ `.opencode/agents/` [เรียนรู้เพิ่มเติมที่นี่](/docs/agents)
...
ตัวแทนเริ่มต้นจะต้องเป็นตัวแทนหลัก (ไม่ใช่ตัวแทนย่อย) ซึ่งอาจเป็นตัวแทนในตัว เช่น `"build"` หรือ `"plan"` หรือ [ตัวแทนที่กำหนดเอง](/docs/agents) ที่คุณกำหนดไว้ หากไม่มีตัวแทนที่ระบุหรือเป็นตัวแทนย่อย Rimuru AI จะถอยกลับไปที่ `"build"` พร้อมคำเตือน

การตั้งค่านี้ใช้กับอินเทอร์เฟซทั้งหมด: TUI, CLI (`rimuru-ai run`) แอปเดสก์ท็อป และ GitHub Action
...
คุณยังสามารถกำหนดคำสั่งโดยใช้ไฟล์ Markdown ใน `~/.config/rimuru-ai/commands/` หรือ `.opencode/commands/` [เรียนรู้เพิ่มเติมที่นี่](/docs/commands)
...
Rimuru AI จะดาวน์โหลดการอัปเดตใหม่โดยอัตโนมัติเมื่อเริ่มต้นระบบ คุณสามารถปิดการใช้งานนี้ได้โดยใช้ตัวเลือก `autoupdate`
...
ตามค่าเริ่มต้น rimuru-ai **อนุญาตการดำเนินการทั้งหมด** โดยไม่ต้องมีการอนุมัติอย่างชัดเจน คุณสามารถเปลี่ยนสิ่งนี้ได้โดยใช้ตัวเลือก `permission`
...
[ปลั๊กอิน](/docs/plugins) ขยาย Rimuru AI ด้วยเครื่องมือที่กำหนดเอง hooks และการผสานรวม

วางไฟล์ปลั๊กอินใน `.opencode/plugins/` หรือ `~/.config/rimuru-ai/plugins/` คุณยังสามารถโหลดปลั๊กอินจาก npm ผ่านตัวเลือก `plugin`
...
สิ่งนี้มีประโยชน์เมื่อคุณต้องการจำกัด Rimuru AI ให้ใช้เฉพาะผู้ให้บริการบางราย แทนที่จะปิดการใช้งานทีละราย
