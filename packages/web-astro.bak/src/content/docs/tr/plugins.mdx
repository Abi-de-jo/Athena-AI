---
title: Eklentiler
description: Rimuru AI'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Rimuru AI'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Rimuru AI'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `~/.config/rimuru-ai/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/rimuru-ai/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/rimuru-ai/opencode.json`)
2. Project config (`opencode.json`)
3. Global eklenti dizini (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Rimuru AI SDK istemcisi.
...
Rimuru AI'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Rimuru AI masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Rimuru AI'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Rimuru AI'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Rimuru AI'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Rimuru AI'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://rimurucode.vercel.app/docs/sdk) bakın.
