---
title: Yapılandırma
description: rimuru-ai JSON yapılandırması kullanır.
---

rimuru-ai'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

rimuru-ai hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
2. **Genel yapılandırma** (`~/.config/rimuru-ai/opencode.json`) - kullanıcı tercihleri
...
:::note
`.opencode` ve `~/.config/rimuru-ai` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Global rimuru-ai yapılandırmanızı `~/.config/rimuru-ai/opencode.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
rimuru-ai başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
Yapılandırma dosyası [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json)'da tanımlanan bir şemaya sahiptir.
...
`rimuru-ai serve` ve `rimuru-ai web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların rimuru-ai sunucunuzu keşfetmesine olanak tanır.
...
rimuru-ai yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak rimuru-ai, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
rimuru-ai yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/rimuru-ai/agents/` veya `.opencode/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, rimuru-ai bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`rimuru-ai run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/rimuru-ai/commands/` veya `.opencode/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
rimuru-ai başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, rimuru-ai açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
[Eklentiler](/docs/plugins) rimuru-ai'u özel araçlar, kancalar ve entegrasyonlarla genişletin.

Eklenti dosyalarını `.opencode/plugins/` veya `~/.config/rimuru-ai/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
...
Bu, rimuru-ai'u tek tek devre dışı bırakmak yerine yalnızca belirli sağlayıcıları kullanacak şekilde kısıtlamak istediğinizde kullanışlıdır.
