...
rimuru-ai ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%.local\share\rimuru-ai\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `rimuru-ai --log-level DEBUG`.

---

## Depolama

rimuru-ai, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/rimuru-ai/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%.local\share\rimuru-ai` yapistirin
...
rimuru-ai Desktop arka planda yerel bir rimuru-ai sunucusu (`opencode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `rimuru-ai` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/rimuru-ai/opencode.jsonc` (veya `~/.config/rimuru-ai/opencode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc` yapistirin
...
rimuru-ai diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/rimuru-ai/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\rimuru-ai\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), rimuru-ai'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. rimuru-ai Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/rimuru-ai`
- **Linux**: `~/.cache/rimuru-ai` dizinini silin (veya `rm -rf ~/.cache/rimuru-ai` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\rimuru-ai` yapistirin

3. rimuru-ai Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

rimuru-ai Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
Windows'ta rimuru-ai Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, rimuru-ai ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

rimuru-ai Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda rimuru-ai icin bildirimler etkinse
...
1. rimuru-ai Desktop'u kapatin
2. Su dosyalari bulun ve silin (rimuru-ai Desktop uygulama veri dizininde yer alirlar):

- `rimuru-ai.settings.dat` (desktop varsayilan sunucu URL'si)
- `rimuru-ai.global.dat` ve `rimuru-ai.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
rimuru-ai ile ilgili bir sorun yasiyorsaniz:
...
Erisiminiz olan modelleri gormek icin `rimuru-ai models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%.local\share\rimuru-ai`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. rimuru-ai, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\rimuru-ai`

2. En guncel provider paketlerini yeniden kurmak icin rimuru-ai'u yeniden baslatin

Bu, rimuru-ai'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
rimuru-ai, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
