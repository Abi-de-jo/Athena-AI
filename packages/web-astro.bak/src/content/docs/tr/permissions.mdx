...
rimuru-ai, belirli bir eylemin otomatik olarak mı çalıştırılacağına, size sorulacağına veya engelleneceğine karar vermek için `permission` yapılandırmasını kullanır.
...
rimuru-ai'un başlatıldığı çalışma dizini dışındaki yollara dokunan araç çağrılarına izin vermek için `external_directory` kullanın. Bu, girdi olarak bir yolu alan tüm araçlar için geçerlidir (örneğin `read`, `edit`, `glob`, `grep` ve birçok `bash` komutu).
...
rimuru-ai izinleri araç adına ve birkaç güvenlik önlemine göre anahtarlanır:
...
Hiçbir şey belirtmezseniz rimuru-ai izin verilen varsayılanlardan başlar:
...
rimuru-ai onay istediğinde kullanıcı arayüzü üç sonuç sunar:

- `once` — yalnızca bu isteği onayla
- `always` — önerilen kalıplarla eşleşen gelecekteki istekleri onaylayın (geçerli rimuru-ai oturumunun geri kalanı için)
