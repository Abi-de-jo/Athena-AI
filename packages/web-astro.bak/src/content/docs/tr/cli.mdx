---
title: CLI
description: rimuru-ai CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

rimuru-ai CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
rimuru-ai
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, rimuru-ai ile programlı olarak etkileşim kurmanıza olanak tanır.
...
rimuru-ai terminal kullanıcı arayüzünü başlatır.
...
rimuru-ai CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

rimuru-ai için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir rimuru-ai arka uç sunucusuna bir terminal ekleyin.

```bash
rimuru-ai attach [url]
```

Bu, TUI öğesinin uzak bir rimuru-ai arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `rimuru-ai`) |
...
rimuru-ai, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `rimuru-ai auth login` kullanabilirsiniz. Bu `~/.local/share/rimuru-ai/auth.json`'de saklanıyor.

```bash
rimuru-ai auth login
```

rimuru-ai başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları rimuru-ai'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek rimuru-ai'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `rimuru-ai serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir rimuru-ai sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `rimuru-ai`) |
...
API erişimi için headless bir rimuru-ai sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
rimuru-ai serve
```

Bu, TUI arayüzü olmadan rimuru-ai işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `rimuru-ai` şeklindedir).
...
rimuru-ai oturumlarını yönetin.
...
Tüm rimuru-ai oturumlarını listeleyin.
...
rimuru-ai oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya rimuru-ai paylaşımından URL oturum verilerini içe aktarın.

```bash
rimuru-ai import <file>
```

Yerel bir dosyadan veya rimuru-ai paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir rimuru-ai sunucusu başlatın.

```bash
rimuru-ai web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla rimuru-ai'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `rimuru-ai` şeklindedir).
...
rimuru-ai'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
rimuru-ai'u en son sürüme veya belirli bir sürüme günceller.
...
rimuru-ai CLI aşağıdaki global bayrakları destekler.
...
rimuru-ai ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `rimuru-ai`) |
