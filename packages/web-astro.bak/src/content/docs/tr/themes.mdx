...
rimuru-ai ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak rimuru-ai kendi `rimuru-ai` temasını kullanır.
...
rimuru-ai birden fazla yerleşik temayla gelir.
...
- rimuru-ai'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
rimuru-ai, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/rimuru-ai/themes/*.json` veya `$XDG_CONFIG_HOME/rimuru-ai/themes/*.json`
