---
title: Kurallar
description: rimuru-ai için özel talimatları ayarlayın.
---

Bir `AGENTS.md` dosyası oluşturarak rimuru-ai için özel talimatlar verebilirsiniz. Bu, Cursor rules yaklaşımına benzer. Özel projeniz için davranışını özelleştirmek amacıyla LLM'in içeriğine dahil edilecek talimatları içerir.

---

## Başlat

Yeni bir `AGENTS.md` dosyası oluşturmak için `/init` komutunu rimuru-ai'da çalıştırabilirsiniz.

:::tip
Projenizin `AGENTS.md` dosyasını Git'e göndermelisiniz.
:::

Bu, projenin neyle ilgili olduğunu anlamak için projenizi ve tüm içeriğini tarayacak ve onunla bir `AGENTS.md` dosyası oluşturacaktır. Bu, rimuru-ai'un projede daha iyi gezinmesine yardımcı olur.
...
rimuru-ai ayrıca `AGENTS.md` dosyasının birden fazla konumdan okunmasını da destekler. Ve bu farklı amaçlara hizmet ediyor.
...
Ayrıca `~/.config/rimuru-ai/AGENTS.md` dosyasında genel kurallara sahip olabilirsiniz. Bu, tüm rimuru-ai oturumlarına uygulanır.
...
rimuru-ai, Claude Code'dan geçiş yapan kullanıcılar için yedek olarak Claude Code'un dosya kurallarını destekler:

- **Proje kuralları**: Proje dizininizdeki `CLAUDE.md` (`AGENTS.md` yoksa kullanılır)
- **Global kurallar**: `~/.claude/CLAUDE.md` (`~/.config/rimuru-ai/AGENTS.md` yoksa kullanılır)
...
2. **Global dosya** `~/.config/rimuru-ai/AGENTS.md` adresinde
3. `~/.claude/CLAUDE.md` adresindeki **Claude Kod dosyası** (devre dışı bırakılmadığı sürece)

Her kategoride ilk eşleşen dosya kazanır. Örneğin, hem `AGENTS.md` hem de `CLAUDE.md` varsa yalnızca `AGENTS.md` kullanılır. Benzer şekilde, `~/.config/rimuru-ai/AGENTS.md`, `~/.claude/CLAUDE.md`'ye göre önceliklidir.

---

## Özel talimatlar

Özel talimat dosyalarını `opencode.json` veya global `~/.config/rimuru-ai/opencode.json`'de belirtebilirsiniz. Bu, sizin ve ekibinizin, mevcut kuralları AGENTS.md'ye kopyalamak yerine yeniden kullanmasına olanak tanır.
...
`AGENTS.md`'nizde açık talimatlar sağlayarak rimuru-ai'u harici dosyaları okumayı öğretebilirsiniz. İşte pratik bir örnek:
...
- rimuru-ai'un dosyaları yalnızca belirli bir görev için gerektiğinde yüklediğinden emin olun
