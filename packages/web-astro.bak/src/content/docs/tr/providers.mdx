---
title: Sağlayıcılar
description: rimuru-ai'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

rimuru-ai, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı rimuru-ai yapılandırmanızda yapılandırın.
...
`~/.local/share/rimuru-ai/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları rimuru-ai'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Rimuru AI Zen, rimuru-ai ekibi tarafından sağlanan modellerin bir listesidir.
rimuru-ai ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Rimuru AI Zen ile başlamanızı öneririz.
:::
...
rimuru-ai'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Rimuru AI Go

Rimuru AI Go, rimuru-ai ile iyi çalıştığı test edilmiş ve doğrulanmış, rimuru-ai ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Rimuru AI Go`'yu seçin ve [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen) adresine gidin.
...
Amazon Bedrock'u rimuru-ai ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   rimuru-ai'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
...
:::info
Claude Pro/Max aboneliğinizi rimuru-ai'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
rimuru-ai'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   rimuru-ai'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
rimuru-ai, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. rimuru-ai'u kilitlemek için
...
Daha fazla belge [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth) ana sayfasında.
...
GitHub Copilot aboneliğinizi rimuru-ai'la kullanmak için:
...
Google Vertex AI'yi rimuru-ai ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları rimuru-ai çalıştırılırken ayarlayın.
...
Helicone'un rimuru-ai aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her rimuru-ai görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
rimuru-ai'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla rimuru-ai'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla rimuru-ai'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini rimuru-ai için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/rimuru-ai)'a bakın.
:::
...
Ollama Cloud'u rimuru-ai ile kullanmak için:
...
3. rimuru-ai'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: rimuru-ai'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Rimuru AI Zen, rimuru-ai ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Rimuru AI Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Rimuru AI Zen**'i arayın.

   ```txt
   /connect
   ```

3. rimuru-ai API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   rimuru-ai yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca rimuru-ai yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri rimuru-ai yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı rimuru-ai ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
`limit` alanları rimuru-ai'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `rimuru-ai auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için rimuru-ai yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, rimuru-ai yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
