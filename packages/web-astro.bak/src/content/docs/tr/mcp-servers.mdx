...
_Model Bağlam Protokolü_ veya MCP kullanarak rimuru-ai'a harici araçlar ekleyebilirsiniz. rimuru-ai hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [rimuru-ai Config](https://rimurucode.vercel.app/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
rimuru-ai, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde rimuru-ai şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa rimuru-ai, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `rimuru-ai mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından rimuru-ai, belirteçleri `~/.local/share/rimuru-ai/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra rimuru-ai'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi rimuru-ai yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve rimuru-ai'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
