...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. rimuru-ai yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
rimuru-ai içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca rimuru-ai provider kullanılırken veya `OPENCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

rimuru-ai başlatırken etkinleştirmek için:
