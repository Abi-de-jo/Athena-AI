---
title: 插件
description: 编写自己的插件来扩展 Rimuru AI。
---

插件允许你通过挂钩各种事件和自定义行为来扩展 Rimuru AI。你可以创建插件来添加新功能、集成外部服务，或修改 Rimuru AI 的默认行为。
...
**npm 插件**在启动时使用 Bun 自动安装。包及其依赖项会缓存在 `~/.cache/rimuru-ai/node_modules/` 中。
...
1. 全局配置 (`~/.config/rimuru-ai/opencode.json`)
2. 项目配置 (`opencode.json`)
3. 全局插件目录 (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI 会在启动时运行 `bun install` 来安装这些依赖项。之后你的插件和工具就可以导入它们了。
...
- `directory`：当前工作目录。
- `worktree`：git 工作树路径。
- `client`：用于与 AI 交互的 Rimuru AI SDK 客户端。
...
:::note
如果你使用 Rimuru AI 桌面应用，它可以在响应就绪或会话出错时自动发送系统通知。
:::
...
`tool` 辅助函数用于创建 Rimuru AI 可调用的自定义工具。它接受一个 Zod schema 函数，并返回一个工具定义，包含：

- `description`：工具的功能描述
- `args`：工具参数的 Zod schema
- `execute`：工具被调用时执行的函数

你的自定义工具将与内置工具一起在 Rimuru AI 中可用。
...
日志级别：`debug`、`info`、`warn`、`error`。详情请参阅 [SDK 文档](https://rimurucode.vercel.app/docs/sdk)。
