---
title: 自定义工具
description: 创建 LLM 可在 rimuru-ai 中调用的工具。
---

自定义工具是你创建的函数，LLM 可以在对话过程中调用它们。它们与 rimuru-ai 的[内置工具](/docs/tools)（如 `read`、`write` 和 `bash`）协同工作。
...
- 全局定义：将工具文件放在 `~/.config/rimuru-ai/tools/` 中。
