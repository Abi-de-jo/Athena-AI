---
title: 配置
description: 使用 Rimuru AI JSON 配置。
---

您可以使用 JSON 配置文件来配置 Rimuru AI。

---

## 格式

Rimuru AI 支持 **JSON** 和 **JSONC**（带注释的 JSON）格式。
...
2. **全局配置**（`~/.config/rimuru-ai/opencode.json`）- 用户偏好
...
:::note
`.opencode` 和 `~/.config/rimuru-ai` 目录的子目录使用**复数名称**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。为了向后兼容，也支持单数名称（例如 `agent/`）。
:::
...
将全局 Rimuru AI 配置放在 `~/.config/rimuru-ai/opencode.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
当 Rimuru AI 启动时，它会在当前目录中查找配置文件，或向上遍历到最近的 Git 目录。
...
配置文件具有在 [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json) 中定义的 Schema。
...
您可以通过 `server` 选项为 `rimuru-ai serve` 和 `rimuru-ai web` 命令配置服务器设置。
...
- `mdns` - 启用 mDNS 服务发现。这允许网络上的其他设备发现您的 Rimuru AI 服务器。
...
您可以通过 `provider`、`model` 和 `small_model` 选项在 Rimuru AI 配置中设置要使用的提供商和模型。
...
`small_model` 选项为标题生成等轻量级任务配置单独的模型。默认情况下，如果您的提供商有更便宜的模型可用，Rimuru AI 会尝试使用该模型，否则会回退到您的主模型。
...
您可以通过 Rimuru AI 配置中的 `theme` 选项设置要使用的主题。
...
您还可以使用 `~/.config/rimuru-ai/agents/` 或 `.opencode/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。
...
默认代理必须是主代理（不能是子代理）。可以是内置代理（如 `"build"` 或 `"plan"`），也可以是您定义的[自定义代理](/docs/agents)。如果指定的代理不存在或是子代理，Rimuru AI 将回退到 `"build"` 并发出警告。

此设置适用于所有界面：TUI、CLI（`rimuru-ai run`）、桌面应用和 GitHub Action。
...
您还可以使用 `~/.config/rimuru-ai/commands/` 或 `.opencode/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
Rimuru AI 启动时会自动下载新版本。您可以使用 `autoupdate` 选项禁用此功能。
...
默认情况下，Rimuru AI **允许所有操作**，无需明确批准。您可以使用 `permission` 选项更改此行为。
...
[插件](/docs/plugins)通过自定义工具、钩子和集成来扩展 Rimuru AI。

将插件文件放置在 `.opencode/plugins/` 或 `~/.config/rimuru-ai/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。
...
当您希望限制 Rimuru AI 仅使用特定提供商，而不是逐一禁用其他提供商时，此选项非常有用。
