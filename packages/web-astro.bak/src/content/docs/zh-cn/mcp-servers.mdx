...
你可以通过 _Model Context Protocol_（MCP）为 Rimuru AI 添加外部工具。Rimuru AI 同时支持本地和远程服务器。
...
你可以在 [Rimuru AI 配置](https://rimurucode.vercel.app/docs/config/)的 `mcp` 字段下定义 MCP 服务器。为每个 MCP 指定一个唯一的名称，在提示词中可以通过该名称来引用对应的 MCP。
...
Rimuru AI 会自动处理远程 MCP 服务器的 OAuth 身份验证。当服务器需要身份验证时，Rimuru AI 将：
...
如果服务器需要身份验证，Rimuru AI 会在你首次使用时提示你进行认证。你也可以使用 `rimuru-ai mcp auth <server-name>` [手动触发认证流程](#authenticating)。
...
`mcp auth` 命令会打开浏览器进行授权。授权完成后，Rimuru AI 会将 Token 安全地存储在 `~/.local/share/rimuru-ai/mcp-auth.json` 中。
...
你的 MCP 在 Rimuru AI 中作为工具使用，与内置工具并列。因此，你可以像管理其他工具一样，通过 Rimuru AI 配置来管理它们。
...
这会打开浏览器窗口完成 OAuth 流程，将 Rimuru AI 连接到你的 Sentry 账户。
