...
Rimuru AI 使用 `permission` 配置来决定某个操作是否应自动运行、提示你审批，还是被阻止。
...
使用 `external_directory` 允许工具调用访问 Rimuru AI 启动时工作目录之外的路径。这适用于任何接受路径作为输入的工具（例如 `read`、`edit`、`glob`、`grep` 以及许多 `bash` 命令）。
...
Rimuru AI 的权限以工具名称为键，外加几个安全防护项：
...
如果你未指定任何配置，Rimuru AI 将使用宽松的默认值：
...
当 Rimuru AI 提示审批时，界面提供三种选择：

- `once` — 仅批准本次请求
- `always` — 批准与建议模式匹配的后续请求（在当前 Rimuru AI 会话的剩余时间内有效）
- `reject` — 拒绝请求
