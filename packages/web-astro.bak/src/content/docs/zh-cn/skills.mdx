...
代理技能让 Rimuru AI 能够从你的仓库或主目录中发现可复用的指令。
...
- 全局配置：`~/.config/rimuru-ai/skills/<name>/SKILL.md`
...
对于项目本地路径，Rimuru AI 会从当前工作目录向上遍历，直到到达 git 工作树根目录。
在此过程中，它会加载 `.opencode/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全局定义也会从 `~/.config/rimuru-ai/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中加载。
...
Rimuru AI 会在 `skill` 工具描述中列出可用技能。
每个条目包含技能名称和描述：
