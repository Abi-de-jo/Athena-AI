---
title: CLI
description: أوامر وخيارات CLI في Rimuru AI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Rimuru AI افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
rimuru-ai
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Rimuru AI برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Rimuru AI.
...
يتضمن CLI الخاص بـ Rimuru AI أيضا الأوامر التالية.

---

### agent

إدارة وكلاء Rimuru AI.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Rimuru AI قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
rimuru-ai attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Rimuru AI تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `rimuru-ai`) |
...
يعتمد Rimuru AI على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `rimuru-ai auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

عند تشغيل Rimuru AI يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Rimuru AI.
...
تشغيل rimuru-ai في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `rimuru-ai serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم rimuru-ai قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `rimuru-ai`) |
...
بدء خادم Rimuru AI بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
rimuru-ai serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف rimuru-ai دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `rimuru-ai`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Rimuru AI لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Rimuru AI.

```bash
rimuru-ai import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Rimuru AI.
...
بدء خادم Rimuru AI بدون واجهة مع واجهة ويب.

```bash
rimuru-ai web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Rimuru AI عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `rimuru-ai`).
...
إلغاء تثبيت Rimuru AI وإزالة جميع الملفات ذات الصلة.
...
تحديث rimuru-ai إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر rimuru-ai الخيارات العامة التالية.
...
يمكن تهيئة Rimuru AI باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `rimuru-ai`) |
