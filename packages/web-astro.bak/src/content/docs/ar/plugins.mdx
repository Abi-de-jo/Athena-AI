---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Rimuru AI.
---

تتيح لك الإضافات توسيع Rimuru AI عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Rimuru AI.
...
- `~/.config/rimuru-ai/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/rimuru-ai/node_modules/`.
...
1. إعدادات عامة (`~/.config/rimuru-ai/opencode.json`)
2. إعدادات المشروع (`opencode.json`)
3. دليل الإضافات العام (`~/.config/rimuru-ai/plugins/`)
...
يشغّل Rimuru AI الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ rimuru-ai للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع rimuru-ai.
...
:::note
إذا كنت تستخدم تطبيق Rimuru AI لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع rimuru-ai من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى rimuru-ai:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ rimuru-ai استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ rimuru-ai إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://rimurucode.vercel.app/docs/sdk) للتفاصيل.
