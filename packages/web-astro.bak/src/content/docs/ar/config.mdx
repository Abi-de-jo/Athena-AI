---
title: الإعدادات
description: استخدام ملف إعدادات Rimuru AI بصيغة JSON.
---

يمكنك ضبط Rimuru AI باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم Rimuru AI تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
2. **الإعدادات العامة** (`~/.config/rimuru-ai/opencode.json`) - تفضيلات المستخدم
...
:::note
يستخدم الدليلان `.opencode` و`~/.config/rimuru-ai` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
ضع إعدادات Rimuru AI العامة في `~/.config/rimuru-ai/opencode.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
عند تشغيل Rimuru AI، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `rimuru-ai serve` و`rimuru-ai web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم Rimuru AI.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات Rimuru AI عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول Rimuru AI استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات Rimuru AI عبر الخيار `theme`.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/rimuru-ai/agents/` أو `.opencode/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود Rimuru AI إلى `"build"` مع تحذير.

ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`rimuru-ai run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/rimuru-ai/commands/` أو `.opencode/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
سيقوم Rimuru AI بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
افتراضيًا يسمح rimuru-ai **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
[الإضافات](/docs/plugins) توسّع Rimuru AI بأدوات وخُطافات وتكاملات مخصصة.

ضع ملفات الإضافات في `.opencode/plugins/` أو `~/.config/rimuru-ai/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
...
هذا مفيد عندما تريد تقييد Rimuru AI لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
