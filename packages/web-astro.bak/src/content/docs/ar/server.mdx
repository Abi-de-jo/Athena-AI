---
title: الخادم
description: تفاعل مع خادم rimuru-ai عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `rimuru-ai serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل rimuru-ai استخدامها.
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `rimuru-ai`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `rimuru-ai serve` و `rimuru-ai web`.
...
عند تشغيل `rimuru-ai` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم rimuru-ai للتفاعل مع rimuru-ai برمجيا.
:::

تتيح هذه البنية لـ rimuru-ai دعم عدة عملاء وتمكّنك من التفاعل مع rimuru-ai برمجيا.

يمكنك تشغيل `rimuru-ai serve` لبدء خادم مستقل. إذا كانت واجهة rimuru-ai في terminal (TUI)
قيد التشغيل، فسيبدأ `rimuru-ai serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Rimuru AI لـ [IDE](/docs/ide).
...
يعرض خادم rimuru-ai واجهات API التالية.
