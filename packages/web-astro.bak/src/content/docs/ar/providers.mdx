---
title: المزوّدون
description: استخدام أي موفّر LLM في Rimuru AI.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Rimuru AI ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Rimuru AI.
...
في `~/.local/share/rimuru-ai/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Rimuru AI.
...
Rimuru AI Zen هي قائمة نماذج يوفّرها فريق Rimuru AI وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Rimuru AI. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Rimuru AI Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر rimuru-ai، ثم انتقل إلى [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/auth).
...
يعمل مثل أي مزوّد آخر في Rimuru AI واستخدامه اختياري بالكامل.

---

## Rimuru AI Go

Rimuru AI Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Rimuru AI، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Rimuru AI.

1. شغّل الأمر `/connect` في TUI، واختر `Rimuru AI Go`، ثم انتقل إلى [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
لاستخدام Amazon Bedrock مع Rimuru AI:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل rimuru-ai:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Rimuru AI غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين rimuru-ai لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل rimuru-ai بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات rimuru-ai.
...
تخزين الرمز في مساحة تخزين مصادقة rimuru-ai.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Rimuru AI نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Rimuru AI
...
مزيد من التوثيق على صفحة [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع rimuru-ai:
...
لاستخدام Google Vertex AI مع Rimuru AI:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر rimuru-ai، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Rimuru AI تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Rimuru AI مدرجة كجلسة مستقلة.
...
يمكنك ضبط rimuru-ai لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط rimuru-ai لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط rimuru-ai لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Rimuru AI. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/rimuru-ai) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Rimuru AI:
...
3. انسخ مفتاح API لاستخدامه في Rimuru AI.
...
6. **مهم**: قبل استخدام نماذج السحابة في Rimuru AI، يجب سحب معلومات النموذج محليا:
...
Rimuru AI Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Rimuru AI. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Rimuru AI Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Rimuru AI.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات rimuru-ai.
...
5. يمكنك أيضا تخصيصها عبر إعدادات rimuru-ai. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات rimuru-ai. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع rimuru-ai. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
تسمح حقول `limit` لـ Rimuru AI بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `rimuru-ai auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات rimuru-ai وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات rimuru-ai.
