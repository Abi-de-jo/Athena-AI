...
يمكنك إضافة أدوات خارجية إلى Rimuru AI باستخدام _Model Context Protocol_، أو MCP. يدعم Rimuru AI خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Rimuru AI](https://rimurucode.vercel.app/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى Rimuru AI تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Rimuru AI بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Rimuru AI المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `rimuru-ai mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Rimuru AI الرموز بشكل آمن في `~/.local/share/rimuru-ai/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Rimuru AI إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Rimuru AI مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Rimuru AI بحساب Sentry الخاص بك.
