...
يستخدم Rimuru AI إعداد `permission` لتحديد ما إذا كان إجراءٌ ما سيُنفّذ تلقائيًا، أو سيطلب موافقتك، أو سيتم حظره.
...
استخدم `external_directory` للسماح باستدعاءات الأدوات التي تلمس مسارات خارج دليل العمل الذي بدأ منه Rimuru AI. ينطبق ذلك على أي أداة تأخذ مسارًا كمدخل (مثل `read` و`edit` و`glob` و`grep` والعديد من أوامر `bash`).
...
تُعرَّف أذونات Rimuru AI بأسماء الأدوات، بالإضافة إلى بعض حواجز الأمان:
...
إذا لم تحدد شيئًا، يبدأ Rimuru AI بقيم افتراضية متساهلة:
...
عندما يطلب Rimuru AI الموافقة، تعرض الواجهة ثلاث نتائج ممكنة:

- `once` — الموافقة على هذا الطلب فقط
- `always` — الموافقة على الطلبات المستقبلية المطابقة للأنماط المقترحة (لباقي جلسة Rimuru AI الحالية)
- `reject` — رفض الطلب
