---
title: TUI
description: Using the Rimuru AI terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Rimuru AI provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Rimuru AI starts the TUI for the current directory.
...
When using the Rimuru AI TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Rimuru AI. Allows you to select from available providers and add their API keys.
...
Exit Rimuru AI. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long Rimuru AI waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `rimuru-ai.default`.
