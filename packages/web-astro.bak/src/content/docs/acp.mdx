---
title: ACP Support
description: Use Rimuru AI in any ACP-compatible editor.
---

Rimuru AI supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use Rimuru AI via ACP, configure your editor to run the `rimuru-ai acp` command.

The command starts Rimuru AI as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'Rimuru AI' agent in the AI Chat agent selector.
...
To use Rimuru AI as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use Rimuru AI as the ACP agent for chat.
...
Rimuru AI works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your Rimuru AI config
