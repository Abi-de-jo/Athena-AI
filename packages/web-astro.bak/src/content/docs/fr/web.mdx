---
title: Web
description: En utilisant Rimuru AI dans votre navigateur.
---

Rimuru AI peut s'exécuter comme une application Web dans votre navigateur, offrant la même expérience puissante de codage d'IA sans avoir besoin d'un terminal.

![Rimuru AI Web - Nouvelle session](../../../assets/web/web-homepage-new-session.png)
...
Cela démarre un serveur local sur `127.0.0.1` avec un port disponible aléatoire et ouvre automatiquement Rimuru AI dans votre navigateur par défaut.
...
Pour une expérience optimale, exécutez `rimuru-ai web` à partir de [WSL](/docs/windows-wsl) plutôt que de PowerShell. Cela garantit un accès correct au système de fichiers et une intégration du terminal.
:::
...
Par défaut, Rimuru AI sélectionne un port disponible. Vous pouvez spécifier un port :
...
Par défaut, le serveur se lie à `127.0.0.1` (localhost uniquement). Pour rendre Rimuru AI accessible sur votre réseau :

```bash
rimuru-ai web --hostname 0.0.0.0
```

Lorsque vous utilisez `0.0.0.0`, Rimuru AI affichera les adresses locales et réseau :
...
Le nom d'utilisateur par défaut est `rimuru-ai` mais peut être modifié avec `OPENCODE_SERVER_USERNAME`.

---

## Utilisation de l'interface Web

Une fois démarrée, l'interface web permet d'accéder à vos sessions Rimuru AI.
...
![Rimuru AI Web - Session active](../../../assets/web/web-homepage-active-session.png)

### Statut du serveur

Cliquez sur "Voir les serveurs" pour afficher les serveurs connectés et leur état.

![Rimuru AI Web - Voir Serveurs](../../../assets/web/web-homepage-see-servers.png)
