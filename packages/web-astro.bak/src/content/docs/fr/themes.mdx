...
Avec Rimuru AI, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Rimuru AI utilise notre propre thème `rimuru-ai`.
...
Rimuru AI est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Rimuru AI corresponde à l'apparence de leur terminal
...
Rimuru AI prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/rimuru-ai/themes/*.json` ou `$XDG_CONFIG_HOME/rimuru-ai/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/rimuru-ai/themes
vim ~/.config/rimuru-ai/themes/my-theme.json
```
...
{
  "$schema": "https://rimurucode.vercel.app/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
