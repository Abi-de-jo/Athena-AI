---
title: Plugins
description: Écrivez vos propres plugins pour étendre Rimuru AI.
---

Les plugins vous permettent d'étendre Rimuru AI en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Rimuru AI.
...
- `~/.config/rimuru-ai/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/rimuru-ai/node_modules/`.
...
1. Configuration globale (`~/.config/rimuru-ai/opencode.json`)
2. Configuration du projet (`opencode.json`)
3. Répertoire global des plugins (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK rimuru-ai pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre rimuru-ai.
...
:::note
Si vous utilisez l'application de bureau Rimuru AI, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez rimuru-ai de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à rimuru-ai :
...
L'assistant `tool` crée un outil personnalisé que rimuru-ai peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour rimuru-ai aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://rimurucode.vercel.app/docs/sdk) pour plus de détails.
