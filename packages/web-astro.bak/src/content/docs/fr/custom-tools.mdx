...
description: Créez des outils que le LLM peut appeler dans rimuru-ai.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de rimuru-ai comme `read`, `write` et `bash`.
...
- Ou globalement, en les plaçant dans `~/.config/rimuru-ai/tools/`.
