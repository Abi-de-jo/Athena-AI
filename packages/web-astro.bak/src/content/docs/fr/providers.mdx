---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Rimuru AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Rimuru AI utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Rimuru AI.
...
en `~/.local/share/rimuru-ai/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Rimuru AI
configuration.
...
Rimuru AI Zen est une liste de modèles fournis par l'équipe Rimuru AI qui ont été
testé et vérifié pour fonctionner correctement avec Rimuru AI. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Rimuru AI Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez rimuru-ai et dirigez-vous vers [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Rimuru AI et son utilisation est totalement facultative.

---

## Rimuru AI Go

Rimuru AI Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Rimuru AI qui ont été
testé et vérifié pour fonctionner correctement avec Rimuru AI.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Rimuru AI Go` et rendez-vous sur [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Pour utiliser Amazon Bedrock avec Rimuru AI :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de rimuru-ai :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Rimuru AI n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer rimuru-ai pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que rimuru-ai fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration rimuru-ai.
...
pour stocker le jeton dans le stockage d'authentification rimuru-ai.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Rimuru AI utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Rimuru AI
...
Plus de documentation sur la page d'accueil [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth).
...
Pour utiliser votre abonnement GitHub Copilot avec rimuru-ai :
...
Pour utiliser Google Vertex AI avec Rimuru AI :
...
Définissez-les lors de l'exécution de rimuru-ai.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via rimuru-ai, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Rimuru AI en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Rimuru AI répertoriée comme une session distincte.
...
Vous pouvez configurer rimuru-ai pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer rimuru-ai pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer rimuru-ai pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Rimuru AI. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/rimuru-ai) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Rimuru AI :
...
3. Copiez la clé API à utiliser dans Rimuru AI.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Rimuru AI, vous devez extraire les informations du modèle localement :
...
Rimuru AI Zen est une liste de modèles testés et vérifiés fournie par l'équipe Rimuru AI. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Rimuru AI Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Rimuru AI API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration rimuru-ai.
...
5. Vous pouvez également les personnaliser via votre configuration rimuru-ai. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration rimuru-ai. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec rimuru-ai. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
Les champs `limit` permettent à Rimuru AI de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `rimuru-ai auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration rimuru-ai et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration rimuru-ai.
