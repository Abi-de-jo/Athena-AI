---
title: Configuration
description: Utilisation de la configuration JSON Rimuru AI.
---

Vous pouvez configurer Rimuru AI à l'aide d'un fichier de configuration JSON.

---

## Format

Rimuru AI prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
2. **Configuration globale** (`~/.config/rimuru-ai/opencode.json`) - préférences utilisateur
...
:::note
Les répertoires `.opencode` et `~/.config/rimuru-ai` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Placez votre configuration globale Rimuru AI dans `~/.config/rimuru-ai/opencode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/rimuru-ai/tui.json`.
...
Lorsque Rimuru AI démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Le fichier de configuration a un schéma défini dans [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).

La configuration TUI utilise [**`rimurucode.vercel.app/tui.json`**](https://rimurucode.vercel.app/tui.json).
...
Vous pouvez configurer les paramètres du serveur pour les commandes `rimuru-ai serve` et `rimuru-ai web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Rimuru AI.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Rimuru AI via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Rimuru AI essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/rimuru-ai/agents/` ou `.opencode/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Rimuru AI reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`rimuru-ai run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/rimuru-ai/commands/` ou `.opencode/commands/`. [En savoir plus ici](/docs/commands).
...
Rimuru AI téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, rimuru-ai **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Rimuru AI avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.opencode/plugins/` ou `~/.config/rimuru-ai/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre Rimuru AI à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
