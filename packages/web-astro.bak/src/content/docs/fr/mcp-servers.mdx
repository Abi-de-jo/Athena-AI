...
Vous pouvez ajouter des outils externes à Rimuru AI à l'aide du _Model Context Protocol_ ou MCP. Rimuru AI prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Rimuru AI Config](https://rimurucode.vercel.app/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Rimuru AI gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Rimuru AI :
...
Si le serveur nécessite une authentification, Rimuru AI vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `rimuru-ai mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Rimuru AI stockera les jetons en toute sécurité dans `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Rimuru AI, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Rimuru AI comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Rimuru AI à votre compte Sentry.
