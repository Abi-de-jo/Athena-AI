...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Rimuru AI.
...
Voici plusieurs modèles qui fonctionnent bien avec Rimuru AI, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Rimuru AI.
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Rimuru AI Zen](/docs/zen), vous utiliserez `rimuru-ai/gpt-5.1-codex` pour GPT 5.1 Codex.
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Rimuru AI est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Rimuru AI est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
Lorsque Rimuru AI démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Rimuru AI.
