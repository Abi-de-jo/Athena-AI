...
Pour déboguer les problèmes avec Rimuru AI, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%.local\share\rimuru-ai\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `rimuru-ai --log-level DEBUG`.

---

## Stockage

rimuru-ai stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/rimuru-ai/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%.local\share\rimuru-ai`
...
Rimuru AI Desktop exécute un serveur Rimuru AI local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Rimuru AI` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc`
...
Rimuru AI peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/rimuru-ai/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\rimuru-ai\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Rimuru AI puisse le reconstruire.

1. Quittez complètement Rimuru AI Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/rimuru-ai`
- **Linux** : supprimez `~/.cache/rimuru-ai` (ou exécutez `rm -rf ~/.cache/rimuru-ai`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\rimuru-ai`

3. Redémarrez le bureau Rimuru AI.

---

### Résoudre les problèmes de connexion au serveur

Rimuru AI Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Sur Windows, Rimuru AI Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Rimuru AI.

---

### Les notifications ne s'affichent pas

Rimuru AI Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Rimuru AI dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Rimuru AI.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Rimuru AI Desktop) :

- `rimuru-ai.settings.dat` (serveur de bureau par défaut URL)
- `rimuru-ai.global.dat` et `rimuru-ai.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Rimuru AI :
...
3. Assurez-vous d'avoir la dernière version avec `rimuru-ai upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `rimuru-ai models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%.local\share\rimuru-ai`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. rimuru-ai installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\rimuru-ai`

2. Redémarrez rimuru-ai pour réinstaller les derniers packages du fournisseur

Cela forcera rimuru-ai à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
rimuru-ai détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
