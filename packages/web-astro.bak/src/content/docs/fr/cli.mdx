---
title: CLI
description: Options et commandes de la CLI Rimuru AI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI Rimuru AI démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
rimuru-ai
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec Rimuru AI par programme.
...
Démarrez l’interface utilisateur du terminal Rimuru AI.
...
La CLI Rimuru AI dispose également des commandes suivantes.

---

### agent

Gérer les agents pour Rimuru AI.
...
Attachez un terminal à un serveur backend Rimuru AI déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
rimuru-ai attach [url]
```

Cela permet d'utiliser le TUI avec un backend Rimuru AI distant. Par exemple :
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OPENCODE_SERVER_USERNAME` ou `rimuru-ai`) |
...
Rimuru AI est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `rimuru-ai auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

Lorsque Rimuru AI démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans Rimuru AI.
...
Exécutez rimuru-ai en mode non interactif en transmettant directement un prompt.
...
Vous pouvez également vous connecter à une instance `rimuru-ai serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Connectez-vous à un serveur rimuru-ai en cours d'exécution (par exemple, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Mot de passe d'authentification de base (par défaut `OPENCODE_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OPENCODE_SERVER_USERNAME` ou `rimuru-ai`) |
...
Démarrez un serveur Rimuru AI sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
rimuru-ai serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'Rimuru AI sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `rimuru-ai`).
...
Liste toutes les sessions Rimuru AI.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions Rimuru AI.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage Rimuru AI.

```bash
rimuru-ai import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage Rimuru AI.
...
Démarrez un serveur Rimuru AI sans interface graphique avec une interface Web.

```bash
rimuru-ai web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à Rimuru AI via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `rimuru-ai`).
...
Désinstallez Rimuru AI et supprimez tous les fichiers associés.
...
Met à jour rimuru-ai vers la dernière version ou une version spécifique.
...
La CLI rimuru-ai prend les flags globaux suivants.
...
Rimuru AI peut être configuré à l'aide de variables d'environnement.
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `rimuru-ai`) |
