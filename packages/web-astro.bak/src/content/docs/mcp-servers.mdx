...
You can add external tools to Rimuru AI using the _Model Context Protocol_, or MCP. Rimuru AI supports both local and remote servers.
...
You can define MCP servers in your [Rimuru AI Config](https://rimurucode.vercel.app/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Rimuru AI automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Rimuru AI will:
...
If the server requires authentication, Rimuru AI will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `rimuru-ai mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Rimuru AI will store the tokens securely in `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Your MCPs are available as tools in Rimuru AI, alongside built-in tools. So you can manage them through the Rimuru AI config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Rimuru AI to your Sentry account.
