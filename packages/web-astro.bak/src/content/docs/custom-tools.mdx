---
title: Custom Tools
description: Create tools the LLM can call in rimuru-ai.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside rimuru-ai's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Or globally, by placing them in `~/.config/rimuru-ai/tools/`.
