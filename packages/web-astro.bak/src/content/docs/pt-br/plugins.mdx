---
title: Plugins
description: Escreva seus próprios plugins para estender o rimuru-ai.
---

Plugins permitem que você estenda o rimuru-ai conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do rimuru-ai.
...
- `~/.config/rimuru-ai/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/rimuru-ai/node_modules/`.
...
1. Configuração global (`~/.config/rimuru-ai/opencode.json`)
2. Configuração do projeto (`opencode.json`)
3. Diretório de plugins global (`~/.config/rimuru-ai/plugins/`)
...
O rimuru-ai executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do rimuru-ai para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o rimuru-ai.
...
:::note
Se você estiver usando o aplicativo desktop rimuru-ai, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o rimuru-ai de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao rimuru-ai:
...
O helper `tool` cria uma ferramenta personalizada que o rimuru-ai pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o rimuru-ai junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://rimurucode.vercel.app/docs/sdk) para detalhes.
