---
title: CLI
description: Opções e comandos da CLI do rimuru-ai.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do rimuru-ai, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
rimuru-ai
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o rimuru-ai programaticamente.
...
Inicie a interface de usuário do terminal do rimuru-ai.
...
A CLI do rimuru-ai também possui os seguintes comandos.

---

### agent

Gerencie agentes para o rimuru-ai.
...
Anexe um terminal a um servidor backend do rimuru-ai já em execução, iniciado via comandos `serve` ou `web`.

```bash
rimuru-ai attach [url]
```

Isso permite usar o TUI com um backend rimuru-ai remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `rimuru-ai`) |
...
O rimuru-ai é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `rimuru-ai auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

Quando o rimuru-ai é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no rimuru-ai.
...
Execute o rimuru-ai em modo não interativo passando um prompt diretamente.
...
```bash "rimuru-ai run"
rimuru-ai run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `rimuru-ai serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
rimuru-ai run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor rimuru-ai em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `rimuru-ai`) |
...
Inicie um servidor rimuru-ai sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
rimuru-ai serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do rimuru-ai sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `rimuru-ai`).
...
Liste todas as sessões do rimuru-ai.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do rimuru-ai.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do rimuru-ai.

```bash
rimuru-ai import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do rimuru-ai.
...
Inicie um servidor rimuru-ai sem cabeça com uma interface web.

```bash
rimuru-ai web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o rimuru-ai através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `rimuru-ai`).
...
Desinstale o rimuru-ai e remova todos os arquivos relacionados.
...
Atualiza o rimuru-ai para a versão mais recente ou uma versão específica.
...
A CLI do rimuru-ai aceita as seguintes flags globais.
...
O rimuru-ai pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `rimuru-ai`) |
