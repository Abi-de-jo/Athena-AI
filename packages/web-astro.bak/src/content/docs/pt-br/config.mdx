---
title: Configuração
description: Usando a configuração JSON do rimuru-ai.
---

Você pode configurar o rimuru-ai usando um arquivo de configuração JSON.

---

## Formato

O rimuru-ai suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
2. **Configuração global** (`~/.config/rimuru-ai/opencode.json`) - preferências do usuário
...
:::note
Os diretórios `.opencode` e `~/.config/rimuru-ai` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do rimuru-ai em `~/.config/rimuru-ai/opencode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/rimuru-ai/tui.json`.
...
Quando o rimuru-ai é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).

A configuração do TUI usa [**`rimurucode.vercel.app/tui.json`**](https://rimurucode.vercel.app/tui.json).
...
Você pode configurar as configurações do servidor para os comandos `rimuru-ai serve` e `rimuru-ai web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor rimuru-ai.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do rimuru-ai através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o rimuru-ai tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/rimuru-ai/agents/` ou `.opencode/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o rimuru-ai recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`rimuru-ai run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/rimuru-ai/commands/` ou `.opencode/commands/`. [Saiba mais aqui](/docs/commands).
...
O rimuru-ai fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o rimuru-ai **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o rimuru-ai com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.opencode/plugins/` ou `~/.config/rimuru-ai/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o rimuru-ai para usar apenas provedores específicos, em vez de desabilitá-los um a um.
