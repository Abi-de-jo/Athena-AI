...
O rimuru-ai tem uma lista de atalhos de teclado que você pode personalizar através de `tui.json`.
...
O rimuru-ai usa uma tecla `leader` para a maioria dos atalhos de teclado. Isso evita conflitos no seu terminal.
...
O prompt de entrada do aplicativo desktop rimuru-ai suporta atalhos comuns de estilo Readline/Emacs para edição de texto. Estes são embutidos e atualmente não são configuráveis via `opencode.json`.
