---
title: Servidor
description: Interaja com o servidor rimuru-ai via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `rimuru-ai serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente rimuru-ai pode usar.
...
```bash
rimuru-ai serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
rimuru-ai serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `rimuru-ai`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `rimuru-ai serve` quanto ao `rimuru-ai web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password rimuru-ai serve
```
...
Quando você executa `rimuru-ai`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor rimuru-ai para interagir com o rimuru-ai programaticamente.
:::

Essa arquitetura permite que o rimuru-ai suporte múltiplos clientes e permite que você interaja com o rimuru-ai programaticamente.

Você pode executar `rimuru-ai serve` para iniciar um servidor autônomo. Se você tiver o TUI do rimuru-ai em execução, `rimuru-ai serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do rimuru-ai [IDE](/docs/ide).
...
O servidor rimuru-ai expõe as seguintes APIs.
