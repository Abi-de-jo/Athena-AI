---
title: Provedores
description: Usando qualquer provedor LLM no rimuru-ai.
---

import config from "../../../../config.mjs"
export const console = config.console

rimuru-ai usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do rimuru-ai.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/rimuru-ai/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do rimuru-ai.
...
Rimuru AI Zen é uma lista de modelos fornecidos pela equipe do rimuru-ai que foram testados e verificados para funcionar bem com o rimuru-ai. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Rimuru AI Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Rimuru AI Zen` e acesse [rimurucode.vercel.app/zen](https://rimurucode.vercel.app/zen).
...
Funciona como qualquer outro provedor no rimuru-ai e é completamente opcional.

---

## Rimuru AI Go

Rimuru AI Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do rimuru-ai que foram
testados e verificados para funcionar bem com o rimuru-ai.

1. Execute o comando `/connect` no TUI, selecione `Rimuru AI Go` e acesse [rimurucode.vercel.app/zen](https://rimurucode.vercel.app/zen).
...
Para usar o Amazon Bedrock com o rimuru-ai:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o rimuru-ai:
...
:::info
Usar sua assinatura Claude Pro/Max no rimuru-ai não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o rimuru-ai para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o rimuru-ai funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do rimuru-ai.
...
armazenar o token no armazenamento de autenticação do rimuru-ai.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
rimuru-ai usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o rimuru-ai
...
Mais documentação na página [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth).
...
Para usar sua assinatura do GitHub Copilot com o rimuru-ai:
...
Para usar o Google Vertex AI com o rimuru-ai:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o rimuru-ai.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do rimuru-ai, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do rimuru-ai como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do rimuru-ai listada como uma sessão separada.
...
Você pode configurar o rimuru-ai para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o rimuru-ai para usar modelos locais através do LM Studio.
...
Você pode configurar o rimuru-ai para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o rimuru-ai. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/rimuru-ai) para detalhes.
:::
...
Para usar o Ollama Cloud com o rimuru-ai:
...
3. Copie a chave da API para uso no rimuru-ai.
...
6. **Importante**: Antes de usar modelos em nuvem no rimuru-ai, você deve puxar as informações do modelo localmente:
...
Rimuru AI Zen é uma lista de modelos testados e verificados fornecidos pela equipe do rimuru-ai. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Rimuru AI Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do rimuru-ai.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do rimuru-ai.
...
5. Você também pode personalizá-los através da sua configuração do rimuru-ai. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o rimuru-ai:
...
Você também pode personalizar modelos através da sua configuração do rimuru-ai. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o rimuru-ai. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
Os campos `limit` permitem que o rimuru-ai entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `rimuru-ai auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do rimuru-ai e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do rimuru-ai.
