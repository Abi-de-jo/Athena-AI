...
Existem dois tipos de agentes no rimuru-ai; agentes primários e subagentes.
...
rimuru-ai vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Rimuru AI vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Rimuru AI vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Rimuru AI, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
- Global: `~/.config/rimuru-ai/agents/`
- Por projeto: `.opencode/agents/`

```markdown title="~/.config/rimuru-ai/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o rimuru-ai usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do rimuru-ai quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do rimuru-ai usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Rimuru AI Zen](/docs/zen), você usaria `rimuru-ai/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `rimuru-ai models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/gowdaman-dev/rimuru-ai).
:::
...
```markdown title="~/.config/rimuru-ai/agents/docs-writer.md"
---
...
```markdown title="~/.config/rimuru-ai/agents/security-auditor.md"
---
