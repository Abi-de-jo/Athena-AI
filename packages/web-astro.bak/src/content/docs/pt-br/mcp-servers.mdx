...
Você pode adicionar ferramentas externas ao rimuru-ai usando o _Model Context Protocol_, ou MCP. O rimuru-ai suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do rimuru-ai](https://rimurucode.vercel.app/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
O rimuru-ai lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o rimuru-ai irá:
...
Se o servidor requer autenticação, o rimuru-ai solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `rimuru-ai mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o rimuru-ai armazenará os tokens de forma segura em `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no rimuru-ai, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do rimuru-ai como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o rimuru-ai à sua conta do Sentry.
