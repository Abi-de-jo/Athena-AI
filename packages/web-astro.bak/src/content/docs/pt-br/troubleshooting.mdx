...
Para depurar problemas com o rimuru-ai, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%.local\share\rimuru-ai\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `rimuru-ai --log-level DEBUG`.

---

## Armazenamento

O rimuru-ai armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/rimuru-ai/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%.local\share\rimuru-ai`
...
O rimuru-ai Desktop executa um servidor rimuru-ai local (o sidecar `opencode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Rimuru AI` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/rimuru-ai/opencode.jsonc` (ou `~/.config/rimuru-ai/opencode.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc`
...
O rimuru-ai também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/rimuru-ai/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\rimuru-ai\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o rimuru-ai possa reconstruí-lo.

1. Saia completamente do rimuru-ai Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/rimuru-ai`
- **Linux**: exclua `~/.cache/rimuru-ai` (ou execute `rm -rf ~/.cache/rimuru-ai`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\rimuru-ai`

3. Reinicie o rimuru-ai Desktop.

---

### Corrigindo problemas de conexão com o servidor

O rimuru-ai Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
No Windows, o rimuru-ai Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do rimuru-ai.

---

### Notificações não aparecendo

O rimuru-ai Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o rimuru-ai nas configurações do seu sistema operacional, e
...
1. Saia do rimuru-ai Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo rimuru-ai Desktop):

- `rimuru-ai.settings.dat` (URL do servidor padrão do desktop)
- `rimuru-ai.global.dat` e `rimuru-ai.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o rimuru-ai:
...
Para descobrir quais modelos você tem acesso, execute `rimuru-ai models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%.local\share\rimuru-ai`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O rimuru-ai instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\rimuru-ai`

2. Reinicie o rimuru-ai para reinstalar os pacotes de provedores mais recentes

Isso forçará o rimuru-ai a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O rimuru-ai detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
