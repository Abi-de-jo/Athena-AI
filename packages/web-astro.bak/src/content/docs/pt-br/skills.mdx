...
As habilidades do agente permitem que o rimuru-ai descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O rimuru-ai pesquisa nesses locais:

- Configuração do projeto: `.opencode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/rimuru-ai/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o rimuru-ai sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
...
As definições globais também são carregadas de `~/.config/rimuru-ai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
O rimuru-ai lista as habilidades disponíveis na descrição da ferramenta `skill`.
