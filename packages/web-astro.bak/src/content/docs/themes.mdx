...
With Rimuru AI you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Rimuru AI uses our own `rimuru-ai` theme.
...
Rimuru AI comes with several built-in themes.
...
- Want Rimuru AI to match their terminal's appearance
...
Rimuru AI supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/rimuru-ai/themes/*.json` or `$XDG_CONFIG_HOME/rimuru-ai/themes/*.json`
