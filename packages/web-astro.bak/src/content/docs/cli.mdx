---
title: CLI
description: Rimuru AI CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Rimuru AI CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
rimuru-ai
```

But it also accepts commands as documented on this page. This allows you to interact with Rimuru AI programmatically.
...
Start the Rimuru AI terminal user interface.
...
The Rimuru AI CLI also has the following commands.

---

### agent

Manage agents for Rimuru AI.
...
Attach a terminal to an already running Rimuru AI backend server started via `serve` or `web` commands.

```bash
rimuru-ai attach [url]
```

This allows using the TUI with a remote Rimuru AI backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `rimuru-ai`) |
...
Rimuru AI is powered by the provider list at [Models.dev](https://models.dev), so you can use `rimuru-ai auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

When Rimuru AI starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Rimuru AI.
...
Run rimuru-ai in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `rimuru-ai serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>                       |       | Attach to a running rimuru-ai server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr>                     | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr>                     | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `rimuru-ai`) |
...
Start a headless Rimuru AI server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
rimuru-ai serve
```

This starts an HTTP server that provides API access to rimuru-ai functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `rimuru-ai`).
...
Delete an Rimuru AI session.

```bash
rimuru-ai session delete <sessionID>
```
...
Show token usage and cost statistics for your Rimuru AI sessions.
...
Import session data from a JSON file or Rimuru AI share URL.

```bash
rimuru-ai import <file>
```

You can import from a local file or an Rimuru AI share URL.
...
Start a headless Rimuru AI server with a web interface.

```bash
rimuru-ai web
```

This starts an HTTP server and opens a web browser to access Rimuru AI through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `rimuru-ai`).
...
```bash
rimuru-ai plugin <module>
```

Or use the alias.

```bash
rimuru-ai plug <module>
```
...
Fetch and checkout a GitHub PR branch, then run Rimuru AI.
...
Uninstall Rimuru AI and remove all related files.
...
Updates rimuru-ai to the latest version or a specific version.
...
The rimuru-ai CLI takes the following global flags.
...
Rimuru AI can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `rimuru-ai`) |
