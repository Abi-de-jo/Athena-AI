...
description: Usa la configurazione JSON di Rimuru AI.
---

Puoi configurare Rimuru AI usando un file di configurazione JSON.

---

## Formato

Rimuru AI supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
2. **Config globale** (`~/.config/rimuru-ai/opencode.json`) - preferenze utente
...
:::note
Le directory `.opencode` e `~/.config/rimuru-ai` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di Rimuru AI in `~/.config/rimuru-ai/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/rimuru-ai/tui.json`.
...
Quando Rimuru AI si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Il file di configurazione ha uno schema definito in [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).

La configurazione TUI usa [**`rimurucode.vercel.app/tui.json`**](https://rimurucode.vercel.app/tui.json).
...
Puoi configurare le impostazioni del server per i comandi `rimuru-ai serve` e `rimuru-ai web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Rimuru AI.
...
Puoi configurare provider e modelli da usare in Rimuru AI tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Rimuru AI prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/rimuru-ai/agents/` o `.opencode/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Rimuru AI fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`rimuru-ai run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/rimuru-ai/commands/` o `.opencode/commands/`. [Scopri di piu](/docs/commands).
...
Rimuru AI scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, rimuru-ai **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Rimuru AI con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.opencode/plugins/` o `~/.config/rimuru-ai/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Rimuru AI a usare solo alcuni provider invece di disabilitarli uno per uno.
