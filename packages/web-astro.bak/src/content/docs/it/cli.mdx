---
title: CLI
description: Opzioni e comandi della CLI di Rimuru AI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Rimuru AI, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
rimuru-ai
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Rimuru AI in modo programmatico.
...
Avvia l'interfaccia testuale di Rimuru AI (interfaccia utente da terminale).
...
La CLI di Rimuru AI include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Rimuru AI.
...
Collega un terminale a un backend server di Rimuru AI già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
rimuru-ai attach [url]
```

Questo consente di usare la TUI con un backend Rimuru AI remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `rimuru-ai`) |
...
Rimuru AI si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `rimuru-ai auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

Quando Rimuru AI si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Rimuru AI.
...
Esegui rimuru-ai in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `rimuru-ai serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server rimuru-ai in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `rimuru-ai`) |
...
Avvia un server Rimuru AI headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
rimuru-ai serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di rimuru-ai senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `rimuru-ai`).
...
Gestisci le sessioni Rimuru AI.
...
Elenca tutte le sessioni Rimuru AI.
...
Mostra statistiche di utilizzo token e costo per le sessioni Rimuru AI.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Rimuru AI.

```bash
rimuru-ai import <file>
```

Puoi importare da un file locale o da un URL di condivisione Rimuru AI.
...
Avvia un server Rimuru AI headless con interfaccia web.

```bash
rimuru-ai web
```

Avvia un server HTTP e apre un browser per accedere a Rimuru AI tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `rimuru-ai`).
...
Disinstalla Rimuru AI e rimuove tutti i file correlati.
...
Aggiorna rimuru-ai all'ultima versione o a una versione specifica.
...
La CLI di rimuru-ai accetta i seguenti flag globali.
...
Rimuru AI può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `rimuru-ai`)     |
