---
title: Plugin
description: Scrivi plugin per estendere Rimuru AI.
---

I plugin ti permettono di estendere Rimuru AI agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Rimuru AI.
...
- `~/.config/rimuru-ai/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/rimuru-ai/node_modules/`.
...
1. Config globale (`~/.config/rimuru-ai/opencode.json`)
2. Config di progetto (`opencode.json`)
3. Directory plugin globale (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di rimuru-ai per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere rimuru-ai.
...
:::note
Se usi l'app desktop di Rimuru AI, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a rimuru-ai di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a rimuru-ai:
...
L'helper `tool` crea uno strumento personalizzato che rimuru-ai può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in rimuru-ai insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://rimurucode.vercel.app/docs/sdk) per i dettagli.
