---
title: Provider
description: Usa qualsiasi provider LLM in Rimuru AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Rimuru AI usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Rimuru AI.
...
Rimuru AI Zen e una lista di modelli forniti dal team Rimuru AI, testati e
verificati per funzionare bene con Rimuru AI. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Rimuru AI Zen.
:::

1. Esegui `/connect` nella TUI, seleziona rimuru-ai e vai su [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/auth).
...
Funziona come qualsiasi altro provider in Rimuru AI ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Rimuru AI:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui rimuru-ai:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Rimuru AI non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare rimuru-ai per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché rimuru-ai funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di rimuru-ai.
...
salvare il token nello storage di auth di rimuru-ai.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Rimuru AI usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Rimuru AI
...
Maggior documentazione sulla homepage di [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth).
...
Per usare il tuo abbonamento GitHub Copilot con rimuru-ai:
...
Per usare Google Vertex AI con Rimuru AI:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui rimuru-ai.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da rimuru-ai, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Rimuru AI come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Rimuru AI elencata come sessione separata.
...
Puoi configurare rimuru-ai per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare rimuru-ai per usare modelli locali tramite LM Studio.
...
Puoi configurare rimuru-ai per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Rimuru AI. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/rimuru-ai) per dettagli.
:::
...
Per usare Ollama Cloud con Rimuru AI:
...
3. Copia la chiave API da usare in Rimuru AI.
...
6. **Importante**: Prima di usare modelli cloud in Rimuru AI, devi scaricare le informazioni del modello localmente:
...
Rimuru AI Zen è una lista di modelli testati e verificati forniti dal team Rimuru AI. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Rimuru AI Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Rimuru AI.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di rimuru-ai.
...
5. Puoi anche personalizzarli tramite la configurazione di rimuru-ai. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di rimuru-ai. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con rimuru-ai. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
I campi `limit` permettono a Rimuru AI di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `rimuru-ai auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di rimuru-ai e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config rimuru-ai.
