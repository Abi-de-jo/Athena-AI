...
Per diagnosticare problemi con Rimuru AI, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%.local\share\rimuru-ai\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `rimuru-ai --log-level DEBUG`.

---

## Archiviazione

rimuru-ai salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/rimuru-ai/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%.local\share\rimuru-ai`
...
Rimuru AI Desktop esegue in background un server locale di Rimuru AI (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Rimuru AI` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc`
...
Rimuru AI puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/rimuru-ai/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\rimuru-ai\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Rimuru AI possa ricostruirla.

1. Chiudi completamente Rimuru AI Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/rimuru-ai`
- **Linux**: elimina `~/.cache/rimuru-ai` (oppure esegui `rm -rf ~/.cache/rimuru-ai`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\rimuru-ai`

3. Riavvia Rimuru AI Desktop.

---

### Risolvi problemi di connessione al server

Rimuru AI Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, Rimuru AI Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Rimuru AI.

---

### Notifiche non visualizzate

Rimuru AI Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Rimuru AI nelle impostazioni del sistema operativo, e
...
1. Chiudi Rimuru AI Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Rimuru AI Desktop):

- `rimuru-ai.settings.dat` (desktop default server URL)
- `rimuru-ai.global.dat` e `rimuru-ai.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Rimuru AI:
...
Per capire a quali modelli hai accesso, esegui `rimuru-ai models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%.local\share\rimuru-ai`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. rimuru-ai installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\rimuru-ai`

2. Riavvia rimuru-ai per reinstallare i pacchetti provider piu recenti

Questo forzera rimuru-ai a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
rimuru-ai rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
