...
In Rimuru AI esistono due tipi di agenti: agenti primari e subagenti.
...
Rimuru AI include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Rimuru AI include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Rimuru AI include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Rimuru AI, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
- Globale: `~/.config/rimuru-ai/agents/`
...
Se non specifichi una temperature, Rimuru AI usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Rimuru AI sia per la config specifica del progetto.
...
L'ID modello nella configurazione Rimuru AI usa il formato `provider/model-id`. Per esempio, se stai usando [Rimuru AI Zen](/docs/zen), useresti `rimuru-ai/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `rimuru-ai models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/gowdaman-dev/rimuru-ai).
:::
