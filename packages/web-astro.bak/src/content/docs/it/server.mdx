---
title: Server
description: Interagisci con il server rimuru-ai via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `rimuru-ai serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client rimuru-ai.
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `rimuru-ai`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `rimuru-ai serve` sia per `rimuru-ai web`.
...
Quando esegui `rimuru-ai` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server rimuru-ai per interagire con rimuru-ai in modo programmatico.
:::

Questa architettura permette a rimuru-ai di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `rimuru-ai serve` per avviare un server standalone. Se la TUI di rimuru-ai e gia in esecuzione, `rimuru-ai serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Rimuru AI.
...
Il server rimuru-ai espone le seguenti API.
