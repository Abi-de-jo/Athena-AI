...
Puoi aggiungere strumenti esterni a Rimuru AI usando il _Model Context Protocol_, o MCP. Rimuru AI supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Rimuru AI Config](https://rimurucode.vercel.app/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Rimuru AI gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Rimuru AI:
...
Se il server richiede autenticazione, Rimuru AI ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `rimuru-ai mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Rimuru AI memorizzera' i token in modo sicuro in `~/.local/share/rimuru-ai/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Rimuru AI insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Rimuru AI come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Rimuru AI al tuo account Sentry.
