---
title: Regeln
description: Lege eigene Anweisungen fuer rimuru-ai fest.
---

Du kannst rimuru-ai ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in rimuru-ai den Befehl `/init` aus.
...
So kann rimuru-ai sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht rimuru-ai sie zu erweitern.
...
rimuru-ai kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/rimuru-ai/AGENTS.md` speichern.
Diese gelten in allen rimuru-ai-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Rimuru AI dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/rimuru-ai/AGENTS.md` existiert)
...
Beim Start sucht rimuru-ai in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/rimuru-ai/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `opencode.json` oder in `~/.config/rimuru-ai/opencode.json` angeben.
...
rimuru-ai parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
...
Du kannst rimuru-ai explizit anweisen, externe Dateien zu lesen.
