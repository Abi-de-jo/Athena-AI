...
Sie können externe Tools zu Rimuru AI hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Rimuru AI unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Rimuru AI Config](https://rimurucode.vercel.app/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Rimuru AI übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Rimuru AI Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Rimuru AI beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `rimuru-ai mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Rimuru AI die Token sicher in `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Rimuru AI verfügbar. Sie können sie also wie jedes andere Tool über die Rimuru AI-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Rimuru AI mit Ihrem Sentry-Konto zu verbinden.
