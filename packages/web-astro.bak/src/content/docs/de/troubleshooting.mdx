...
Wenn Rimuru AI Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%.local\share\rimuru-ai\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `rimuru-ai --log-level DEBUG`.

---

## Speicher

rimuru-ai speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/rimuru-ai/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%.local\share\rimuru-ai`
...
Rimuru AI Desktop startet im Hintergrund einen lokalen Rimuru AI-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Rimuru AI`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/rimuru-ai/opencode.jsonc` (oder `~/.config/rimuru-ai/opencode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc`
...
Rimuru AI kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/rimuru-ai/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\rimuru-ai\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Rimuru AI ihn neu aufbauen kann.

1. Beende Rimuru AI Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/rimuru-ai`
- **Linux**: loesche `~/.cache/rimuru-ai` (oder fuehre aus: `rm -rf ~/.cache/rimuru-ai`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\rimuru-ai`

3. Starte Rimuru AI Desktop neu.

---

### Server-Verbindungsprobleme beheben

Rimuru AI Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt Rimuru AI Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Rimuru AI-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Rimuru AI Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Rimuru AI in den OS-Einstellungen aktiviert sind, und
...
1. Beende Rimuru AI Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Rimuru AI Desktop):

- `rimuru-ai.settings.dat` (desktop default server URL)
- `rimuru-ai.global.dat` und `rimuru-ai.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Rimuru AI hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `rimuru-ai models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%.local\share\rimuru-ai`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. rimuru-ai installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\rimuru-ai`

2. Starte rimuru-ai neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt rimuru-ai, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
rimuru-ai erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
