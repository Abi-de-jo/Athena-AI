---
title: CLI
description: Rimuru AI CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der Rimuru AI CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
rimuru-ai
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit Rimuru AI interagieren.

```bash
rimuru-ai run "Explain how closures work in JavaScript"
```
...
Starten Sie die Terminalbenutzeroberfläche Rimuru AI.
...
Der Rimuru AI CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für Rimuru AI verwalten.

```bash
rimuru-ai agent [command]
```
...
Schließen Sie ein Terminal an einen bereits laufenden Rimuru AI-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
rimuru-ai attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend Rimuru AI. Zum Beispiel:

```bash
# Start the backend server for web/mobile access
rimuru-ai web --port 4096 --hostname 0.0.0.0

# In another terminal, attach the TUI to the running backend
rimuru-ai attach http://10.20.30.40:4096
```
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `rimuru-ai`) |
...
Rimuru AI wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `rimuru-ai auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/rimuru-ai/auth.json` gespeichert.

```bash
rimuru-ai auth login
```

Wenn Rimuru AI startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
```bash
rimuru-ai mcp auth list
```

Oder nutzen Sie die Kurzversion.

```bash
rimuru-ai mcp auth ls
```
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in Rimuru AI sehen möchten.

```bash
rimuru-ai models --refresh
```
...
Führen Sie Rimuru AI im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.

```bash
rimuru-ai run [message..]
```
...
```bash "rimuru-ai run"
rimuru-ai run Explain the use of context in Go
```

Sie können auch eine Verbindung zu einer laufenden `rimuru-ai serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
rimuru-ai run --attach http://localhost:4096 "Explain async/await in JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden Rimuru AI-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `rimuru-ai`)                  |
...
Starten Sie einen Headless-Rimuru AI-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
rimuru-ai serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf Rimuru AI-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `rimuru-ai`).
...
Rimuru AI-Sitzungen verwalten.

```bash
rimuru-ai session [command]
```
...
Listen Sie alle Rimuru AI-Sitzungen auf.

```bash
rimuru-ai session list
```
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre Rimuru AI-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer Rimuru AI-Freigabe-URL.

```bash
rimuru-ai import <file>
```

Sie können aus einer lokalen Datei oder einer Rimuru AI-Freigabe-URL importieren.

```bash
rimuru-ai import session.json
rimuru-ai import https://opncd.ai/s/abc123
```
...
Starten Sie einen Headless-Rimuru AI-Server mit einer Weboberfläche.

```bash
rimuru-ai web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf Rimuru AI zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `rimuru-ai`).
...
Deinstallieren Sie Rimuru AI und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert Rimuru AI auf die neueste Version oder eine bestimmte Version.

```bash
rimuru-ai upgrade [target]
```
...
Der Rimuru AI CLI akzeptiert die folgenden globalen Flags.
...
Rimuru AI kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `rimuru-ai`) |
