---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Rimuru AI zu erweitern.
---

Mit Plugins können Sie Rimuru AI erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Rimuru AI zu ändern.
...
- `~/.config/rimuru-ai/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/rimuru-ai/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/rimuru-ai/opencode.json`)
2. Projektkonfiguration (`opencode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Rimuru AI-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Rimuru AI erweitern können.
...
:::note
Wenn Sie die Rimuru AI-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Rimuru AI `.env`-Dateien liest:
...
Plugins können Rimuru AI auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Rimuru AI aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Rimuru AI zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://rimurucode.vercel.app/docs/sdk).
