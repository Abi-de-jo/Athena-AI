---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Rimuru AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Rimuru AI verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Rimuru AI-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Rimuru AI anpassen
config.
...
Rimuru AI Zen ist eine Liste von Modellen, die vom Rimuru AI-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Rimuru AI funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Rimuru AI Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Rimuru AI aus und gehen Sie zu [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/auth).
...
Es funktioniert wie jeder andere Anbieter in Rimuru AI und ist völlig optional.

---

## Rimuru AI Go

Rimuru AI Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Rimuru AI-Team getestet und verifiziert wurden, dass sie gut mit Rimuru AI funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Rimuru AI Go` und gehen Sie zu [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
So verwenden Sie Amazon Bedrock mit Rimuru AI:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Rimuru AI ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Rimuru AI wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können rimuru-ai so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Rimuru AI ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Rimuru AI-Konfiguration hinzufügen.
...
um das Token im Rimuru AI-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Rimuru AI verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Rimuru AI
...
Weitere Dokumentation auf der [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth)-Homepage.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Rimuru AI:
...
So verwenden Sie Google Vertex AI mit Rimuru AI:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Rimuru AI ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Rimuru AI konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede Rimuru AI-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Rimuru AI-Konversation als separate Sitzung aufgeführt.
...
Sie können Rimuru AI für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Rimuru AI über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Rimuru AI über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Rimuru AI konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/rimuru-ai).
:::
...
So verwenden Sie Ollama Cloud mit Rimuru AI:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Rimuru AI.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Rimuru AI verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Rimuru AI Zen ist eine Liste getesteter und verifizierter Modelle, die vom Rimuru AI-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Rimuru AI Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Rimuru AI API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Rimuru AI-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Rimuru AI-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Rimuru AI:
...
Sie können Modelle auch über Ihre Rimuru AI-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Rimuru AI verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann Rimuru AI erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `rimuru-ai auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Rimuru AI-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Rimuru AI-Konfiguration übereinstimmt.
