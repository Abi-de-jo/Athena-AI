---
title: Konfiguration
description: Verwenden Sie die Konfiguration Rimuru AI JSON.
---

Sie können Rimuru AI mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

Rimuru AI unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
2. **Globale Konfiguration** (`~/.config/rimuru-ai/opencode.json`) – Benutzereinstellungen
...
:::note
Die Verzeichnisse `.opencode` und `~/.config/rimuru-ai` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Platzieren Sie Ihre globale Rimuru AI-Konfiguration in `~/.config/rimuru-ai/opencode.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Wenn Rimuru AI gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `rimuru-ai serve` und `rimuru-ai web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres Rimuru AI-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer Rimuru AI-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht Rimuru AI, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer Rimuru AI-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/rimuru-ai/agents/` oder `.opencode/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift Rimuru AI mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`rimuru-ai run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/rimuru-ai/commands/` oder `.opencode/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Rimuru AI lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
Rimuru AI erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert Rimuru AI mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.opencode/plugins/` oder `~/.config/rimuru-ai/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie Rimuru AI darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
