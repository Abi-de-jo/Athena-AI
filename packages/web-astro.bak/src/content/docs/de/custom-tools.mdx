...
description: Erstellen Sie Tools, die der LLM in Rimuru AI aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Rimuru AI wie `read`, `write` und `bash`.
...
- Oder global, indem Sie sie in `~/.config/rimuru-ai/tools/` platzieren.
