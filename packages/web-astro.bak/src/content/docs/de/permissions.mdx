...
Rimuru AI verwendet die `permission`-Konfiguration, um zu entscheiden, ob eine bestimmte Aktion automatisch ausgeführt werden soll, Sie dazu auffordern oder blockiert werden soll.
...
Verwenden Sie `external_directory`, um Toolaufrufe zuzulassen, die Pfade außerhalb des Arbeitsverzeichnisses berühren, in dem Rimuru AI gestartet wurde. Dies gilt für jedes Werkzeug, das einen Pfad als Eingabe verwendet (z. B. `read`, `edit`, `glob`, `grep` und viele `bash`-Befehle).
...
Rimuru AI-Berechtigungen basieren auf Tool-Namen sowie einigen Sicherheitsvorkehrungen:
...
Wenn Sie nichts angeben, beginnt Rimuru AI mit zulässigen Standardeinstellungen:
...
Wenn Rimuru AI zur Genehmigung auffordert, bietet UI drei Ergebnisse:

- `once` – nur diese Anfrage genehmigen
- `always` – zukünftige Anfragen genehmigen, die den vorgeschlagenen Mustern entsprechen (für den Rest der aktuellen Rimuru AI-Sitzung)
