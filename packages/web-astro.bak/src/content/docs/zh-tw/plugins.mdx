---
title: 外掛
description: 編寫自己的外掛來擴展 Rimuru AI。
---

外掛允許您透過掛鉤各種事件和自訂行為來擴展 Rimuru AI。您可以建立外掛來新增新功能、整合外部服務，或修改 Rimuru AI 的預設行為。
...
**npm 外掛**在啟動時使用 Bun 自動安裝。套件及其相依套件會快取在 `~/.cache/rimuru-ai/node_modules/` 中。
...
1. 全域設定 (`~/.config/rimuru-ai/opencode.json`)
2. 專案設定 (`opencode.json`)
3. 全域外掛目錄 (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI 會在啟動時執行 `bun install` 來安裝這些相依套件。之後您的外掛和工具就可以匯入它們了。
...
- `directory`：當前工作目錄。
- `worktree`：git 工作樹路徑。
- `client`：用於與 AI 互動的 Rimuru AI SDK 用戶端。
...
:::note
如果您使用 Rimuru AI 桌面應用程式，它可以在回應就緒或工作階段出錯時自動傳送系統通知。
:::
...
`tool` 輔助函式用於建立 Rimuru AI 可呼叫的自訂工具。它接受一個 Zod schema 函式，並回傳一個工具定義，包含：

- `description`：工具的功能描述
- `args`：工具參數的 Zod schema
- `execute`：工具被呼叫時執行的函式

您的自訂工具將與內建工具一起在 Rimuru AI 中可用。
...
日誌層級：`debug`、`info`、`warn`、`error`。詳情請參閱 [SDK 文件](https://rimurucode.vercel.app/docs/sdk)。
