...
代理技能讓 Rimuru AI 能夠從你的儲存庫或主目錄中發現可重複使用的指令。
...
- 全域設定：`~/.config/rimuru-ai/skills/<name>/SKILL.md`
...
對於專案本地路徑，Rimuru AI 會從當前工作目錄向上遍歷，直到到達 git 工作樹根目錄。
在此過程中，它會載入 `.opencode/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全域定義也會從 `~/.config/rimuru-ai/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中載入。
...
Rimuru AI 會在 `skill` 工具描述中列出可用技能。
每個條目包含技能名稱和描述：
