---
title: 設定
description: 使用 Rimuru AI JSON 設定。
---

您可以使用 JSON 設定檔來設定 Rimuru AI。

---

## 格式

Rimuru AI 支援 **JSON** 和 **JSONC**（帶註解的 JSON）格式。
...
2. **全域設定**（`~/.config/rimuru-ai/opencode.json`）- 使用者偏好
...
:::note
`.opencode` 和 `~/.config/rimuru-ai` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
將全域 Rimuru AI 設定放在 `~/.config/rimuru-ai/opencode.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
當 Rimuru AI 啟動時，它會在當前目錄中尋找設定檔，或向上遍歷到最近的 Git 目錄。
...
設定檔具有在 [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json) 中定義的 Schema。
...
您可以透過 `server` 選項為 `rimuru-ai serve` 和 `rimuru-ai web` 指令設定伺服器設定。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他裝置發現您的 Rimuru AI 伺服器。
...
您可以透過 `provider`、`model` 和 `small_model` 選項在 Rimuru AI 設定中設定要使用的供應商和模型。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商有更便宜的模型可用，Rimuru AI 會嘗試使用該模型，否則會回退到您的主模型。
...
您還可以使用 `~/.config/rimuru-ai/agents/` 或 `.opencode/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
預設代理必須是主代理（不能是子代理）。可以是內建代理（如 `"build"` 或 `"plan"`），也可以是您定義的[自訂代理](/docs/agents)。如果指定的代理不存在或是子代理，Rimuru AI 將回退到 `"build"` 並發出警告。

此設定適用於所有介面：TUI、CLI（`rimuru-ai run`）、桌面應用程式和 GitHub Action。
...
您還可以使用 `~/.config/rimuru-ai/commands/` 或 `.opencode/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
Rimuru AI 啟動時會自動下載新版本。您可以使用 `autoupdate` 選項停用此功能。
...
預設情況下，Rimuru AI **允許所有操作**，無需明確批准。您可以使用 `permission` 選項變更此行為。
...
[外掛程式](/docs/plugins)透過自訂工具、掛鉤和整合來擴展 Rimuru AI。

將外掛程式檔案放置在 `.opencode/plugins/` 或 `~/.config/rimuru-ai/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。
...
當您希望限制 Rimuru AI 僅使用特定供應商，而不是逐一停用其他供應商時，此選項非常有用。
