---
title: Config
description: Using the Rimuru AI JSON config.
---

You can configure Rimuru AI using a JSON config file.

---

## Format

Rimuru AI supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
2. **Global config** (`~/.config/rimuru-ai/opencode.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`opencode.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/rimuru-ai/` on macOS) - admin-controlled
...
:::note
The `.opencode` and `~/.config/rimuru-ai` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global Rimuru AI config in `~/.config/rimuru-ai/opencode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/rimuru-ai/tui.json`.
...
When Rimuru AI starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/rimuru-ai/` |
| Linux    | `/etc/rimuru-ai/`                         |
| Windows  | `%ProgramData%\rimuru-ai`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Rimuru AI reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Rimuru AI checks these paths:
...
Use the `ai.opencode.managed` PayloadType. The Rimuru AI config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).

TUI config uses [**`rimurucode.vercel.app/tui.json`**](https://rimurucode.vercel.app/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://rimurucode.vercel.app/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
You can configure server settings for the `rimuru-ai serve` and `rimuru-ai web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Rimuru AI server.
...
If not specified, Rimuru AI will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Rimuru AI config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Rimuru AI tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Rimuru AI actions on configured resources. Currently, policies can control which providers Rimuru AI may use.
...
Rimuru AI normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Rimuru AI omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/rimuru-ai/agents/` or `.opencode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Rimuru AI will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`rimuru-ai run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/rimuru-ai/commands/` or `.opencode/commands/`. [Learn more here](/docs/commands).
...
Rimuru AI uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Rimuru AI will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, rimuru-ai **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Rimuru AI with custom tools, hooks, and integrations.

Place plugin files in `.opencode/plugins/` or `~/.config/rimuru-ai/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Rimuru AI to only use specific providers rather than disabling them one by one.
