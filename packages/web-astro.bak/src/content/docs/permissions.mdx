...
Rimuru AI uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Rimuru AI was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Rimuru AI permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, Rimuru AI starts from permissive defaults:
...
When Rimuru AI prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Rimuru AI session)
