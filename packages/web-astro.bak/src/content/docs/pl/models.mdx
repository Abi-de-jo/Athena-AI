...
rimuru-ai używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługi uruchamiania modeli pierwszych.

---

## Dostawcy

Dostępna usługa dostawcy jest dostępna. Jeśli otrzymasz poświadczenie dostawcy za pomocą polecenia `/connect`, będzie on dostępny po uruchomieniu rimuru-ai.
...
Oto kilka modeli, które dobrze współpracują z rimuru-ai, w kolejności. (Nie jest to lista wyczerpująca i nie jest konieczna aktualna):
...
Tutaj pełny identyfikator do `provider_id/model_id`. Na przykład, użycie [Rimuru AI Zen](/docs/zen), przestrzeganie `rimuru-ai/gpt-5.1-codex` dla Kodeksu GPT 5.1.
...
Wiele modeli obsługuje wiele wariantów lub różnych wybranych. rimuru-ai jest dostarczany z podstawowymi wariantami wariantów dla dostawców.

### Wbudowane warianty

rimuru-ai jest dostarczany z domyślnymi wariantami dla wielu dostawców:
...
Po uruchomieniu rimuru-ai sprawdzanie modeli w następującej kolejności:

1. Flaga wiersza autora `--model` lub `-m`. Format jest taki sam jak w pliku konfiguracyjnym: `provider_id/model_id`.

2. Lista modeli w konstrukcji rimuru-ai.
