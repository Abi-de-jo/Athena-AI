...
Aby debugować problemy z rimuru-ai, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%.local\share\rimuru-ai\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `rimuru-ai --log-level DEBUG`.

---

## Przechowywanie danych

rimuru-ai przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/rimuru-ai/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%.local\share\rimuru-ai`
...
rimuru-ai Desktop uruchamia w tle lokalny serwer rimuru-ai (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Rimuru AI` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/rimuru-ai/opencode.jsonc` (lub `~/.config/rimuru-ai/opencode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc`
...
rimuru-ai może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/rimuru-ai/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\rimuru-ai\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby rimuru-ai mógł ją odbudować.

1. Całkowicie zamknij rimuru-ai Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/rimuru-ai`
- **Linux**: usuń `~/.cache/rimuru-ai` (lub uruchom `rm -rf ~/.cache/rimuru-ai`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\rimuru-ai`

3. Uruchom ponownie rimuru-ai Desktop.

---

### Napraw problemy z połączeniem z serwerem

rimuru-ai Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
W systemie Windows rimuru-ai Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami rimuru-ai.

---

### Brak powiadomień

rimuru-ai Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla rimuru-ai w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie rimuru-ai Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji rimuru-ai Desktop):

- `rimuru-ai.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `rimuru-ai.global.dat` i `rimuru-ai.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z rimuru-ai:
...
3. Upewnij się, że masz najnowszą wersję z `rimuru-ai upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `rimuru-ai models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%.local\share\rimuru-ai`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. rimuru-ai dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\rimuru-ai`

2. Uruchom ponownie kod rimuru-ai, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to rimuru-ai do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
rimuru-ai wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
