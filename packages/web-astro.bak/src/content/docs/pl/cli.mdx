---
title: CLI
description: Opcje i polecenia wiersza poleceń Rimuru AI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Rimuru AI domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
rimuru-ai
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Rimuru AI programowo.
...
Uruchamia interfejs terminalowy (TUI) Rimuru AI.
...
Interfejs CLI Rimuru AI zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Rimuru AI.
...
Dołącz terminal do już działającego serwera backendu Rimuru AI uruchomionego za pomocą `serve` lub `web`.

```bash
rimuru-ai attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Rimuru AI. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `rimuru-ai`) |
...
Rimuru AI integruje się z wieloma dostawcami. Możesz użyć `rimuru-ai auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

Podczas uruchamiania Rimuru AI ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Rimuru AI.
...
Uruchom Rimuru AI w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `rimuru-ai serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera Rimuru AI (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `rimuru-ai`) |
...
Uruchom serwer Rimuru AI (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
rimuru-ai serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Rimuru AI bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `rimuru-ai`).
...
Lista wszystkich sesji Rimuru AI.
...
Pokaż statystyki wykorzystania tokenów i sesji Rimuru AI.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Rimuru AI.
...
Uruchom serwer Rimuru AI z interfejsem internetowym.

```bash
rimuru-ai web
```

Uruchamia to serwer HTTP i udostępnia Rimuru AI przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `rimuru-ai`).
...
Odinstaluj Rimuru AI i usuń wszystkie powiązane pliki.
...
Aktualizuj Rimuru AI do najnowszej lub określonej wersji.
...
Interfejs CLI Rimuru AI przyjmuje następujące flagi globalne dla każdego polecenia.
...
Rimuru AI można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `rimuru-ai`)    |
