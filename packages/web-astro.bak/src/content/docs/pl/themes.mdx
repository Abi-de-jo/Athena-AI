...
Dzięki rimuru-ai możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie rimuru-ai używa naszego własnego motywu `rimuru-ai`.
...
rimuru-ai ma kilka wbudowanych motywów.
...
- Chcą, aby Rimuru AI pasował do wyglądu ich terminala
...
rimuru-ai obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/rimuru-ai/themes/*.json` lub `$XDG_CONFIG_HOME/rimuru-ai/themes/*.json`
