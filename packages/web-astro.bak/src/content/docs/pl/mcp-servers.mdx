...
Możesz dodać dodatkowe informacje do rimuru-ai za pomocą _Model Context Protocol_, w skrócie MCP. rimuru-ai obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [rimuru-ai Config](https://rimurucode.vercel.app/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
rimuru-ai automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, rimuru-ai:
...
Jeśli serwer wymaga uwierzytelnienia, rimuru-ai poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `rimuru-ai mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji rimuru-ai bezpieczne przechowa tokeny w `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w rimuru-ai, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie rimuru-ai, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć rimuru-ai z kontem Sentry.
