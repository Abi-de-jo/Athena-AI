...
Rimuru AI zawiera listę skrótów klawiszowych, które można dostosować za pomocą `tui.json`.
...
rimuru-ai używa klucza `leader` dla głównych skrótów klawiszowych. Rozwiązanie problemu konfliktów w terminalu.
...
Pole sprawdzania aplikacji komputerowej rimuru-ai obsługuje popularne skróty w stylu Readline/Emacs do edycji tekstu. Są one dostępne i obecnie nie można ich skonfigurować za pomocą `opencode.json`.
