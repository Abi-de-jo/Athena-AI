...
rimuru-ai używa `permission`, aby uniknąć, czy dana akcja została wykonana automatycznie, zachowana monit lub została zablokowana.
...
Użycie `external_directory`, aby zezwolić na wywołanie narzędzia, które obsługuje obsługę poza katalogiem roboczym, z uruchomieniem rimuru-ai. Dotyczy każdego narzędzia, które jako dane wejściowe zostało przyjęte (na przykład `read`, `edit`, `glob`, `grep` i wiele założycieli `bash`).
...
Uprawnienia rimuru-ai są określane na podstawie nazwy narzędzia i kilku zabezpieczeń:
...
Jeżeli nic nie jest określone, rimuru-ai zostanie uruchomione od dopuszczalnych rozwiązań diagnostycznych:
...
Gdy rimuru-ai poprosi o zatwierdzenie, interfejs użytkownika oferujący trzy wyniki:

- `once` — zatwierdź tylko tę prośbę
- `always` — zastosowanie, które następuje po zastosowaniu sugerowanych wzorców (przez następczą bieżącą sesję rimuru-ai)
- `reject` — odrzucić
