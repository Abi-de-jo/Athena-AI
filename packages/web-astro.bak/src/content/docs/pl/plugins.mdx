---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć rimuru-ai.
---

Wtyczki umożliwiające wyłączenie rimuru-ai poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania rimuru-ai.
...
- `~/.config/rimuru-ai/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/rimuru-ai/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/rimuru-ai/opencode.json`)
2. Konfiguracja projektu (`opencode.json`)
3. Globalny katalog wtyczek (`~/.config/rimuru-ai/plugins/`)
...
rimuru-ai uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z rimuru-ai do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do rimuru-ai.
...
:::note
Jeśli korzystasz z aplikacji komputerowej rimuru-ai, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj rimuru-ai czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do rimuru-ai:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod rimuru-ai. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://rimurucode.vercel.app/docs/sdk).
