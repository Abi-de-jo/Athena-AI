...
W Rimuru AI obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Rimuru AI zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Rimuru AI ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Rimuru AI ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Rimuru AI, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
- Globalnie: `~/.config/rimuru-ai/agents/`
...
Jeśli nie zostanie skonfigurowana, Rimuru AI używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Rimuru AI, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Rimuru AI w `provider/model-id`. Na przykład, [Rimuru AI Zen](/docs/zen), użyj `rimuru-ai/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `rimuru-ai models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/gowdaman-dev/rimuru-ai).
:::
