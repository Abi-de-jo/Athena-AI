---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w rimuru-ai.
---

import config from "../../../../config.mjs"
export const console = config.console

rimuru-ai używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji rimuru-ai.
...
w `~/.local/share/rimuru-ai/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim rimuru-ai
config.
...
Rimuru AI Zen to lista modeli dostarczonych przez zespół rimuru-ai, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z rimuru-ai. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Rimuru AI Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Rimuru AI Zen` i przejdź do [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Działa jak każdy inny dostawca w rimuru-ai i jest całkowicie opcjonalny w użyciu.

---

## Rimuru AI Go

Rimuru AI Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół rimuru-ai, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z rimuru-ai.

1. Uruchom polecenie `/connect` w TUI, wybierz `Rimuru AI Go` i przejdź do [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Aby używać Amazon Bedrock z rimuru-ai:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania rimuru-ai:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w rimuru-ai nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować rimuru-ai do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod rimuru-ai działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji rimuru-ai.
...
rimuru-ai używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować rimuru-ai
...
Więcej informacji znajdziesz na stronie [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem rimuru-ai:
...
Aby używać Google Vertex AI z rimuru-ai:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania rimuru-ai.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą rimuru-ai, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację rimuru-ai jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja rimuru-ai będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować rimuru-ai tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować rimuru-ai tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować rimuru-ai tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem rimuru-ai. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/rimuru-ai).
:::
...
Aby korzystać z Ollama Cloud z rimuru-ai:
...
3. Skopiuj klucz API do użycia w rimuru-ai.
...
6. **Ważne**: Przed użyciem modeli chmurowych w rimuru-ai musisz pobrać informacje o modelu lokalnie:
...
Rimuru AI Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół rimuru-ai. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Rimuru AI Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API rimuru-ai.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji rimuru-ai.
...
5. Możesz je także dostosować za pomocą konfiguracji rimuru-ai. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z rimuru-ai:
...
Możesz także dostosować modele za pomocą konfiguracji rimuru-ai. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z rimuru-ai. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
Pola `limit` pozwalają rimuru-ai zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `rimuru-ai auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację rimuru-ai i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji rimuru-ai.
