---
title: Konfiguracja
description: Ustawienia Rimuru AI w pliku JSON.
---

Możesz dostosować Rimuru AI za pomocą pliku konfiguracyjnego JSON.

---

## Format

Rimuru AI obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
2. **Konfiguracja globalna** (`~/.config/rimuru-ai/opencode.json`) – preferencje użytkownika
...
:::note
W katalogach `.opencode` i `~/.config/rimuru-ai` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację Rimuru AI w `~/.config/rimuru-ai/opencode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/rimuru-ai/tui.json`.
...
Kiedy Rimuru AI się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).
...
Możesz skonfigurować serwer dla `rimuru-ai serve` i `rimuru-ai web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Rimuru AI.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Rimuru AI wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/rimuru-ai/agents/` lub `.opencode/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Rimuru AI powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`rimuru-ai run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/rimuru-ai/commands/` lub `.opencode/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Rimuru AI może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Rimuru AI nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Rimuru AI **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Rimuru AI o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.opencode/plugins/` lub `~/.config/rimuru-ai/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Rimuru AI tylko do określonych dostawców.
