---
title: Zasady
description: Ustaw niestandardowe instrukcje dla rimuru-ai.
---

Możesz dostarczyć niestandardowe instrukcje do rimuru-ai, tworząc plik `AGENTS.md`. Jest to podobne do zasad Cursora. Zawiera instrukcje, które zostaną uwzględnione w kontekście LLM, aby dostosować jego zachowanie do konkretnego projektu.
...
rimuru-ai obsługuje także odczytywanie pliku `AGENTS.md` z wielu lokalizacji. A to służy różnym celom.
...
Możesz także mieć reguły globalne w pliku `~/.config/rimuru-ai/AGENTS.md`. Jest to stosowane we wszystkich sesjach rimuru-ai.
...
W przypadku użytkowników migrujących z Claude Code rimuru-ai obsługuje konwencje plików Claude Code jako rozwiązania awaryjne:

- **Zasady projektu**: `CLAUDE.md` w katalogu projektu (używane, jeśli nie istnieje `AGENTS.md`)
- **Zasady globalne**: `~/.claude/CLAUDE.md` (używane, jeśli nie istnieje `~/.config/rimuru-ai/AGENTS.md`)
...
Po uruchomieniu rimuru-ai szuka plików reguł w następującej kolejności:

1. **Pliki lokalne** poprzez przejście z bieżącego katalogu (`AGENTS.md`, `CLAUDE.md`)
2. **Plik globalny** w `~/.config/rimuru-ai/AGENTS.md`
3. **Plik Claude Code** w `~/.claude/CLAUDE.md` (chyba że wyłączony)

Pierwszy pasujący plik wygrywa w każdej kategorii. Na przykład, jeśli masz zarówno `AGENTS.md`, jak i `CLAUDE.md`, używany jest tylko `AGENTS.md`. Podobnie `~/.config/rimuru-ai/AGENTS.md` ma pierwszeństwo przed `~/.claude/CLAUDE.md`.

---

## Instrukcje niestandardowe

Możesz określić niestandardowe pliki instrukcji w swoim `opencode.json` lub globalnym `~/.config/rimuru-ai/opencode.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
Chociaż rimuru-ai nie analizuje automatycznie odniesień do plików w `AGENTS.md`, możesz osiągnąć podobną funkcjonalność na dwa sposoby:
...
Możesz nauczyć rimuru-ai czytać pliki zewnętrzne, podając wyraźne instrukcje w swoim `AGENTS.md`. Oto praktyczny przykład:
...
- Upewnij się, że rimuru-ai ładuje pliki tylko wtedy, gdy są potrzebne do konkretnego zadania
