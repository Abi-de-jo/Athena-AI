---
title: CLI
description: Rimuru AI CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Rimuru AI CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
rimuru-ai
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、Rimuru AI をプログラム的に操作できるようになります。
...
Rimuru AI ターミナルユーザーインターフェイスを開始します。
...
Rimuru AI CLI には次のコマンドもあります。

---

### agent

Rimuru AI のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の Rimuru AI バックエンドサーバーにターミナルを接続します。

```bash
rimuru-ai attach [url]
```

これにより、リモート Rimuru AI バックエンドで TUI を使用できるようになります。例えば：
...
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `rimuru-ai`） |
...
Rimuru AI は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`rimuru-ai auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/rimuru-ai/auth.json` に保存されます。

```bash
rimuru-ai auth login
```

Rimuru AI が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを Rimuru AI で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで rimuru-ai を実行します。
...
実行中の `rimuru-ai serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
| <nobr><code>{"--attach"}</code></nobr>   |          | 実行中の rimuru-ai サーバー (http://localhost:4096 など) に接続します。                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`     | Basic 認証パスワード（デフォルトは `OPENCODE_SERVER_PASSWORD`）                           |
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `rimuru-ai`）         |
...
API アクセスのためにヘッドレス Rimuru AI サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
rimuru-ai serve
```

これにより、TUI インターフェイスを使用せずに rimuru-ai 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `rimuru-ai`)。
...
Rimuru AI セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは Rimuru AI 共有 URL からセッションデータをインポートします。

```bash
rimuru-ai import <file>
```

ローカルファイルまたは Rimuru AI 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス Rimuru AI サーバーを起動します。

```bash
rimuru-ai web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて Rimuru AI にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `rimuru-ai`)。
...
Rimuru AI をアンインストールし、関連ファイルをすべて削除します。
...
rimuru-ai を最新バージョンまたは特定のバージョンに更新します。
...
rimuru-ai CLI は次のグローバルフラグを受け取ります。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `rimuru-ai`) をオーバーライドします。 |
