...
Rimuru AI を使用すると、いくつかの組み込みテーマから 1 つを選択したり、ターミナルのテーマに適合するテーマを使用したり、独自のカスタムテーマを定義したりできます。

デフォルトでは、Rimuru AI は独自の `rimuru-ai` テーマを使用します。
...
Rimuru AI にはいくつかの組み込みテーマが付属しています。
...
Rimuru AI は、ユーザーがテーマを簡単に作成およびカスタマイズできる柔軟な JSON ベースのテーマシステムをサポートしています。
...
2. **ユーザー設定ディレクトリ** - `~/.config/rimuru-ai/themes/*.json` または `$XDG_CONFIG_HOME/rimuru-ai/themes/*.json` で定義されます
...
Rimuru AI は、ユーザーがテーマを簡単に作成およびカスタマイズできる柔軟な JSON ベースのテーマ システムをサポートしています。
