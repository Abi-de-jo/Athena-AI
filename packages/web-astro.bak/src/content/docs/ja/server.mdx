---
title: サーバー
description: HTTP 経由で Rimuru AI サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`rimuru-ai serve` コマンドは、Rimuru AI クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
HTTP 基本認証でサーバーを保護するには、`OPENCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `rimuru-ai` になるか、`OPENCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`rimuru-ai serve` と `rimuru-ai web` の両方に当てはまります。
...
`rimuru-ai` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Rimuru AI サーバーを使用して、プログラムで Rimuru AI と対話します。
:::

`rimuru-ai serve` を実行してスタンドアロンサーバーを起動できます。すでに Rimuru AI TUI を実行している場合でも、`rimuru-ai serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Rimuru AI [IDE](/docs/ide) プラグインによって使用されます。
