---
title: プラグイン
description: Rimuru AI を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Rimuru AI を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Rimuru AI のデフォルトの動作を変更したりできます。
...
- `~/.config/rimuru-ai/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/rimuru-ai/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/rimuru-ai/opencode.json`)
2. プロジェクト設定 (`opencode.json`)
3. グローバルプラグインディレクトリ (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Rimuru AI SDK クライアント。
...
ここでは、Rimuru AI を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://rimurucode.vercel.app/docs/sdk) を参照してください。
...
Rimuru AI が `.env` ファイルを読み取らないようにします。
...
プラグインは Rimuru AI にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Rimuru AI が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Rimuru AI で使用できます。
