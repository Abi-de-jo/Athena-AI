---
title: CLI
description: Opciones y comandos de la CLI de Rimuru AI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Rimuru AI de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
rimuru-ai
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Rimuru AI mediante programación.
...
Inicie la interfaz de usuario del terminal Rimuru AI.
...
La CLI de Rimuru AI también tiene los siguientes comandos.

---

### agent

Administrar agentes para Rimuru AI.
...
Conecte una terminal a un servidor backend Rimuru AI que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
rimuru-ai attach [url]
```

Esto permite usar TUI con un backend remoto Rimuru AI. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `rimuru-ai`) |
...
Rimuru AI funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `rimuru-ai auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

Cuando se inicia Rimuru AI, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Rimuru AI.
...
Ejecute rimuru-ai en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `rimuru-ai serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor rimuru-ai en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `rimuru-ai`) |
...
Inicie un servidor Rimuru AI sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
rimuru-ai serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad rimuru-ai sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `rimuru-ai`).
...
Administrar Rimuru AI sesiones.
...
Enumere todas las sesiones Rimuru AI.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Rimuru AI.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Rimuru AI.

```bash
rimuru-ai import <file>
```

Puede importar desde un archivo local o una URL compartida Rimuru AI.
...
Inicie un servidor Rimuru AI sin cabeza con una interfaz web.

```bash
rimuru-ai web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Rimuru AI a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `rimuru-ai`).
...
Desinstale Rimuru AI y elimine todos los archivos relacionados.
...
Actualiza rimuru-ai a la última versión o a una versión específica.
...
La CLI de Rimuru AI toma las siguientes banderas globales.
...
Rimuru AI se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `rimuru-ai`) |
