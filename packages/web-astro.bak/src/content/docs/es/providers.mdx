---
title: Proveedores
description: Usando cualquier proveedor LLM en Rimuru AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Rimuru AI usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Rimuru AI.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Rimuru AI
configuración.
...
Rimuru AI Zen es una lista de modelos proporcionados por el equipo Rimuru AI que han sido
probado y verificado para funcionar bien con Rimuru AI. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Rimuru AI Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione rimuru-ai y diríjase a [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/auth).
...
Funciona como cualquier otro proveedor en Rimuru AI y su uso es completamente opcional.

---

## Rimuru AI Go

Rimuru AI Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Rimuru AI que han sido
probado y verificado para funcionar bien con Rimuru AI.

1. Ejecute el comando `/connect` en TUI, seleccione `Rimuru AI Go` y diríjase a [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Para usar Amazon Bedrock con Rimuru AI:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta rimuru-ai:
...
:::info
El uso de su suscripción Claude Pro/Max en Rimuru AI no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar rimuru-ai para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que rimuru-ai funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración rimuru-ai.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Rimuru AI`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación rimuru-ai.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Rimuru AI utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Rimuru AI
...
Más documentación en la página de inicio de [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth).
...
Para utilizar su suscripción GitHub Copilot con rimuru-ai:
...
Para utilizar Google Vertex AI con Rimuru AI:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta rimuru-ai.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de rimuru-ai, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Rimuru AI como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Rimuru AI enumerada como una sesión separada.
...
Puede configurar rimuru-ai para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar rimuru-ai para usar modelos locales a través de LM Studio.
...
Puedes configurar rimuru-ai para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Rimuru AI. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/rimuru-ai) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Rimuru AI:
...
3. Copie la clave API para usarla en Rimuru AI.
...
6. **Importante**: Antes de usar modelos en la nube en Rimuru AI, debe obtener la información del modelo localmente:
...
Rimuru AI Zen es una lista de modelos probados y verificados proporcionada por el equipo Rimuru AI. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Rimuru AI Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Rimuru AI API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración rimuru-ai.
...
5. También puedes personalizarlos a través de tu configuración rimuru-ai. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Rimuru AI:
...
También puede personalizar modelos a través de su configuración rimuru-ai. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con rimuru-ai. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
Los campos `limit` le permiten a Rimuru AI comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `rimuru-ai auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración rimuru-ai y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración rimuru-ai.
