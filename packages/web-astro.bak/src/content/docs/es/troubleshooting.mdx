...
Para depurar problemas con Rimuru AI, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/rimuru-ai/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%.local\share\rimuru-ai\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `rimuru-ai --log-level DEBUG`.

---

## Almacenamiento

rimuru-ai almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/rimuru-ai/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%.local\share\rimuru-ai`
...
Rimuru AI Desktop ejecuta un servidor Rimuru AI local (el sidecar `opencode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Rimuru AI` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/rimuru-ai/opencode.jsonc` (o `~/.config/rimuru-ai/opencode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc`
...
Rimuru AI también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/rimuru-ai/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\rimuru-ai\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Rimuru AI pueda reconstruirlo.

1. Salga de Rimuru AI Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/rimuru-ai`
- **Linux**: eliminar `~/.cache/rimuru-ai` (o ejecutar `rm -rf ~/.cache/rimuru-ai`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\rimuru-ai`

3. Reinicie el escritorio Rimuru AI.

---

### Solucionar problemas de conexión del servidor

Rimuru AI Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
En Windows, el escritorio Rimuru AI requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Rimuru AI.

---

### Notificaciones que no se muestran

Rimuru AI Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Rimuru AI en la configuración de su sistema operativo, y
...
1. Salga del escritorio Rimuru AI.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Rimuru AI):

- `rimuru-ai.settings.dat` (URL del servidor predeterminado de escritorio)
- `rimuru-ai.global.dat` y `rimuru-ai.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Rimuru AI:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/gowdaman-dev/rimuru-ai/issues**](https://github.com/gowdaman-dev/rimuru-ai/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**rimurucode.vercel.app/discord**](https://rimurucode.vercel.app/discord)
...
3. Asegúrese de tener la última versión con `rimuru-ai upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `rimuru-ai models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/rimuru-ai
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%.local\share\rimuru-ai`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. rimuru-ai instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/rimuru-ai
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\rimuru-ai`

2. Reinicie rimuru-ai para reinstalar los paquetes del proveedor más recientes.

Esto obligará a rimuru-ai a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
rimuru-ai detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
