---
title: Complementos
description: Escriba sus propios complementos para extender Rimuru AI.
---

Los complementos le permiten extender Rimuru AI al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Rimuru AI.
...
- `~/.config/rimuru-ai/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/rimuru-ai/node_modules/`.
...
1. Configuración global (`~/.config/rimuru-ai/opencode.json`)
2. Configuración del proyecto (`opencode.json`)
3. Directorio global de complementos (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK rimuru-ai para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar rimuru-ai.
...
:::note
Si está utilizando la aplicación de escritorio Rimuru AI, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que rimuru-ai lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a rimuru-ai:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@rimuru-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que rimuru-ai puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para rimuru-ai junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://rimurucode.vercel.app/docs/sdk) para obtener más detalles.
