...
Puede agregar herramientas externas a Rimuru AI utilizando el _Model Context Protocol_ o MCP. Rimuru AI admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Rimuru AI Config](https://rimurucode.vercel.app/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Rimuru AI maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Rimuru AI:
...
Si el servidor requiere autenticación, Rimuru AI le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `rimuru-ai mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Rimuru AI almacenará los tokens de forma segura en `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Rimuru AI, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Rimuru AI como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Rimuru AI a su cuenta Sentry.
