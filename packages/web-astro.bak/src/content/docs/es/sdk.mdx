---
title: SDK
description: Cliente JS con seguridad de tipos para el servidor rimuru-ai.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El SDK rimuru-ai JS/TS proporciona un cliente con seguridad de tipos para interactuar con el servidor.
Úselo para crear integraciones y controlar rimuru-ai mediante programación.
...
Cree una instancia de rimuru-ai:
...
const rimuru-ai = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${rimuru-ai.server.url}`)

rimuru-ai.server.close()
```

## Solo cliente

Si ya tiene una instancia en ejecución de rimuru-ai, puede crear una instancia de cliente para conectarse a ella:
...
```typescript
import type { Session, Message, Part } from "@rimuru-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*rimuru-ai" },
})
