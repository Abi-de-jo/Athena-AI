---
title: Configuración
description: Usando la configuración Rimuru AI JSON.
---

Puede configurar Rimuru AI usando un archivo de configuración JSON.

---

## Formato

Rimuru AI admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
2. **Configuración global** (`~/.config/rimuru-ai/opencode.json`) - preferencias del usuario
...
:::note
Los directorios `.opencode` y `~/.config/rimuru-ai` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global Rimuru AI en `~/.config/rimuru-ai/opencode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Cuando se inicia Rimuru AI, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
El archivo de configuración tiene un esquema definido en [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).
...
Puede configurar los ajustes del servidor para los comandos `rimuru-ai serve` y `rimuru-ai web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor Rimuru AI.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración Rimuru AI a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, Rimuru AI intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración Rimuru AI a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/rimuru-ai/agents/` o `.opencode/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, Rimuru AI recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`rimuru-ai run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/rimuru-ai/commands/` o `.opencode/commands/`. [Más información aquí](/docs/commands).
...
Rimuru AI descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
De forma predeterminada, rimuru-ai **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Los [complementos](/docs/plugins) amplían Rimuru AI con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.opencode/plugins/` o `~/.config/rimuru-ai/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir Rimuru AI para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
