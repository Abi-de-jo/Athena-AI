---
title: Providers
description: Using any LLM provider in Rimuru AI.
---

import config from "../../../config.mjs"
export const console = config.console

Rimuru AI uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Rimuru AI config.
...
in `~/.local/share/rimuru-ai/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Rimuru AI
config.
...
{
  "$schema": "https://rimurucode.vercel.app/config.json",
  "provider": {
    "anthropic": {
      "options": {
...
Rimuru AI Zen is a list of models provided by the Rimuru AI team that have been
tested and verified to work well with Rimuru AI. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Rimuru AI Zen.
:::

1. Run the `/connect` command in the TUI, select `Rimuru AI Zen`, and head to [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
It works like any other provider in Rimuru AI and is completely optional to use.

---

## Rimuru AI Go

Rimuru AI Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Rimuru AI team that have been
tested and verified to work well with Rimuru AI.

1. Run the `/connect` command in the TUI, select `Rimuru AI Go`, and head to [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
To use Amazon Bedrock with Rimuru AI:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running rimuru-ai:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY rimuru-ai

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile rimuru-ai

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX rimuru-ai
   ```
...
Rimuru AI. Anthropic explicitly prohibits this.

Previous versions of Rimuru AI came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Rimuru AI with zero setup:
...
You can configure rimuru-ai to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for rimuru-ai to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX rimuru-ai
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX rimuru-ai
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your rimuru-ai config.
...
Rimuru AI supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Rimuru AI uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Rimuru AI. Sign in and approve.

   :::note
   Rimuru AI requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Rimuru AI surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Rimuru AI forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Rimuru AI is experimental. Features, configuration, and
behavior may change in future releases.
:::

Rimuru AI integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in rimuru-ai auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Rimuru AI uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Rimuru AI
...
More documentation on [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, Rimuru AI will:
...
4. Route tool execution requests through Rimuru AI's permission-gated tool system
...
{
  "$schema": "https://rimurucode.vercel.app/config.json",
  "plugin": ["rimuru-ai-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with rimuru-ai:
...
To use Google Vertex AI with Rimuru AI:
...
2. Set the required environment variables:
   ...
   Set them while running rimuru-ai.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id rimuru-ai
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through rimuru-ai, you can always configure it yourself.
...
```jsonc title="~/.config/rimuru-ai/opencode.jsonc"
{
  "$schema": "https://rimurucode.vercel.app/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Rimuru AI conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Rimuru AI conversation listed as a separate session.
...
You can configure rimuru-ai to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure rimuru-ai to use local models through LM Studio.
...
You can configure rimuru-ai to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Rimuru AI. See the [Ollama integration docs](https://docs.ollama.com/integrations/rimuru-ai) for details.
:::
...
To use Ollama Cloud with Rimuru AI:
...
3. Copy the API key for use in Rimuru AI.
...
6. **Important**: Before using cloud models in Rimuru AI, you must pull the model information locally:
...
Rimuru AI Zen is a list of tested and verified models provided by the Rimuru AI team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Rimuru AI Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Enter your Rimuru AI API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your rimuru-ai config.
...
5. You can also customize them through your rimuru-ai config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' rimuru-ai
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group rimuru-ai
   ```
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=rimuru-ai&utm_medium=docs&utm_campaign=cortex-provider).

rimuru-ai's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Rimuru AI captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting rimuru-ai:
...
You can also customize models through your rimuru-ai config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Rimuru AI opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Rimuru AI refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Rimuru AI is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Rimuru AI — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Rimuru AI prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Rimuru AI polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with rimuru-ai. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
The `limit` fields allow Rimuru AI to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `rimuru-ai auth list` to see if the credentials
...
2. For custom providers, check the rimuru-ai config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your rimuru-ai config.
