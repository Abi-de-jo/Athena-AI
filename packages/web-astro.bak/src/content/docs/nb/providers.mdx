---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Rimuru AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Rimuru AI bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Rimuru AI-konfigurasjonen.
...
i `~/.local/share/rimuru-ai/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Rimuru AI
konfig.
...
Rimuru AI Zen er en liste over modeller levert av Rimuru AI-teamet som har vært
testet og verifisert for å fungere godt med Rimuru AI. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Rimuru AI Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Rimuru AI Zen` og gå til [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Det fungerer som alle andre leverandører i Rimuru AI og er helt valgfritt å bruke.

---

## Rimuru AI Go

Rimuru AI Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Rimuru AI-teamet som har vært
testet og verifisert for å fungere godt med Rimuru AI.

1. Kjør kommandoen `/connect` i TUI, velg `Rimuru AI Go`, og gå til [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Slik bruker du Amazon Bedrock med Rimuru AI:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Rimuru AI:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i rimuru-ai støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere rimuru-ai til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Rimuru AI skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Rimuru AI-konfigurasjonen.
...
for å lagre token i Rimuru AI auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Rimuru AI bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Rimuru AI
...
Mer dokumentasjon på [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth) hjemmeside.
...
Slik bruker du GitHub Copilot-abonnementet med Rimuru AI:
...
Slik bruker du Google Vertex AI med Rimuru AI:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Rimuru AI.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Rimuru AI, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Rimuru AI-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Rimuru AI-samtale oppført som en separat økt.
...
Du kan konfigurere Rimuru AI for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Rimuru AI for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Rimuru AI for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Rimuru AI. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/rimuru-ai) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Rimuru AI:
...
3. Kopier API-nøkkelen for bruk i Rimuru AI.
...
6. **Viktig**: Før du bruker skymodeller i Rimuru AI, må du hente modellinformasjonen lokalt:
...
Rimuru AI Zen er en liste over testede og verifiserte modeller levert av Rimuru AI-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Rimuru AI Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Rimuru AI API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Rimuru AI-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Rimuru AI-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Rimuru AI:
...
Du kan også tilpasse modeller gjennom Rimuru AI-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Rimuru AI. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar Rimuru AI forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `rimuru-ai auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Rimuru AI-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Rimuru AI-konfigurasjonen.
