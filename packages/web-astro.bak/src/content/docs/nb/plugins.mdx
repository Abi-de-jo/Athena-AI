...
description: Skriv dine egne programtillegg for å utvide Rimuru AI.
---

Plugins lar deg utvide Rimuru AI ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Rimuru AI.
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/rimuru-ai/node_modules/`.
...
1. Global konfigurasjon (`~/.config/rimuru-ai/opencode.json`)
2. Prosjektkonfigurasjon (`opencode.json`)
3. Global plugin-katalog (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Rimuru AI SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Rimuru AI.
...
:::note
Hvis du bruker Rimuru AI-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Rimuru AI fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Rimuru AI:
...
`tool`-hjelperen lager et tilpasset verktøy som Rimuru AI kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Rimuru AI sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://rimurucode.vercel.app/docs/sdk) for detaljer.
