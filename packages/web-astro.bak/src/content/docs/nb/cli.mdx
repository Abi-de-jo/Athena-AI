---
title: CLI
description: Rimuru AI CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Rimuru AI CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
rimuru-ai
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Rimuru AI programmatisk.
...
Start Rimuru AI-terminalbrukergrensesnittet.
...
Administrer agenter for Rimuru AI.
...
Koble en terminal til en allerede kjørende Rimuru AI-backend-server startet via `serve` eller `web` kommandoer.

```bash
rimuru-ai attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Rimuru AI-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `rimuru-ai`) |
...
Rimuru AI drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `rimuru-ai auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

Når Rimuru AI starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Rimuru AI.
...
Kjør Rimuru AI i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `rimuru-ai serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende Rimuru AI-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `rimuru-ai`) |
...
Start en headless Rimuru AI-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
rimuru-ai serve
```

Dette starter en HTTP-server som gir API tilgang til Rimuru AI-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `rimuru-ai`).
...
Vis tokenbruk og kostnadsstatistikk for Rimuru AI-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Rimuru AI share-URL.
...
Start en headless Rimuru AI-server med et webgrensesnitt.

```bash
rimuru-ai web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Rimuru AI via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `rimuru-ai`).
...
Avinstaller Rimuru AI og fjern alle relaterte filer.
...
Oppdaterer Rimuru AI til siste versjon eller en spesifikk versjon.
...
Rimuru AI CLI bruker følgende globale flagg.
...
Rimuru AI kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `rimuru-ai`) |
