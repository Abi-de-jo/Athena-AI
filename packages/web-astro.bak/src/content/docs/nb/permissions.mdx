...
Rimuru AI bruker `permission`-konfigurasjonen for å bestemme om en gitt handling skal kjøres automatisk, spørre deg eller blokkeres.
...
Bruk `external_directory` for å tillate verktøyanrop som berører stier utenfor arbeidskatalogen der Rimuru AI ble startet. Dette gjelder alle verktøy som tar en bane som input (for eksempel `read`, `edit`, `glob`, `grep` og mange `bash`-kommandoer).
...
Rimuru AI-tillatelser tastes inn etter verktøynavn, pluss et par sikkerhetsvakter:
...
Hvis du ikke spesifiserer noe, starter Rimuru AI fra tillatte standardinnstillinger:
...
Når Rimuru AI ber om godkjenning, gir brukergrensesnittet tre alternativer:

- `once` — godkjenne nettopp denne forespørselen
- `always` — godkjenn fremtidige forespørsler som samsvarer med de foreslåtte mønstrene (for resten av den nåværende Rimuru AI-økten)
