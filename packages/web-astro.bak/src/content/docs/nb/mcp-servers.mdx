...
Du kan legge til eksterne verktøy i Rimuru AI ved å bruke _Model Context Protocol_ eller MCP. Rimuru AI støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Rimuru AI Config](https://rimurucode.vercel.app/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Rimuru AI håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Rimuru AI:
...
Hvis serveren krever autentisering, vil Rimuru AI be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `rimuru-ai mcp auth <server-name>`.
...
```bash
rimuru-ai mcp auth my-oauth-server
```
...
```bash
rimuru-ai mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Rimuru AI lagre tokens sikkert i `~/.local/share/rimuru-ai/mcp-auth.json`.
...
rimuru-ai mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Rimuru AI, sammen med innebygde verktøy. Så du kan administrere dem gjennom Rimuru AI-konfigurasjonen som alle andre verktøy.
...
```bash
rimuru-ai mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Rimuru AI til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Rimuru AI-samtale oppført som en separat økt.
