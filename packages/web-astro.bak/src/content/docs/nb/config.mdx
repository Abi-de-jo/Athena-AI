---
title: Konfigurasjon
description: Ved å bruke rimuru-ai JSON-konfigurasjonen.
---

Du kan konfigurere Rimuru AI ved å bruke en JSON konfigurasjonsfil.

---

## Format

Rimuru AI støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global konfigurasjon** (`~/.config/rimuru-ai/opencode.json`) - brukerinnstillinger
...
:::note
`.opencode` og `~/.config/rimuru-ai` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale Rimuru AI-konfigurasjon i `~/.config/rimuru-ai/opencode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Når Rimuru AI starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Konfigurasjonsfilen har et skjema som er definert i [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `rimuru-ai serve` og `rimuru-ai web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Rimuru AI-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Rimuru AI-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Rimuru AI å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/rimuru-ai/agents/` eller `.opencode/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Rimuru AI falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`rimuru-ai run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/rimuru-ai/commands/` eller `.opencode/commands/`. [Les mer her](/docs/commands).
...
Rimuru AI vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Rimuru AI alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Rimuru AI med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.opencode/plugins/` eller `~/.config/rimuru-ai/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Rimuru AI til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
