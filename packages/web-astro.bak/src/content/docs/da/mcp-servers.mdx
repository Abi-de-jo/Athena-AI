...
Du kan tilføje eksterne værktøjer til Rimuru AI ved hjælp af _Model Context Protocol_ eller MCP. Rimuru AI understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Rimuru AI Config](https://rimurucode.vercel.app/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Rimuru AI håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Rimuru AI:
...
Hvis serveren kræver godkendelse, vil Rimuru AI bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `rimuru-ai mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Rimuru AI opbevare tokens sikkert i `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Rimuru AI-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Rimuru AI til din Sentry-konto.
