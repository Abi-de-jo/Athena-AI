...
For at fejlfinde problemer med Rimuru AI, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%.local\share\rimuru-ai\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `rimuru-ai --log-level DEBUG`.

---

## Lagring

rimuru-ai gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/rimuru-ai/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%.local\share\rimuru-ai`
...
Rimuru AI Desktop kører en lokal Rimuru AI-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Rimuru AI`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/rimuru-ai/opencode.jsonc` (eller `~/.config/rimuru-ai/opencode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc`
...
Rimuru AI kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/rimuru-ai/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\rimuru-ai\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Rimuru AI kan genopbygge den.

1. Afslut Rimuru AI Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/rimuru-ai`
- **Linux**: slet `~/.cache/rimuru-ai` (eller kør `rm -rf ~/.cache/rimuru-ai`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\rimuru-ai`

3. Start Rimuru AI Desktop på ny.

---

### Løs problemer med serverforbindelse

Rimuru AI Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
På Windows kræver Rimuru AI Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Rimuru AI.

---

### Meddelelser vises ikke

Rimuru AI Desktop viser kun systemvarsler når:

- varsler er aktiveret for Rimuru AI i dine OS-indstillinger, og
...
1. Afslut Rimuru AI Desktop.
2. Find og slet disse filer (de findes i Rimuru AI Desktop-appens datamappe):

- `rimuru-ai.settings.dat` (skrivebordsstandardserver URL)
- `rimuru-ai.global.dat` og `rimuru-ai.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Rimuru AI:
...
For at finde ud af hvilke modeller du har adgang til, kør `rimuru-ai models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%.local\share\rimuru-ai`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. rimuru-ai installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\rimuru-ai`

2. Start rimuru-ai på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge rimuru-ai til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
rimuru-ai vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
