---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Rimuru AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Rimuru AI bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Rimuru AI-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/rimuru-ai/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Rimuru AI-konfigurationen.
...
Rimuru AI Zen er en liste over modeller leveret af Rimuru AI-teamet, som er blevet testet og verificeret til at fungere godt med Rimuru AI. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Rimuru AI Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg rimuru-ai og gå til [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/auth).
...
Det fungerer som alle andre udbydere i Rimuru AI og er helt valgfrit at bruge.

---

## Rimuru AI Go

Rimuru AI Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Rimuru AI-teamet, som er testet og verificeret til at fungere godt med Rimuru AI.

1. Kør kommandoen `/connect` i TUI, vælg `Rimuru AI Go`, og gå til [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Sådan bruger du Amazon Bedrock med Rimuru AI:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører rimuru-ai:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Rimuru AI understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere rimuru-ai til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at rimuru-ai skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem rimuru-ai-konfigurationen.
...
gemme tokenet i rimuru-ai auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Rimuru AI bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Rimuru AI
...
Mere dokumentation på [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth) hjemmesiden.
...
Sådan bruger du GitHub Copilot-abonnementet med Rimuru AI:
...
Sådan bruger du Google Vertex AI med Rimuru AI:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører rimuru-ai.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem rimuru-ai, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Rimuru AI-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Rimuru AI-samtale opført som en separat session.
...
Du kan konfigurere rimuru-ai til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere rimuru-ai til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere rimuru-ai til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Rimuru AI. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/rimuru-ai) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Rimuru AI:
...
3. Kopier API-nøglen til brug i Rimuru AI.
...
6. **Vigtigt**: Før du bruger skymodeller i Rimuru AI, skal du hente modelinformationen lokalt:
...
Rimuru AI Zen er en liste over testede og verificerede modeller leveret af Rimuru AI-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Rimuru AI Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Indtast Rimuru AI API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem rimuru-ai-konfigurationen.
...
5. Du kan også tilpasse dem gennem rimuru-ai-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Rimuru AI:
...
Du kan også tilpasse modeller gennem rimuru-ai-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med rimuru-ai. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
`limit`-felterne lader Rimuru AI forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `rimuru-ai auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek rimuru-ai-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i rimuru-ai-konfigurationen.
