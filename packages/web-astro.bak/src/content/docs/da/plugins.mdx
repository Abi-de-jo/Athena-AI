---
title: Plugins
description: Skriv dine egne plugins for at udvide Rimuru AI.
---

Plugins giver dig mulighed for at udvide Rimuru AI ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Rimuru AIs standardadfærd.
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/rimuru-ai/node_modules/`.
...
1. Global konfiguration (`~/.config/rimuru-ai/opencode.json`)
2. Projektkonfiguration (`opencode.json`)
3. Global plugin-mappe (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En rimuru-ai SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide rimuru-ai.
...
:::note
Hvis du bruger Rimuru AI desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå rimuru-ai i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til rimuru-ai:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som rimuru-ai kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for rimuru-ai sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://rimurucode.vercel.app/docs/sdk) for detaljer.
