---
title: Konfiguration
description: Ved at bruge Rimuru AI JSON konfig.
---

Du kan konfigurere Rimuru AI ved hjælp af en JSON-konfigurationsfil.

---

## Format

Rimuru AI understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global config** (`~/.config/rimuru-ai/opencode.json`) - brugerpræferencer
...
:::note
`.opencode` og `~/.config/rimuru-ai` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale Rimuru AI-konfiguration i `~/.config/rimuru-ai/opencode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Når Rimuru AI starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Konfigurationsfilen har et skema, der defineres i [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `rimuru-ai serve` og `rimuru-ai web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Rimuru AI-server.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Rimuru AI-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Rimuru AI at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Rimuru AI-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/rimuru-ai/agents/` eller `.opencode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Rimuru AI falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`rimuru-ai run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/rimuru-ai/commands/` eller `.opencode/commands/`. [Learn more here](/docs/commands).
...
Rimuru AI vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard rimuru-ai **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Rimuru AI med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.opencode/plugins/` eller `~/.config/rimuru-ai/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Rimuru AI til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
