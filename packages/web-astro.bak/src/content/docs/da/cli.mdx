---
title: CLI
description: Rimuru AI CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Rimuru AI CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
rimuru-ai
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Rimuru AI programmatisk.
...
Start Rimuru AI terminalbrugergrænsefladen.
...
Rimuru AI CLI har også følgende kommandoer.

---

### agent

Administrer agenter i Rimuru AI.
...
Tilslut en terminal til en allerede kørende Rimuru AI backend-server startet via `serve` eller `web` kommandoer.

```bash
rimuru-ai attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Rimuru AI backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `rimuru-ai`) |
...
Rimuru AI drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `rimuru-ai auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

Når Rimuru AI starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Rimuru AI.
...
Kør rimuru-ai i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `rimuru-ai serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende rimuru-ai-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `rimuru-ai`)   |
...
Start en hovedløs Rimuru AI-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
rimuru-ai serve
```

Dette starter en HTTP-server, der giver API-adgang til rimuru-ai-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `rimuru-ai`).
...
Administrator Rimuru AI sessionsholder.
...
Liste over alle Rimuru AI sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Rimuru AI-sessioner.
...
Importer sessionsdata fra en JSON fil eller Rimuru AI del URL.

```bash
rimuru-ai import <file>
```

Du kan importere fra en lokal fil eller en Rimuru AI share URL.
...
Start en hovedløs Rimuru AI-server med en webgrænseflade.

```bash
rimuru-ai web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Rimuru AI via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `rimuru-ai`).
...
Afinstaller Rimuru AI og fjern alle relaterede filer.
...
Opdaterer Rimuru AI til den nyeste version eller en specifik version.
...
rimuru-ai CLI tager følgende globale flag.
...
Rimuru AI kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `rimuru-ai`) |
