---
title: Server
description: Interact with rimuru-ai server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `rimuru-ai serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an rimuru-ai client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `rimuru-ai`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `rimuru-ai serve` and `rimuru-ai web`.
...
When you run `rimuru-ai` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the rimuru-ai server to interact with rimuru-ai programmatically.
:::

This architecture lets rimuru-ai support multiple clients and allows you to interact with rimuru-ai programmatically.

You can run `rimuru-ai serve` to start a standalone server. If you have the
rimuru-ai TUI running, `rimuru-ai serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Rimuru AI [IDE](/docs/ide) plugins.
...
The rimuru-ai server exposes the following APIs.
