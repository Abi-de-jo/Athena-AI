...
rimuru-ai는 주어진 동작이 자동으로 실행되는지 결정하기 위해 `permission` config를 사용하거나 차단합니다.
...
`external_directory`를 사용하여 도구가 rimuru-ai가 시작된 작업 디렉토리 밖에 터치 경로가 호출되도록합니다. 이것은 입력 (예 : `read`, `edit`, `glob`, `grep` 및 많은 `bash` 명령)로 경로를 수행하는 모든 도구에 적용됩니다.
...
rimuru-ai 권한은 도구 이름에 의해 키 입력되며, 두 개의 안전 가드 :
...
아무것도 지정하지 않는 경우, rimuru-ai는 permissive 기본값에서 시작합니다.
...
rimuru-ai가 승인되면 UI는 세 가지 결과를 제공합니다.

- `once` - 이 요청을 승인
- `always` - 제안 된 패턴과 일치하는 미래 요청 (현재 rimuru-ai 세션의 나머지)
- `reject` - 요청을 거부
