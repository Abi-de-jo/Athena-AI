---
title: 플러그인
description: Rimuru AI를 확장하기 위해 자신만의 플러그인을 작성하세요.
---

플러그인은 다양한 이벤트와 사용자 정의 행동으로 후킹하여 rimuru-ai를 확장 할 수 있습니다. 플러그인을 사용하여 새로운 기능을 추가하거나, 외부 서비스와 통합하거나, rimuru-ai의 기본 동작을 수정할 수 있습니다.
...
- `~/.config/rimuru-ai/plugins/` - 글로벌 플러그인
...
**npm 플러그인**은 시작시 Bun을 사용하여 자동으로 설치됩니다. 패키지와 그 의존성은 `~/.cache/rimuru-ai/node_modules/`에 캐시됩니다.
...
1. 글로벌 구성 (`~/.config/rimuru-ai/opencode.json`)
2. 프로젝트 구성 (`opencode.json`)
3. 글로벌 플러그인 디렉토리 (`~/.config/rimuru-ai/plugins/`)
...
rimuru-ai는 시작 시 `bun install`을 실행하여 이를 설치합니다. 이후 플러그인 및 도구에서 가져올 수 있습니다.
...
- `client`: AI와 상호 작용을 위한 rimuru-ai SDK 클라이언트.
...
rimuru-ai를 확장하기 위해 사용할 수 있는 플러그인 예제입니다.
...
:::note
rimuru-ai 데스크톱 앱을 사용하는 경우 응답이 준비되어 있거나 세션 오류가 있을 때 시스템 알림을 자동으로 보낼 수 있습니다.
:::
...
플러그인은 rimuru-ai에 사용자 정의 도구를 추가할 수 있습니다:
...
`tool` helper는 rimuru-ai가 호출할 수 있는 사용자 정의 도구를 만듭니다. Zod 스키마 함수를 받고 다음을 포함하는 도구 정의를 반환합니다:

- `description`: 도구가 하는 일
- `args`: 도구의 인수에 대한 Zod 스키마
- `execute`: 도구가 호출될 때 실행되는 함수

사용자 정의 도구는 내장 도구와 함께 rimuru-ai에서 사용할 수 있습니다.
...
레벨: `debug`, `info`, `warn`, `error`. [SDK 문서](https://rimurucode.vercel.app/docs/sdk)를 참고하세요.
