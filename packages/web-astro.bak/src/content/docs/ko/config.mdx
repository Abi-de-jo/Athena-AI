...
JSON config 파일로 Rimuru AI를 설정할 수 있습니다.

---

## 형식

Rimuru AI는 **JSON**과 **JSONC**(주석이 포함된 JSON) 형식을 모두 지원합니다.
...
2. **Global config**(`~/.config/rimuru-ai/opencode.json`) - 사용자 기본 설정
...
:::note
`.opencode` 및 `~/.config/rimuru-ai` 디렉토리는 하위 디렉토리에 **복수형 이름**을 사용합니다: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, `themes/`. 단수형 이름(예: `agent/`)도 하위 호환성을 위해 지원합니다.
:::
...
전역 Rimuru AI config는 `~/.config/rimuru-ai/opencode.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.

TUI 관련 설정은 `~/.config/rimuru-ai/tui.json`을 사용하세요.
...
Rimuru AI 시작 시 현재 디렉토리에서 config 파일을 찾고, 없으면 가장 가까운 Git 디렉토리까지 상위로 탐색합니다.
...
server/runtime config schema는 [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json)에 정의되어 있습니다.

TUI config는 [**`rimurucode.vercel.app/tui.json`**](https://rimurucode.vercel.app/tui.json)을 사용합니다.
...
`server` 옵션으로 `rimuru-ai serve`와 `rimuru-ai web` 명령의 server 설정을 구성할 수 있습니다.
...
- `mdns` - mDNS service discovery를 활성화합니다. 네트워크 내 다른 기기에서 Rimuru AI server를 찾을 수 있습니다.
...
Rimuru AI config의 `provider`, `model`, `small_model` 옵션으로 사용할 provider와 model을 설정할 수 있습니다.
...
`small_model`은 제목 생성 같은 경량 작업에 사용할 별도 model을 설정합니다. 기본적으로 Rimuru AI는 provider에서 더 저렴한 model을 사용할 수 있으면 우선 사용하고, 없으면 메인 model로 fallback합니다.
...
`~/.config/rimuru-ai/agents/` 또는 `.opencode/agents/`의 Markdown 파일로 agent를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
기본 agent는 primary agent여야 합니다(subagent 불가). `"build"`, `"plan"` 같은 내장 agent나 직접 정의한 [custom agent](/docs/agents)를 지정할 수 있습니다. 지정한 agent가 없거나 subagent이면 Rimuru AI는 경고와 함께 `"build"`로 fallback합니다.

이 설정은 TUI, CLI(`rimuru-ai run`), 데스크톱 앱, GitHub Action 등 모든 인터페이스에 적용됩니다.
...
`~/.config/rimuru-ai/commands/` 또는 `.opencode/commands/`의 Markdown 파일로 command를 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
Rimuru AI는 시작 시 새 업데이트를 자동으로 다운로드합니다. `autoupdate` 옵션으로 비활성화할 수 있습니다.
...
기본적으로 Rimuru AI는 **명시적 승인 없이 모든 작업을 허용**합니다. `permission` 옵션으로 이 동작을 바꿀 수 있습니다.
...
[Plugins](/docs/plugins)는 custom tool, hook, integration으로 Rimuru AI를 확장합니다.

plugin 파일은 `.opencode/plugins/` 또는 `~/.config/rimuru-ai/plugins/`에 두세요. `plugin` 옵션으로 npm plugin을 로드할 수도 있습니다.
...
provider를 하나씩 비활성화하는 대신, Rimuru AI가 특정 provider만 사용하도록 제한하고 싶을 때 유용합니다.
