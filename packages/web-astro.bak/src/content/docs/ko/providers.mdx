---
title: 공급자
description: Rimuru AI에서 LLM 공급자를 사용합니다.
---

import config from "../../../../config.mjs"
export const console = config.console

Rimuru AI는 [AI SDK](https://ai-sdk.dev/) 및 [Models.dev](https://models.dev)를 사용하여 **75개 이상의 LLM 공급자**를 지원하며 로컬 모델도 실행할 수 있습니다.

필요한 공급자를 추가하려면:

1. `/connect` 명령을 사용하여 공급자의 API 키를 추가하십시오.
2. Rimuru AI 구성에서 공급자를 구성하십시오.
...
`~/.local/share/rimuru-ai/auth.json`

---

### 구성

Rimuru AI 설정의 `provider` 섹션을 통해 공급자를 사용자 정의할 수 있습니다.
...
Rimuru AI Zen은 Rimuru AI 팀이 Rimuru AI와 잘 작동하도록 테스트하고 검증한 모델 목록입니다. [더 알아보기](/docs/zen).

:::tip
처음이라면 Rimuru AI Zen으로 시작하는 것이 좋습니다.
:::

1. TUI에서 `/connect` 명령을 실행하고 `Rimuru AI Zen`을 선택한 뒤, [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen)로 이동합니다.
...
Rimuru AI의 다른 공급자처럼 작동하며 사용은 완전히 선택 사항입니다.

---

## Rimuru AI Go

Rimuru AI Go는 Rimuru AI 팀이 테스트하고 검증하여 Rimuru AI와 잘 작동하는 인기 있는 오픈 코딩 모델에 안정적으로 액세스할 수 있는 저렴한 구독 요금제입니다.

1. TUI에서 `/connect` 명령을 실행하고 `Rimuru AI Go`를 선택한 뒤 [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen)로 이동하십시오.
...
Rimuru AI로 Amazon Bedrock을 사용하려면:
...
2. 다음 방법 중 하나를 사용하여 **인증을 구성**합니다:
   ...
   ***
   ...
   Rimuru AI를 실행하는 동안 다음 환경 변수 중 하나를 설정합니다:
...
:::info
Rimuru AI에서 Claude Pro/Max 구독을 사용하는 것은 [Anthropic](https://anthropic.com)에서 공식적으로 지원하지 않습니다.
:::
...
OpenAI 호환 API 서버 뒤에서 로컬 LLM을 실행하는 데스크톱 애플리케이션인 [Atomic Chat](https://atomic.chat)을 통해 로컬 모델을 사용하도록 rimuru-ai를 구성할 수 있습니다(기본 엔드포인트 `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)로 이동하여 모델을 배포합니다.
   ...
   :::note
   배포 이름은 제대로 작동하려면 Rimuru AI의 모델 이름과 일치해야 합니다.
   :::
...
5. 모델을 선택하려면 `/models` 명령을 실행하십시오.
   ...
   Rimuru AI 구성을 통해 모델을 추가할 수 있습니다.
...
:::note[규정 준수 참고 사항]
Rimuru AI는 세션 제목 생성과 같은 일부 AI 작업을 위해 작은 모델을 사용합니다.
기본적으로 Zen에서 호스팅되는 gpt-5-nano를 사용하도록 구성됩니다.
Rimuru AI를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `opencode.json` 파일에 다음을 추가하십시오.
...
[rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth) 홈페이지에 추가 문서가 있습니다.
...
Rimuru AI에서 GitHub Copilot 구독을 사용하려면:
...
Rimuru AI로 Google Vertex AI를 사용하려면:
...
Rimuru AI를 통해 자동으로 구성되지 않는 Helicone의 기능이나 모델이 있는 경우 직접 구성할 수 있습니다.
...
Helicone의 [Sessions](https://docs.helicone.ai/features/sessions) 기능을 사용하면 관련 LLM 요청을 그룹화할 수 있습니다. [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) 플러그인을 사용하여 각 Rimuru AI 대화를 Helicone 세션으로 자동 기록하십시오.
...
이 플러그인은 `Helicone-Session-Id` 및 `Helicone-Session-Name` 헤더를 요청에 주입합니다. Helicone의 세션 페이지에서 각 Rimuru AI 대화가 별도의 세션으로 나열되는 것을 볼 수 있습니다.
...
:::tip
Ollama는 Rimuru AI에 대해 자동으로 구성될 수 있습니다. 자세한 내용은 [Ollama 통합 문서](https://docs.ollama.com/integrations/rimuru-ai)를 참조하십시오.
:::
...
6. **중요**: Rimuru AI에서 클라우드 모델을 사용하기 전에, 로컬에서 모델 정보를 가져와야 합니다:
...
Rimuru AI Zen은 Rimuru AI 팀에서 제공하는 테스트 및 검증된 모델 목록입니다. [더 알아보기](/docs/zen).

1. **<a href={console}>Rimuru AI Zen</a>**에 로그인하고 **Create API Key**를 클릭합니다.

2. `/connect` 명령을 실행하고 **Rimuru AI Zen**을 검색하십시오.

   ```txt
   /connect
   ```

3. Rimuru AI API 키를 입력하십시오.
...
4. 많은 OpenRouter 모델은 기본적으로 미리 로드되어 있으므로 `/models` 명령을 실행하여 원하는 것을 선택하십시오.
   ...
   Rimuru AI 구성을 통해 모델을 추가할 수도 있습니다.
...
5. 또한 Rimuru AI 구성을 통해 사용자 정의할 수도 있습니다. 다음은 공급자 순서를 지정하는 예입니다.
...
Rimuru AI로 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)를 사용하려면:
...
Rimuru AI 구성을 통해 모델을 사용자 정의할 수도 있습니다. 다음은 공급자 라우팅 순서를 지정하는 예입니다.
...
:::tip
Rimuru AI에서 모든 OpenAI 호환 공급자를 사용할 수 있습니다. 대부분의 최신 AI 공급자는 OpenAI 호환 API를 제공합니다.
:::
...
`limit` 필드를 사용하면 Rimuru AI가 남은 컨텍스트 양을 파악할 수 있습니다. 표준 공급자는 models.dev에서 자동으로 이를 가져옵니다.
...
1. **인증 설정 확인**: `rimuru-ai auth list`를 실행하여 공급자의 자격 증명이 구성에 추가되었는지 확인하십시오.

   Amazon Bedrock과 같이 인증을 위해 환경 변수에 의존하는 공급자에는 적용되지 않습니다.

2. 사용자 정의 공급자의 경우, Rimuru AI 구성을 확인하고 다음을 수행하십시오:
   - `/connect` 명령에 사용된 공급자 ID가 Rimuru AI 구성의 ID와 일치하는지 확인하십시오.
