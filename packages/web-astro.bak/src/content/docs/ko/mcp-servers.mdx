...
Model Context Protocol , 또는 MCP를 사용하여 rimuru-ai에 외부 도구를 추가할 수 있습니다. rimuru-ai는 로컬 및 원격 서버를 모두 지원합니다.
...
rimuru-ai는 원격 MCP 서버에 대한 OAuth 인증을 자동으로 처리합니다. 서버가 인증이 필요할 때, rimuru-ai는:
...
서버가 인증이 필요한 경우, rimuru-ai는 먼저 사용하려고 할 때 인증됩니다. 그렇지 않다면, `rimuru-ai mcp auth <server-name>`를 사용하여 [수동으로 트리거](#authenticating)할 수 있습니다.
...
`mcp auth` 명령은 사용자의 브라우저를 엽니다. `~/.local/share/rimuru-ai/mcp-auth.json`에서 토큰을 안전하게 저장합니다.
...
MCP는 rimuru-ai의 도구로 사용할 수 있으며 내장 도구와 함께 사용할 수 있습니다. 다른 도구와 같이 rimuru-ai config를 통해 관리할 수 있습니다.
...
브라우저 창을 열어 OAuth 흐름을 완료하고 rimuru-ai를 Sentry 계정에 연결하십시오.
