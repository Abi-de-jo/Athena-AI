...
rimuru-ai는 [AI SDK](https://ai-sdk.dev/) 및 [Models.dev](https://models.dev)를 사용하여 **75+ LLM 제공 업체**를 지원하며 로컬 모델을 실행합니다.

---

## 공급자

가장 인기 있는 공급자는 기본적으로 미리 로드되어 있습니다. `/connect` 명령을 통해 공급자를위한 자격 증명을 추가하면 rimuru-ai를 시작할 때 사용할 수 있습니다.
...
전체 ID는 `provider_id/model_id`입니다. 예를 들어, [Rimuru AI Zen](/docs/zen)을 사용한다면, GPT 5.1 Codex에 `rimuru-ai/gpt-5.1-codex`를 사용할 수 있습니다.
...
많은 모델이 다양한 구성의 변형(variant)을 지원합니다. rimuru-ai는 인기있는 공급자를위한 기본 변형으로 배송됩니다.

## 내장 변형

rimuru-ai는 많은 공급자를 위한 기본 변종으로 발송합니다:
...
rimuru-ai가 시작될 때, 다음의 우선순위 순서에 있는 모형을 검사합니다:

1. `--model` 또는 `-m` 명령 선 깃발. 형식은 구성 파일과 동일합니다. `provider_id/model_id`.

2. rimuru-ai config의 모델 목록.
