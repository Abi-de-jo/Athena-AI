---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Rimuru AI.
---

Dodaci vam omogućavaju da proširite Rimuru AI spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Rimuru AI.
...
- `~/.config/rimuru-ai/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/rimuru-ai/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/rimuru-ai/opencode.json`)
2. Konfiguracija projekta (`opencode.json`)
3. Globalni direktorij dodataka (`~/.config/rimuru-ai/plugins/`)
...
Rimuru AI pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Rimuru AI.
...
Spriječite rimuru-ai da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u rimuru-ai:
...
Pomoćnik `tool` kreira prilagođeni alat koji rimuru-ai može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za rimuru-ai zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://rimurucode.vercel.app/docs/sdk) za detalje.
