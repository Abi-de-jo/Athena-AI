---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Rimuru AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Rimuru AI koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Rimuru AI konfiguraciji.
...
u `~/.local/share/rimuru-ai/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Rimuru AI
config.
...
Rimuru AI Zen je lista modela koje je obezbedio Rimuru AI tim koji su bili
testirano i potvrđeno da dobro radi sa Rimuru AI. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Rimuru AI Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite rimuru-ai i idite na [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/auth).
...
Radi kao i svaki drugi provajder u Rimuru AI i potpuno je opcionalan za korištenje.

---

## Rimuru AI Go

Rimuru AI Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Rimuru AI tim i koji su testirani i verificirani da dobro rade s Rimuru AI-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Rimuru AI Go` i idite na [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Da biste koristili Amazon Bedrock s Rimuru AI:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete rimuru-ai:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Rimuru AI.
:::
...
Možete konfigurirati rimuru-ai za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi rimuru-ai ispravno radio.
   :::
...
Također možete dodati modele kroz svoju rimuru-ai konfiguraciju.
...
da pohrani token u rimuru-ai auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Rimuru AI koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Rimuru AI
...
Više dokumentacije na početnoj stranici [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth).
...
Da biste koristili svoju GitHub Copilot pretplatu s rimuru-ai:
...
Za korištenje Google Vertex AI s Rimuru AI:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće rimuru-ai.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem rimuru-aia, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Rimuru AI razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Rimuru AI razgovor naveden kao zasebna sesija.
...
Možete konfigurirati rimuru-ai za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati rimuru-ai za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati rimuru-ai za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Rimuru AI. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/rimuru-ai) za detalje.
:::
...
Da biste koristili Ollama Cloud s Rimuru AI:
...
3. Kopirajte API ključ za korištenje u Rimuru AI.
...
6. **Važno**: Prije upotrebe modela oblaka u Rimuru AI, morate lokalno povući informacije o modelu:
...
Rimuru AI Zen je lista testiranih i verifikovanih modela koju je obezbedio Rimuru AI tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Rimuru AI Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Rimuru AI Zen**.

```txt
   /connect
```

3. Unesite svoj Rimuru AI API ključ.
...
Također možete dodati dodatne modele putem vaše rimuru-ai konfiguracije.
...
5. Također ih možete prilagoditi putem vaše rimuru-ai konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju rimuru-ai konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s rimuru-ai-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
Polja `limit` omogućavaju Rimuru AI da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `rimuru-ai auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Rimuru AI konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj rimuru-ai konfiguraciji.
