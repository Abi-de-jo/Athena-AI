---
title: CLI
description: Rimuru AI CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Rimuru AI CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
rimuru-ai
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa Rimuru AI.
...
Pokrenite Rimuru AI terminalski korisnički interfejs.
...
Rimuru AI CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte Rimuru AI agentima.
...
Priključite terminal na već pokrenut Rimuru AI backend server pokrenut putem `serve` ili `web` naredbi.

```bash
rimuru-ai attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim Rimuru AI backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `rimuru-ai`) |
...
Rimuru AI pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `rimuru-ai auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/rimuru-ai/auth.json`.

```bash
rimuru-ai auth login
```

Kada se Rimuru AI pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Rimuru AI.
...
Pokrenite Rimuru AI u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `rimuru-ai serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti Rimuru AI server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `rimuru-ai`) |
...
Pokrenite Rimuru AI headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
rimuru-ai serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti Rimuru AI-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `rimuru-ai`).
...
Upravljajte Rimuru AI sesijama.
...
Prikaži statistiku upotrebe tokena i troškova za vaše Rimuru AI sesije.
...
Uvezite podatke sesije iz JSON datoteke ili Rimuru AI dijeljenog URL-a.

```bash
rimuru-ai import <file>
```

Možete uvesti iz lokalne datoteke ili Rimuru AI dijeljenog URL-a.
...
Pokrenite Rimuru AI headless server sa web interfejsom.

```bash
rimuru-ai web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup Rimuru AI-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `rimuru-ai`).
...
Deinstalirajte Rimuru AI i uklonite sve povezane datoteke.
...
Ažurira Rimuru AI na najnoviju verziju ili određenu verziju.
...
Rimuru AI CLI prihvata sljedeće globalne zastavice.
...
Rimuru AI se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `rimuru-ai`) |
