...
Da biste otklonili probleme s Rimuru AI, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%.local\share\rimuru-ai\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `rimuru-ai --log-level DEBUG`.

---

## Pohrana

Rimuru AI pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/rimuru-ai/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%.local\share\rimuru-ai`
...
Rimuru AI Desktop pokreće lokalni Rimuru AI server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Rimuru AI` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/rimuru-ai/opencode.jsonc` (ili `~/.config/rimuru-ai/opencode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/rimuru-ai/opencode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\rimuru-ai\opencode.jsonc`
...
Rimuru AI također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/rimuru-ai/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\rimuru-ai\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Rimuru AI mogao ponovo izgraditi.

1. Potpuno zatvorite Rimuru AI Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/rimuru-ai`
- **Linux**: obrišite `~/.cache/rimuru-ai` (ili pokrenite `rm -rf ~/.cache/rimuru-ai`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\rimuru-ai`

3. Ponovo pokrenite Rimuru AI Desktop.

---

### Rješavanje problema sa vezom na serveru

Rimuru AI Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, Rimuru AI Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Rimuru AI karakteristikama.

---

### Obavještenja se ne prikazuju

Rimuru AI Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Rimuru AI u postavkama vašeg OS-a, i
...
1. Zatvorite Rimuru AI Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Rimuru AI Desktop aplikacije):

- `rimuru-ai.settings.dat` (URL zadanog servera za desktop)
- `rimuru-ai.global.dat` i `rimuru-ai.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Rimuru AI:
...
Da saznate kojim modelima imate pristup, pokrenite `rimuru-ai models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%.local\share\rimuru-ai`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Rimuru AI dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\rimuru-ai`

2. Ponovo pokrenite Rimuru AI da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Rimuru AI da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Rimuru AI će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
