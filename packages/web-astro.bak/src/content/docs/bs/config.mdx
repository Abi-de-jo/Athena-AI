---
title: Konfiguracija
description: Korištenje Rimuru AI JSON konfiguracije.
---

Možete konfigurirati Rimuru AI koristeći JSON konfiguracijski fajl.

---

## Format

Rimuru AI podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
2. **Globalna konfiguracija** (`~/.config/rimuru-ai/opencode.json`) - korisničke preferencije
...
:::note
Direktoriji `.opencode` i `~/.config/rimuru-ai` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu Rimuru AI konfiguraciju u `~/.config/rimuru-ai/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/rimuru-ai/tui.json`.
...
Kada se Rimuru AI pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).

TUI konfiguracija koristi [**`rimurucode.vercel.app/tui.json`**](https://rimurucode.vercel.app/tui.json).
...
Možete konfigurirati postavke servera za naredbe `rimuru-ai serve` i `rimuru-ai web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Rimuru AI server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Rimuru AI konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Rimuru AI pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/rimuru-ai/agents/` ili `.opencode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Rimuru AI će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`rimuru-ai run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/rimuru-ai/commands/` ili `.opencode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Rimuru AI će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Rimuru AI **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Rimuru AI sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.opencode/plugins/` ili `~/.config/rimuru-ai/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Rimuru AI da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
