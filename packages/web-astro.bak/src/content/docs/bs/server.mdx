---
title: Server
description: Komunicirajte s rimuru-ai serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `rimuru-ai serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji rimuru-ai klijent moze koristiti.
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `rimuru-ai`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `rimuru-ai serve` i za `rimuru-ai web`.
...
Kada pokrenete `rimuru-ai`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite rimuru-ai server za programsku interakciju sa rimuru-ai-om.
:::

Ova arhitektura omogucava rimuru-ai podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `rimuru-ai serve` da startate standalone server. Ako je rimuru-ai TUI vec pokrenut, `rimuru-ai serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Rimuru AI [IDE](/docs/ide) pluginovi.
...
rimuru-ai server izlaže sljedece API-je.
