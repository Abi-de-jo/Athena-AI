...
Možete dodati vanjske alate u Rimuru AI koristeći _Model Context Protocol_, ili MCP. Rimuru AI podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Rimuru AI Config](https://rimurucode.vercel.app/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Rimuru AI automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Rimuru AI će:
...
Ako server zahtijeva autentifikaciju, Rimuru AI će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `rimuru-ai mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Rimuru AI sigurno čuva tokene u `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Rimuru AI, zajedno s ugrađenim alatima. Možete njima upravljati kroz Rimuru AI konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Rimuru AI sa vašim Sentry nalogom.
