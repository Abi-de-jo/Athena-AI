...
Postoje dvije vrste agenata u Rimuru AI: primarni agenti i podagenti.
...
Rimuru AI dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Rimuru AI dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Rimuru AI dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Rimuru AI-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
- Globalno: `~/.config/rimuru-ai/agents/`
...
Ako temperatura nije navedena, Rimuru AI koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Rimuru AI konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Rimuru AI konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Rimuru AI Zen](/docs/zen), koristili biste `rimuru-ai/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `rimuru-ai models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/gowdaman-dev/rimuru-ai).
:::
