---
title: Плагины
description: Напишите свои собственные плагины для расширения rimuru-ai.
---

Плагины позволяют расширять rimuru-ai, подключаясь к различным событиям и настраивая поведение. Вы можете создавать плагины для добавления новых функций, интеграции с внешними сервисами или изменения поведения rimuru-ai по умолчанию.
...
- `~/.config/rimuru-ai/plugins/` — глобальные плагины
...
**Плагины npm** устанавливаются автоматически с помощью Bun при запуске. Пакеты и их зависимости кэшируются в `~/.cache/rimuru-ai/node_modules/`.
...
1. Глобальная конфигурация (`~/.config/rimuru-ai/opencode.json`)
2. Конфигурация проекта (`opencode.json`)
3. Глобальный каталог плагинов (`~/.config/rimuru-ai/plugins/`)
...
rimuru-ai запускает `bun install` при запуске для их установки. Затем ваши плагины и инструменты смогут импортировать их.
...
Вот несколько примеров плагинов, которые вы можете использовать для расширения rimuru-ai.
...
:::note
Если вы используете настольное приложение rimuru-ai, оно может автоматически отправлять системные уведомления, когда ответ готов или когда возникает ошибка сеанса.
:::
...
Плагины также могут добавлять в rimuru-ai собственные инструменты:
...
Помощник `tool` создает собственный инструмент, который может вызывать rimuru-ai. Он принимает функцию схемы Zod и возвращает определение инструмента:
...
Уровни: `debug`, `info`, `warn`, `error`. Подробности см. в документации SDK](https://rimurucode.vercel.app/docs/sdk).
