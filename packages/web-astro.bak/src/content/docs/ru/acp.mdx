---
title: Поддержка ACP
description: Используйте rimuru-ai в любом ACP-совместимом редакторе.
---

rimuru-ai поддерживает [Agent Client Protocol](https://agentclientprotocol.com) (ACP), что позволяет использовать его непосредственно в совместимых редакторах и IDE.
...
Чтобы использовать rimuru-ai через ACP, настройте свой редактор для запуска команды `rimuru-ai acp`.

Команда запускает rimuru-ai как ACP-совместимый подпроцесс, который взаимодействует с вашим редактором через JSON-RPC через stdio.
...
Чтобы открыть его, используйте новый агент rimuru-ai в селекторе агентов AI Chat.
...
Чтобы использовать rimuru-ai в качестве агента ACP в [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), добавьте в конфигурацию Neovim следующее:
...
Эта конфигурация настраивает CodeCompanion для использования rimuru-ai в качестве агента ACP для чата.
...
rimuru-ai через ACP работает так же, как и в терминале. Поддерживаются все функции:
...
- Серверы MCP, настроенные в вашей конфигурации rimuru-ai
