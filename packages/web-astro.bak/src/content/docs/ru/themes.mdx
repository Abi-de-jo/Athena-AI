...
С помощью rimuru-ai вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию rimuru-ai использует нашу собственную тему `rimuru-ai`.
...
rimuru-ai поставляется с несколькими встроенными темами.
...
- Хотите, чтобы rimuru-ai соответствовал внешнему виду их терминала
...
rimuru-ai поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/rimuru-ai/themes/*.json` или `$XDG_CONFIG_HOME/rimuru-ai/themes/*.json`.
