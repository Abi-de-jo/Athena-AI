---
title: Провайдеры
description: Использование любого провайдера LLM в rimuru-ai.
---

import config from "../../../../config.mjs"
export const console = config.console

rimuru-ai использует [AI SDK](https://ai-sdk.dev/) и [Models.dev](https://models.dev) для поддержки **более 75 поставщиков LLM** и поддерживает запуск локальных моделей.

Чтобы добавить провайдера, вам необходимо:

1. Добавьте ключи API для провайдера с помощью команды `/connect`.
2. Настройте провайдера в вашей конфигурации rimuru-ai.
...
в `~/.local/share/rimuru-ai/auth.json`.

---

### Настройка

Вы можете настроить поставщиков через раздел `provider` в вашем rimuru-ai.
конфиг.
...
Rimuru AI Zen — это список моделей, предоставленный командой rimuru-ai, которые были
протестировано и проверено на хорошую работу с rimuru-ai. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с Rimuru AI Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `Rimuru AI Zen` и перейдите по адресу [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Он работает как любой другой поставщик в rimuru-ai и его использование совершенно необязательно.

---

## Rimuru AI Go

Rimuru AI Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой rimuru-ai, которые были
протестированы и проверены на хорошую работу с rimuru-ai.

1. Запустите команду `/connect` в TUI, выберите `Rimuru AI Go` и перейдите по адресу [rimurucode.vercel.app/auth](https://rimurucode.vercel.app/zen).
...
Чтобы использовать Amazon Bedrock с rimuru-ai:
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Установите одну из этих переменных среды при запуске rimuru-ai:
...
:::info
Использование вашей подписки Claude Pro/Max в rimuru-ai официально не поддерживается [Anthropic](https://anthropic.com).
:::
...
Вы можете настроить rimuru-ai для работы с локальными моделями через [Atomic Chat](https://atomic.chat) — десктопное приложение, которое запускает локальные LLM за OpenAI-совместимым API-сервером (конечная точка по умолчанию `http://127.0.0.1:1337/v1`).
...
2. Перейдите в [Azure AI Foundry](https://ai.azure.com/) и разверните модель.
   ...
   :::примечание
   Для правильной работы rimuru-ai имя развертывания должно совпадать с именем модели.
   :::
...
5. Запустите команду `/models`, чтобы выбрать модель.
   ...
   Вы также можете добавлять модели через конфигурацию rimuru-ai.
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `Rimuru AI`, Области применения: `api`
...
для хранения токена в хранилище аутентификации rimuru-ai.
:::

##### Самостоятельная GitLab

:::note[примечание о соответствии]
rimuru-ai использует небольшую модель для некоторых задач ИИ, таких как создание заголовка сеанса.
По умолчанию он настроен на использование gpt-5-nano, размещенного на Zen. Чтобы заблокировать rimuru-ai
...
Дополнительная документация на домашней странице [rimuru-ai-gitlab-auth](https://www.npmjs.com/package/rimuru-ai-gitlab-auth).
...
Чтобы использовать Google Vertex AI с rimuru-ai:
...
2. Установите необходимые переменные среды:
   ...
   Установите их во время запуска rimuru-ai.
...
Если вы видите функцию или модель от Helicone, которая не настраивается автоматически через rimuru-ai, вы всегда можете настроить ее самостоятельно.
...
Функция Helicone [Sessions](https://docs.helicone.ai/features/sessions) позволяет группировать связанные запросы LLM вместе. Используйте плагин [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), чтобы автоматически регистрировать каждый диалог rimuru-ai как сеанс в Helicone.
...
Плагин вставляет в ваши запросы заголовки `Helicone-Session-Id` и `Helicone-Session-Name`. На странице «Сеансы» Helicone вы увидите каждый диалог rimuru-ai, указанный как отдельный сеанс.
...
Вы можете настроить rimuru-ai для использования локальных моделей с помощью [утилиты llama-server llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Вы можете настроить rimuru-ai для использования локальных моделей через LM Studio.
...
Вы можете настроить rimuru-ai для использования локальных моделей через Ollama.

:::tip
Ollama может автоматически настроиться для rimuru-ai. Подробности см. в документации по интеграции Ollama](https://docs.ollama.com/integrations/rimuru-ai).
:::
...
Чтобы использовать Ollama Cloud с rimuru-ai:
...
3. Скопируйте ключ API для использования в rimuru-ai.
...
6. **Важно**. Перед использованием облачных моделей в rimuru-ai необходимо получить информацию о модели локально:
...
Rimuru AI Zen — это список протестированных и проверенных моделей, предоставленный командой rimuru-ai. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>Rimuru AI Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **Rimuru AI Zen**.

   ```txt
   /connect
   ```

3. Введите свой ключ API rimuru-ai.
...
4. Многие модели OpenRouter предварительно загружены по умолчанию. Запустите команду `/models`, чтобы выбрать нужную.
   ...
   Вы также можете добавить дополнительные модели через конфигурацию rimuru-ai.
...
5. Вы также можете настроить их через конфигурацию rimuru-ai. Вот пример указания провайдера
...
Чтобы использовать [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) с rimuru-ai:
...
Вы также можете настраивать модели через конфигурацию rimuru-ai. Ниже приведен пример указания порядка маршрутизации поставщика.
...
Поля `limit` позволяют rimuru-ai понять, сколько контекста у вас осталось. Стандартные поставщики автоматически извлекают их из models.dev.
...
1. **Проверьте настройку аутентификации**: запустите `rimuru-ai auth list`, чтобы проверить, верны ли учетные данные.
...
2. Для пользовательских поставщиков проверьте конфигурацию rimuru-ai и:
   - Убедитесь, что идентификатор провайдера, используемый в команде `/connect`, соответствует идентификатору в вашей конфигурации rimuru-ai.
