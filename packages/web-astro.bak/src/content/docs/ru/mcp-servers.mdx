...
Вы можете добавить внешние инструменты в rimuru-ai, используя _Model Context Protocol_ или MCP. rimuru-ai поддерживает как локальные, так и удаленные серверы.
...
Вы можете определить серверы MCP в своем [rimuru-ai Config](https://rimurucode.vercel.app/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
...
rimuru-ai автоматически обрабатывает аутентификацию OAuth для удаленных серверов MCP. Когда серверу требуется аутентификация, rimuru-ai:
...
Если сервер требует аутентификации, rimuru-ai предложит вам пройти аутентификацию при первой попытке его использования. Если нет, вы можете [вручную запустить поток ](#authenticating) с помощью `rimuru-ai mcp auth <server-name>`.
...
Команда `mcp auth` откроет ваш браузер для авторизации. После того как вы авторизуетесь, rimuru-ai надежно сохранит токены в `~/.local/share/rimuru-ai/mcp-auth.json`.
...
Ваши MCP доступны в виде инструментов rimuru-ai наряду со встроенными инструментами. Таким образом, вы можете управлять ими через конфигурацию rimuru-ai, как и любым другим инструментом.
...
Откроется окно браузера для завершения процесса OAuth и подключения rimuru-ai к вашей учетной записи Sentry.
