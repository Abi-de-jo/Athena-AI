---
title: Конфигурация
description: Использование конфигурации rimuru-ai JSON.
---

Вы можете настроить rimuru-ai, используя файл конфигурации JSON.

---

## Формат

rimuru-ai поддерживает форматы **JSON** и **JSONC** (JSON с комментариями).
...
2. **Глобальная конфигурация** (`~/.config/rimuru-ai/opencode.json`) — настройки пользователя.
...
:::note
В каталогах `.opencode` и `~/.config/rimuru-ai` для подкаталогов используются **множественные имена**: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` и `themes/`. Единственные имена (например, `agent/`) также поддерживаются для обратной совместимости.
:::
...
Поместите глобальную конфигурацию rimuru-ai в `~/.config/rimuru-ai/opencode.json`. Используйте глобальную конфигурацию для общепользовательских настроек, таких как темы, поставщики или привязки клавиш.

Для настроек, специфичных для TUI, используйте `~/.config/rimuru-ai/tui.json`.
...
Когда rimuru-ai запускается, он ищет файл конфигурации в текущем каталоге или переходит к ближайшему каталогу Git.
...
Файл конфигурации имеет схему, определенную в [**`rimurucode.vercel.app/config.json`**](https://rimurucode.vercel.app/config.json).

Конфигурация TUI использует [**`rimurucode.vercel.app/tui.json`**](https://rimurucode.vercel.app/tui.json).
...
Вы можете настроить параметры сервера для команд `rimuru-ai serve` и `rimuru-ai web` с помощью опции `server`.
...
- `mdns` — включить обнаружение службы mDNS. Это позволит другим устройствам в сети обнаружить ваш сервер rimuru-ai.
...
Вы можете настроить поставщиков и модели, которые хотите использовать в своей конфигурации rimuru-ai, с помощью параметров `provider`, `model` и `small_model`.
...
Опция `small_model` настраивает отдельную модель для облегченных задач, таких как создание заголовков. По умолчанию rimuru-ai пытается использовать более дешевую модель, если она доступна у вашего провайдера, в противном случае он возвращается к вашей основной модели.
...
Вы также можете определить агентов, используя файлы Markdown в `~/.config/rimuru-ai/agents/` или `.opencode/agents/`. [Подробнее здесь](/docs/agents).
...
Агент по умолчанию должен быть основным агентом (а не субагентом). Это может быть встроенный агент, например `"build"` или `"plan"`, или [пользовательский агент](/docs/agents), который вы определили. Если указанный агент не существует или является субагентом, rimuru-ai вернется к `"build"` с предупреждением.

Этот параметр применяется ко всем интерфейсам: TUI, CLI (`rimuru-ai run`), настольному приложению и действию GitHub.
...
Вы также можете определять команды, используя файлы Markdown в `~/.config/rimuru-ai/commands/` или `.opencode/commands/`. [Подробнее здесь](/docs/commands).
...
rimuru-ai автоматически загрузит все новые обновления при запуске. Вы можете отключить это с помощью опции `autoupdate`.
...
По умолчанию rimuru-ai **разрешает все операции**, не требуя явного разрешения. Вы можете изменить это, используя опцию `permission`.
...
[Плагины](/docs/plugins) расширяют rimuru-ai с помощью пользовательских инструментов, перехватчиков и интеграций.

Поместите файлы плагина в `.opencode/plugins/` или `~/.config/rimuru-ai/plugins/`. Вы также можете загружать плагины из npm с помощью опции `plugin`.
...
Это полезно, если вы хотите ограничить rimuru-ai использованием только определенных поставщиков, а не отключать их по одному.
