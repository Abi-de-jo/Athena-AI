...
function View(props: { api: TuiPluginApi; sessionID: string }) {
  ...
  const has = createMemo(() =>
    props.api.state.provider.some(
      (item) => item.id !== "rimuru-ai" || Object.values(item.models).some((model) => model.cost?.input !== 0),
    ),
  )
  ...
  return (
    <box gap={1}>
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>Rimuru includes free models so you can start immediately.</text>
      ...
      <text fg={theme().textMuted}>
        <span style={{ fg: theme().primary }}>(◕‿◕)</span> <b>Rimuru</b>{" "}
