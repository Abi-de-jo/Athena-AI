...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("rimuru.status"),
...
const TIPS: Tip[] = [
  ...
  "Create {highlight}rimuru.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/rimuru/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.rimuru/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.rimuru/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.rimuru/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.rimuru/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Rimuru from reading sensitive files",
  "Use {highlight}rimuru run{/highlight} for non-interactive scripting",
  "Use {highlight}rimuru --continue{/highlight} to resume the last session",
  "Use {highlight}rimuru run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}rimuru serve{/highlight} for headless API access to Rimuru",
  "Use {highlight}rimuru run --attach{/highlight} to connect to a running server",
  "Run {highlight}rimuru upgrade{/highlight} to update to the latest version",
  "Run {highlight}rimuru auth list{/highlight} to see all configured providers",
  "Run {highlight}rimuru agent create{/highlight} for guided agent creation",
  "Use {highlight}/rimuru{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}rimuru github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/rimuru fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.rimuru/themes/{/highlight} directory",
  ...
  "Run {highlight}rimuru debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/gowdaman/rimuru{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Rimuru Zen for curated, tested models",
