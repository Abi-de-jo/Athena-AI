import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@rimurucode-ai/plugin/tui"
...
import type { useRimuruKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useRimuruKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getRimuruModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getRimuruModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
