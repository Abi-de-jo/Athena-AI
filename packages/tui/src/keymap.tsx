...
export const RIMURU_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const RIMURU_MODE_KEY = "rimuru.mode"

export const RimuruKeymapProvider = KeymapProvider
export const useRimuruKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type RimuruModeStack = ReturnType<typeof createRimuruModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, RimuruModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createRimuruModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(RIMURU_MODE_KEY, RIMURU_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(RIMURU_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(RIMURU_MODE_KEY, stack.at(-1)?.mode ?? RIMURU_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? RIMURU_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(RIMURU_MODE_KEY, undefined)
...
export function useRimuruModeStack() {
  return getRimuruModeStack(useRimuruKeymap())
}

export function getRimuruModeStack(keymap: OpenTuiKeymap) {
...
export function registerRimuruKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createRimuruModeStack(keymap)
