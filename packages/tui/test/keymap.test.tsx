...
import { getRimuruModeStack, RIMURU_BASE_MODE, RimuruKeymapProvider, registerRimuruKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerRimuruKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when rimuru mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getRimuruModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getRimuruModeStack(keymap).push("autocomplete")
