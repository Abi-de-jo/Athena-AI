...
- `native-request.ts` converts opencode's normalized session input into a native `@rimuru-ai/llm` `LLMRequest`. It does not execute requests.
...
src/session/
  ...
  llm/
    ...
    native-request.ts       opencode/AI SDK-shaped input -> @rimuru-ai/llm LLMRequest
...
`native-runtime.ts` evaluates the gate and either bridges into `@rimuru-ai/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays opencode-owned in both branches; only request lowering and transport differ.
